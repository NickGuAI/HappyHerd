...
| Minimal Linux boot adapter — `deploy/happyherd-daemon.cron`, `scripts/start-host-daemon.sh` | Host reboot | Invokes installed `happyherd daemon start` and exits | Daemon absent after reboot | Does not own daemon/provider processes | Happy detached daemon lifecycle | **APPROVED TO KEEP** |
