...
const EXPECTED_REPOSITORY = 'ghcr.io/nickguai/happyherd';
...
export function validateReleaseManifest(manifest) {
    ...
    if (manifest.repository !== EXPECTED_REPOSITORY) {
        fail(`repository must be ${EXPECTED_REPOSITORY}`);
    }
...
function fixture() {
    const slot = (sourceSha, digestChar) => ({
        image: `${EXPECTED_REPOSITORY}@sha256:${digestChar.repeat(64)}`,
        immutableTag: `${EXPECTED_REPOSITORY}:sha-${sourceSha}`,
        sourceSha,
        smoke: {
...
function selfTest() {
    ...
    mutable.current.image = `${EXPECTED_REPOSITORY}:latest`;
    try {
