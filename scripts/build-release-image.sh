...
REPOSITORY="${HAPPYHERD_IMAGE_REPOSITORY:-ghcr.io/nickguai/happyherd}"
...
docker build \
    ...
    --build-arg "HAPPYHERD_SOURCE_URL=https://github.com/NickGuAI/HappyHerd" \
...
actual_brand="$(docker image inspect --format '{{ index .Config.Labels "ai.gehirn.happyherd.brand.sha256" }}' "$IMAGE_TAG")"
