...
CONFIG_ROOT=/etc/pmai-discord-agent
...
install -d -o "$AGENT_USER" -g "$AGENT_USER" -m 0700 \
    ...
    "$AGENT_ROOT/happy-home/commanders/pmai-team-agent" \
    "$AGENT_ROOT/workspace"

install -o "$AGENT_USER" -g "$AGENT_USER" -m 0600 \
    "$ROOT/deploy/pmai-discord-agent-runtime/happy-home/AGENTS.md" \
    "$AGENT_ROOT/happy-home/AGENTS.md"
install -o "$AGENT_USER" -g "$AGENT_USER" -m 0600 \
    "$ROOT/deploy/pmai-discord-agent-runtime/happy-home/agentcontext/USER.md" \
    "$AGENT_ROOT/happy-home/agentcontext/USER.md"
install -o "$AGENT_USER" -g "$AGENT_USER" -m 0600 \
    "$ROOT/deploy/pmai-discord-agent-runtime/happy-home/commanders/pmai-team-agent/COMMANDER.md" \
    "$AGENT_ROOT/happy-home/commanders/pmai-team-agent/COMMANDER.md"
install -o "$AGENT_USER" -g "$AGENT_USER" -m 0600 \
    "$ROOT/deploy/pmai-discord-agent-runtime/workspace/AGENTS.md" \
...
if [[ ! -e "$AGENT_ROOT/happy-home/settings.json" ]]; then
    install -o "$AGENT_USER" -g "$AGENT_USER" -m 0600 \
        "$ROOT/deploy/pmai-discord-agent-runtime/settings.template.json" \
        "$AGENT_ROOT/happy-home/settings.json"
fi

if [[ ! -e "$CONFIG_ROOT/bridge.env" ]]; then
    install -o root -g "$BRIDGE_USER" -m 0640 \
        "$ROOT/deploy/pmai-discord-agent.env.example" \
        "$CONFIG_ROOT/bridge.env"
fi
if [[ ! -e "$CONFIG_ROOT/daemon.env" ]]; then
    install -o root -g "$AGENT_USER" -m 0640 \
        "$ROOT/deploy/pmai-happyherd-daemon.env.example" \
        "$CONFIG_ROOT/daemon.env"
fi

install -o root -g root -m 0644 \
    "$ROOT/deploy/pmai-happyherd-daemon.cron" \
    /etc/cron.d/pmai-happyherd-daemon
install -o root -g root -m 0644 \
    "$ROOT/deploy/pmai-discord-agent.service" \
    /etc/systemd/system/pmai-discord-agent.service
systemctl daemon-reload

printf 'PMAI Discord runtime prepared. Install fresh credentials and replace all bridge.env placeholders before enablement.\n'
