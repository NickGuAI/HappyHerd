...
if ! command -v bwrap >/dev/null 2>&1 \
    ...
    if [[ "$MODE" == source ]]; then
        printf 'agent sandbox broker canary skipped: bwrap, rg, and socat are host-only dependencies.\n'
        exit 0
    fi
    printf 'error: agent sandbox broker canary requires bwrap, rg, and socat\n' >&2
    exit 1
fi
...
const server = createServer((_request, response) => response.end('agent-broker-ok'));
const readOnlyDirectory = await mkdtemp('/var/tmp/happyherd-agent-sandbox-readonly-');
const workspaceDirectory = await mkdtemp('/var/tmp/happyherd-agent-sandbox-workspace-');
...
try {
  ...
  await SandboxManager.initialize({
    network: {
      allowedDomains: ['happyherd-agent-broker.localhost'],
      deniedDomains: [],
  ...
  const childProgram = [
    ...
    `const response = await request("http://happyherd-agent-broker.localhost:${address.port}", { dispatcher: new ProxyAgent(proxy) });`,
    'const body = await response.body.text();',
    'if (response.statusCode === 200 && body === "agent-broker-ok") process.exit(0);',
    'process.exit(2);',
  ].join(' ');
  ...
  if (result.status !== 0) {
    throw new Error(`sandboxed agent broker request failed: ${result.stderr.trim()}`);
  }
} finally {
...
printf 'agent sandbox broker canary passed.\n'
