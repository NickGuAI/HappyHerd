...
grep -Fq 'release-bundled ripgrep is missing' "$ROOT/scripts/prepare-happyherd-agent-runtime.sh" || fail 'runtime preparation does not require bundled ripgrep'
grep -Fq 'release-bundled ripgrep is unusable' "$ROOT/scripts/validate-happyherd-agent-runtime.sh" || fail 'runtime validator does not execute bundled ripgrep as the agent user'
