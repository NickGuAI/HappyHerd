...
ROLLBACK="$ROOT/scripts/rollback-pmai-discord-agent.sh"
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/pmai-discord-rollback-test.XXXXXX")"
...
fail() {
    printf 'pmai-discord-rollback-contract: %s\n' "$*" >&2
    exit 1
}
...
for sha in "$previous_sha" "$target_sha"; do
    ...
    mkdir -p "$release/daemon/bin" "$release/pmai-discord-agent/dist"
    printf '#!/usr/bin/env node\n' > "$release/daemon/bin/happy.mjs"
    printf 'export {};\n' > "$release/pmai-discord-agent/dist/index.mjs"
...
# shellcheck disable=SC2016
printf '%s\n' '#!/usr/bin/env bash' 'printf "systemctl %s\n" "$*" >> "$PMAI_ROLLBACK_EVENT_LOG"' > "$TMP_ROOT/bin/systemctl"
# shellcheck disable=SC2016
printf '%s\n' \
    '#!/usr/bin/env bash' \
    '[[ "$1" == -u ]] || exit 2' \
...
write_release_stubs() {
    ...
    printf '%s\n' \
        '#!/usr/bin/env bash' \
        'printf "start %s\n" "$(basename "$(dirname "$(dirname "$(readlink -f "$0")")")")" >> "$PMAI_ROLLBACK_EVENT_LOG"' \
        "exit $start_status" \
        > "$release/scripts/start-host-daemon.sh"
    # shellcheck disable=SC2016
    printf '%s\n' \
        '#!/usr/bin/env bash' \
        'printf "validate %s %s\n" "$(basename "$(dirname "$(dirname "$(readlink -f "$0")")")")" "$2" >> "$PMAI_ROLLBACK_EVENT_LOG"' \
        '[[ "$2" == template ]] && exit 0' \
        "exit $validator_status" \
        > "$release/scripts/validate-pmai-discord-agent-runtime.sh"
    # shellcheck disable=SC2016
    printf '%s\n' \
        '#!/usr/bin/env bash' \
        'printf "health %s\n" "$(basename "$(dirname "$(dirname "$(readlink -f "$0")")")")" >> "$PMAI_ROLLBACK_EVENT_LOG"' \
        'exit 0' \
        > "$release/scripts/health-pmai-discord-agent.sh"
...
export PMAI_ROLLBACK_EVENT_LOG="$event_log"
...
[[ "$(grep -Fc 'systemctl start pmai-discord-agent.service' "$event_log")" -ge 1 ]] || fail 'failed rollback did not restore bridge service'

printf 'PMAI Discord Agent rollback contract tests passed.\n'
