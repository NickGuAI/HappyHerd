...
BRIDGE_ENV="${1:-/etc/pmai-discord-agent/bridge.env}"
DAEMON_ENV="${2:-/etc/pmai-discord-agent/daemon.env}"
BRIDGE_USER=pmai-discord-bridge
AGENT_USER=pmai-happyherd-agent
BRIDGE_KEY=/var/lib/pmai-discord-bridge/happy-agent/agent.key
DAEMON_KEY=/var/lib/pmai-happyherd-agent/happy-home/access.key
DAEMON_SETTINGS=/var/lib/pmai-happyherd-agent/happy-home/settings.json
...
export PMAI_PROVISION_HAPPY_SERVER_URL="$HAPPY_SERVER_URL"
export PMAI_PROVISION_BRIDGE_KEY="$BRIDGE_KEY"
export PMAI_PROVISION_DAEMON_KEY="$DAEMON_KEY"
node <<'NODE'
...
const serverUrl = new URL(process.env.PMAI_PROVISION_HAPPY_SERVER_URL);
...
const issueToken = async () => {
  ...
  const response = await fetch(new URL('/v1/auth', serverUrl), {
    ...
    headers: {
      ...
      'x-happy-client': 'pmai-discord-provisioner/1',
    },
    body: JSON.stringify({
...
(async () => {
  ...
  try {
    writeAtomicExclusive(process.env.PMAI_PROVISION_BRIDGE_KEY, {
      token: bridgeToken,
      secret: encodedSecret,
    });
    createdTargets.push(process.env.PMAI_PROVISION_BRIDGE_KEY);
    writeAtomicExclusive(process.env.PMAI_PROVISION_DAEMON_KEY, {
      token: daemonToken,
      secret: encodedSecret,
    });
    createdTargets.push(process.env.PMAI_PROVISION_DAEMON_KEY);
  } catch (error) {
...
NODE
unset PMAI_PROVISION_HAPPY_SERVER_URL PMAI_PROVISION_BRIDGE_KEY PMAI_PROVISION_DAEMON_KEY
...
runuser -u "$AGENT_USER" -- env -i \
    HOME=/var/lib/pmai-happyherd-agent \
    HAPPY_SERVER_URL="$HAPPY_SERVER_URL" \
    HAPPY_WEBAPP_URL="$HAPPY_SERVER_URL" \
    HAPPY_HOME_DIR=/var/lib/pmai-happyherd-agent/happy-home \
...
export PMAI_PROVISION_BRIDGE_ENV="$BRIDGE_ENV"
export PMAI_PROVISION_MACHINE_ID="$machine_id"
node <<'NODE'
const fs = require('node:fs');
const path = process.env.PMAI_PROVISION_BRIDGE_ENV;
const lines = fs.readFileSync(path, 'utf8').split('\n');
let replacements = 0;
const updated = lines.map((line) => {
  if (!line.startsWith('PMAI_HAPPY_MACHINE_ID=')) return line;
  replacements += 1;
  return `PMAI_HAPPY_MACHINE_ID=${process.env.PMAI_PROVISION_MACHINE_ID}`;
});
if (replacements !== 1) throw new Error('bridge environment must contain one PMAI_HAPPY_MACHINE_ID assignment');
fs.writeFileSync(path, updated.join('\n'));
NODE
unset PMAI_PROVISION_BRIDGE_ENV PMAI_PROVISION_MACHINE_ID

runuser -u "$AGENT_USER" -- /opt/happyherd/current/scripts/start-host-daemon.sh "$DAEMON_ENV"
printf 'Dedicated PMAI HappyHerd account, machine, and daemon provisioned without personal credentials.\n'
