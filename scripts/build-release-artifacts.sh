...
export EXPO_PUBLIC_HAPPYHERD_REPOSITORY_DISPLAY="$REPOSITORY_DISPLAY"
export EXPO_PUBLIC_HAPPYHERD_REPOSITORY_URL="$REPOSITORY_URL"
export EXPO_PUBLIC_HAPPYHERD_ISSUE_URL="$ISSUE_URL"
...
pnpm --filter @happyherd/happyherd-agent --fail-if-no-match build

bridge_deploy_log="$STAGE/happyherd-agent-deploy.log"
set +e
pnpm --frozen-lockfile --offline --filter @happyherd/happyherd-agent --fail-if-no-match deploy \
    --prod "$STAGE/happyherd-agent" >"$bridge_deploy_log" 2>&1
...
if [[ "$bridge_deploy_status" -ne 0 ]] && \
    ...
    die 'pnpm could not create the locked HappyHerd Agent deployment payload'
fi
[[ -f "$STAGE/happyherd-agent/package.json" ]] || die 'HappyHerd Agent deployment package is missing'
[[ -f "$STAGE/happyherd-agent/pnpm-lock.yaml" ]] || die 'HappyHerd Agent deployment lockfile is missing'

export DEPLOY_STAGE="$STAGE/happyherd-agent"
node <<'NODE'
const fs = require('node:fs');
const path = require('node:path');
...
rm -f "$STAGE/happyherd-agent/node_modules/.modules.yaml"
while IFS= read -r -d '' shim; do
    resolved="$(readlink -f "$shim")"
    [[ -n "$resolved" && "$resolved" == "$STAGE/happyherd-agent/"* ]] || \
        die "HappyHerd Agent executable shim escapes its archive: $shim"
done < <(find "$STAGE/happyherd-agent/node_modules" -type l -path '*/.bin/*' -print0)
if find "$STAGE/happyherd-agent/node_modules" -type f -path '*/.bin/*' -print -quit | grep -q .; then
    die 'HappyHerd Agent dependency tree contains non-relocatable executable shims'
fi
(
    cd "$STAGE/happyherd-agent"
    node --input-type=module -e \
        "const bridge = await import('./dist/index.mjs'); if (typeof bridge.startHappyHerdAgent !== 'function') process.exit(1)"
)
...
stable_archive happyherd-agent "happyherd-agent-${platform}.tar.gz"
