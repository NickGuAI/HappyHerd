...
const capture=await createCapture({group:'chat',entrySource:readFileSync(resolve(repoRoot,'scripts/kilv-capture/chat.tsx'),'utf8').replaceAll('../../server/packages/happyherd-app/sources/',resolve(appRoot,'sources')+'/'),virtualModules,onResolve(args){
...
try{
 ...
 for(const scene of scenes)for(const theme of themes)for(const width of widths){
  ...
  try{
   ...
   await capture.capture(page,{panelId:scene,label:descriptions[scene][0],sourcePaths:sceneSources[scene].map(p=>'server/packages/happyherd-app/sources/'+p+'.tsx'),state:'synthetic-review',fontEvidence,stateDescription:states[scene]??'Editable and read-only synthetic TypeScript values',limitations,changeDescription:descriptions[scene][1],...(onlineStatusEvidence?{onlineStatusEvidence}:{}),visibleBodyText:(await page.locator('body').innerText()).slice(0,3000)});
   if(['question','tool-question'].includes(scene)){
    await page.getByText('Focused tests',{exact:true}).click();
    await capture.capture(page,{panelId:scene,label:descriptions[scene][0]+' — selected answer',sourcePaths:sceneSources[scene].map(p=>'server/packages/happyherd-app/sources/'+p+'.tsx'),state:'selected-answer',fontEvidence,stateDescription:'Focused tests selected via actual visible option; submit enabled; no submit RPC invoked',limitations,changeDescription:descriptions[scene][1]});
   }
