...
docker run --detach \
    ...
    --env PUBLIC_URL=https://happyherd.example.com \
    --env HANDY_MASTER_SECRET="$master_secret" \
    --env HAPPY_HOME_DIR=/happyherd-cli \
    --env 'HAPPY_INJECT_HTML_CONFIG={"serverUrl":"https://happyherd.example.com","disableAnalytics":true}' \
    "$IMAGE_REF" >/dev/null
...
brand_sha="$(docker image inspect --format '{{ index .Config.Labels "org.happyherd.brand.sha256" }}' "$IMAGE_REF")"
