...
| Credentials & Accounts management | `.dev/COUPLINGS.md`, `.dev/VERIFY.md` | `happyherd-cli` credential pool and authenticated machine RPC for machine-local provider accounts; `happyherd-server` `/v1/credentials` for account-wide saved credentials; `happyherd credentials list|run` as the terminal path for saved credentials; HappyHerd app Settings page |
