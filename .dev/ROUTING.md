...
| CLI command or provider behavior | `server/packages/happy-cli/bin/happy.mjs`, `server/packages/happy-cli/src/index.ts`, provider directory under `server/packages/happy-cli/src/`; GrokBuild uses `src/agent/acp/{acpAgentConfig,runAcp,sessionConfigMetadata}.ts` | `server/packages/happy-cli/src/{capabilities/agentCapabilities,utils/detectCLI}.ts`, daemon, API clients, and app agent selection, attachment, resume, and fork policy | `pnpm --filter happy typecheck` and `pnpm --filter happy test`; for Grok also cover ACP config/capabilities and affected app tests |
...
| Automations | `server/packages/happy-wire/src/automation.ts`; `server/packages/happy-cli/src/automations/{store,service,sessionBootstrap,providerOutcome}.ts`; session command owner `server/packages/happy-app/sources/utils/heartbeatCommand.ts` | Daemon lifecycle/API handlers, `apiSession`, `MessageQueue2`, Claude/Codex queue consumers, and app Session Info/SessionView/automation route/card/ops | Wire, CLI, and app automation tests; heartbeat coverage must include exact-session resume, FIFO isolation, coalescing, and lifecycle receipts |
