...
|---|---|---|
| Codex `turn/plan/updated` | Client scopes to the root thread/turn, deduplicates repeated snapshots and orders timestamps; `nativePlans.ts` maps native `inProgress` to `in_progress` and emits paired `TodoWrite` envelopes | HappyHerd's persisted transcript retains full replacements and explicit empty clears. Native Thread history does not expose these transient progress notifications; do not invent them during native fork. |
