...
| Desktop multi-file workspace | Focus `sources/-session/SessionView.sideChat.test.ts`, `sources/components/{DesktopFileWorkspace,desktopFileWorkspaceModel,sidebarNavigationLayout,sideChatPresentation}.test.ts`, `sources/components/{desktopWorkspace,sideChatHeader.browser,FileViewPanel.native}.test.ts`, `sources/sync/{localSettings,ops.rig}.test.ts`, `packages/happy-cli/src/api/apiMachine.test.ts`, and `packages/happy-cli/src/modules/common/registerCommonHandlers.readFile.test.ts`. Render the production components through their real SessionView host at Web Desktop and a 390 × 844 compact fullscreen width. Require `DesktopFileWorkspace` to be the sole same-session plain-file workspace: chat file links and All Files must converge on the same deduplicated state, with no separate linked-file panel, duplicate header, or second composer. Directory, cross-session, failed-resolution, and line/column links must retain the standalone `WorkspaceLinkViewer` route. Require real DOM clicks and `page.mouse` divider drag, boundary collapse/reopen with no overlap, and preservation of the Main Agent chat, mount identity, scroll position, and unsaved draft. Prove Preview/Edit on wide and compact layouts, with Delete as a separate wide action; prove tabs, picker, deduplication, switching, view-only close, and narrow absence of the desktop tab row and divider; require zero page errors and `console.error` messages. Direct prop calls, source or bundle strings, screenshots alone, and unit tests alone cannot satisfy the interaction gate. Then run full app tests and typecheck, UI inventory, i18n check, production web export, and authenticated deployed-domain interaction proof after release when a safe auth harness exists. |
