...
| Desktop multi-file workspace | Focus `sources/-session/SessionView.sideChat.test.ts`, `sources/components/{DesktopFileWorkspace,desktopFileWorkspaceModel,sidebarNavigationLayout,sideChatPresentation}.test.ts`, `sources/components/{desktopWorkspace.browser,FileViewPanel.native}.test.ts`, `sources/sync/{localSettings,ops.rig}.test.ts`, `packages/happy-cli/src/api/apiMachine.test.ts`, and `packages/happy-cli/src/modules/common/registerCommonHandlers.readFile.test.ts`. Render the production components through their real host at Web Desktop and a 390 × 844 compact fullscreen width. Require real DOM clicks and `page.mouse` divider drag, boundary collapse/reopen with no overlap, and preservation of the Main Agent chat, mount identity, and typed draft. Prove wide Source/Edit/Delete with no separate Preview, confirmation plus removal callback, machine capability publication, exact machine RPC routing, and filesystem unlink; prove compact Source/Preview/Edit with no wide tab row, divider, or Delete; require zero page errors and `console.error` messages. Direct prop calls, source or bundle strings, screenshots alone, and unit tests alone cannot satisfy the interaction gate. Then run full app tests and typecheck, UI inventory, i18n check, production web export, and authenticated deployed-domain interaction proof after release when a safe auth harness exists. |
