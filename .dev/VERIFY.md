...
```bash
cd server
pnpm --filter happyherd-app exec vitest run sources/path/example.test.ts
pnpm --filter @happyherd/cli exec vitest run --project unit src/path/example.test.ts
pnpm --filter ./packages/happyherd-server exec vitest run sources/path/example.test.ts
```
...
|---|---|
| `.dev` context or HappyHerd skill catalog | Run `git diff --check` and `node scripts/lint-source.mjs`. Validate every changed skill with the installed `skill-creator` `quick_validate.py`; resolve every relative Markdown link from its containing file. When the safeguard skill changes, require `diff -ru .dev/skills/happyherd-user-safeguard server/packages/happyherd-cli/skills/happyherd-user-safeguard`. When a selective Claude/Codex reinstall is authorized, compare every installed HappyHerd skill with `.dev/skills` and prove an allowlist-excluding fingerprint of every unrelated installed skill is identical before and after. |
| Safeguard prompt assessment | CLI `src/userSafeguard/userSafeguard.test.ts`, app `sources/components/{safeguardReminder,MessageView}.test.ts`, `sources/components/markdown/{MarkdownView.web,MarkdownView.workspaceLinks}.test.ts`, `sources/components/sideChatHeader.browser.test.ts -t safeguard`; cover valid/invalid/incomplete states, composer send/option/reopen under desktop/mobile light/dark, RN structural parity |
| App logic | `pnpm --filter happyherd-app typecheck`; `pnpm --filter happyherd-app test --run` |
| Native macOS/iOS distribution | Follow `docs/native-app-builds.md`; focus `sources/nativeBuildConfig.test.ts`, run a production iOS bundle and Xcode Release build, and build the Tauri app/DMG. Record the actual bundle ID/version and keep compilation, Apple signing/notarization, device export, and authenticated native journey evidence separate. |
...
| Catalog keys/placeholders | First `pnpm --filter happyherd-app i18n:generate`, review generated changes, then `i18n:check` |
| Route or UI-owning module | First `pnpm --filter happyherd-app ui:inventory:generate`, review generated changes, then `i18n:check` |
| Named project sessions, list views, or sidebar navigation, or New Session/HomeDock launch with project choice | `projectSessionList` unit tests, `storage.projects`, `sessionListGrouping` and `settings`, `projectsSuperSession.browser` tests at 1440x900/390x844, navigation and persisted setting validation, app typecheck, i18n inventory, and export smoke tests. Keep live authenticated proof separate from rendered fixtures. Focus default/No Project/override, assignment before first send, and retained-session retry on both launch paths covered via `sources/sync/newSessionProject.test.ts`, `sources/sync/persistence.spec.ts`, `sources/hooks/useNewSessionDraft.test.ts`, `sources/hooks/useStartSessionFromDraft.test.ts`, `sources/app/(app)/new/index.launch.test.ts`, and Project cases in `sources/components/sideChatHeader.browser.test.ts`. |
...
| Wire protocol | `pnpm --filter @happyherd/wire test`, then affected app/CLI/agent/server consumers |
| HappyHerd CLI/provider/session logic | `pnpm --filter @happyherd/cli typecheck`; `pnpm --filter @happyherd/cli test` |
| Native plans/questions/answers (#295) | Follow [the interaction playbook](playbooks/native-plan-question-interactions.md). Run CLI `codexAppServerClient`, `permissionHandler`, `AcpSessionManager.plan`, native mapper/backfill and runner tests; app `agentCommunications`, `ToolView`, reducer and `nativeQuestions.browser` tests. Require exact root JSON-RPC identity, one settlement, stale-generation rejection, completed receipt reload, plan replace/empty clear and native history identity. Browser component fixtures at 1440×900/390×844 are separate from authenticated provider and SessionView acceptance. Then run CLI/app full checks, i18n/inventory, changelog generation, Web export/smoke and contract suite. |
...
| Session continuity across updates/restarts | `pnpm --filter @happyherd/cli test:session-continuity` must include recent and older-than-14-day records, then run the HappyHerd CLI typecheck/tests |
...
| Agent runtime | `pnpm --filter happyherd-control-agent test`; `pnpm --filter @happyherd/happyherd-agent test` |
| Server | `pnpm --filter ./packages/happyherd-server typecheck`; `test`; `build` |
...
introduces or expands HappyHerd-owned permission enforcement.

## User-visible change gate

Every user-visible HappyHerd change updates
`server/packages/happyherd-app/CHANGELOG.md`. Regenerate its checked-in JSON:
...
grep -F '<title>HappyHerd</title>' \
  packages/happyherd-app/dist-ci/index.html
pnpm --filter happyherd-app web:smoke
pnpm --filter @happyherd/cli build
pnpm --filter ./packages/happyherd-server build
```
...
3. stop, upgrade, and start the daemon as the same host account with the same
   HappyHerd home and environment;
