...
| Side-chat app presentation | Focus `sources/-session/SessionView.sideChat.test.ts`, `sources/components/{SideChatPanel,sideChatPresentation}.test.ts`, and `sources/sync/{sideChatSessions,sessionListVisibility}.test.ts`; prove exact-parent filtering, newest-child focus, tab switching, stopped-child resume, archived hiding, non-destructive collapse, top-level-list exclusion, and both wide-sidebar and narrow/full-screen hosts; then run the full app checks and production web export. |
| Local side-chat lifecycle | Focus `src/commands/{machine,sideChat}.test.ts`, `src/daemon/{controlClient,sideChatLifecycle,run.resume}.test.ts`, `packages/happy-server/sources/app/presence/sessionCache.spec.ts`, and `packages/happy-server/sources/app/api/routes/sessionRoutes.resume.spec.ts`; prove create/list/status/stop/close/reopen, sequential close-all partial receipts and sufficient client budget, active-but-unowned failure, committed-write/lost-ack reconciliation, durable post-restart discovery, exact remote read-back, authenticated resume suppression release, and immediate server reactivation. Then, without an account-control link, run the command sequence in `.dev/playbooks/side-chat-lifecycle.md` and confirm no QR prompt, no stale active child, preserved lineage, and nonzero exit for a failed JSON receipt. |
