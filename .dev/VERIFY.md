...
| Side-chat app presentation | Focus `sources/-session/SessionView.sideChat.test.ts`, `sources/components/{SideChatPanel,sideChatPresentation}.test.ts`, and `sources/sync/{ops.codexFork,sideChatSessions,sessionListVisibility}.test.ts`; prove the persistent right-panel New side chat action at zero children, one-click creation with no fields, parent-only dedicated machine RPC submission, successful empty-child hydrate/focus/open with the normal composer, exact-parent filtering, newest-child focus, tab switching, stopped-child resume, archived hiding, non-destructive collapse, top-level-list exclusion, desktop `Side chats N` click-to-render in the right-panel host, and the same narrow/mobile full-screen selection behavior; then run the full app checks and production web export. |
| Desktop multi-file workspace | Focus `sources/-session/SessionView.sideChat.test.ts`, `sources/components/{DesktopFileWorkspace,desktopFileWorkspaceModel,sidebarNavigationLayout,sideChatPresentation}.test.ts`, `sources/components/FileViewPanel.native.test.ts`, and `sources/sync/localSettings.test.ts`; prove unique-path open/dedupe/focus, active/background/last close, dirty cancel/discard, keyed editor retention through tab/picker/divider interactions, simultaneous Main Agent composer rendering, independent left-navigation collapse, Web/Mac wide parity, and desktop-wide versus Web Mobile presentation; then run the full app checks, UI inventory, and production web export. |
