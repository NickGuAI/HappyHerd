...
| Safeguard prompt assessment | CLI `src/userSafeguard/userSafeguard.test.ts`, app `sources/components/{safeguardReminder,MessageView}.test.ts`, `sources/components/sideChatHeader.browser.test.ts -t safeguard`; cover valid/invalid/incomplete states, composer send/option/reopen under desktop/mobile light/dark, RN structural parity |
