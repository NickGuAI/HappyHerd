...
| File workspace consolidation | Read [the workspace playbook](playbooks/file-workspaces.md) first. At `e1b1180a`, focused `SessionView` coverage proves only same-session plain-file links and All Files converge in `DesktopFileWorkspace`; line/column, cross-session, directory, and failed probes still use `WorkspaceLinkViewer`. The unified target must instead exercise Chat Workspace (the current Main Agent session cwd), Machine Workspace (selected machine-wide browser and its left-navigation entry), and every reply link—including line/column—through one right-side tab/viewer state. Use the real `SessionView` and Machine Workspace hosts at desktop and 390 × 844 widths; click links and both workspace selectors, open a duplicate path, switch tabs, preview/edit/delete, send feedback, and drag the divider with `page.mouse`. Assert one mounted viewer/composer, preserved active chat, draft, scroll, machine/session identity, and location focus; on compact mobile assert the responsive full-screen surface without desktop tabs or divider. Keep explicit failed-resolution UI, but do not add another file viewer. Require zero page errors and `console.error` messages. Then run full app tests/typecheck, UI inventory, `i18n:check`, production web export, and authenticated deployed-domain interaction proof after release when a safe auth harness exists. |
