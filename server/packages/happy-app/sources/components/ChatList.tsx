...
const ChatListInternal = React.memo((props: {
    ...
    const windowedMessages = React.useMemo(() => {
        ...
        const focusIndex = focusRequestKey !== releasedFocusRequestKey && props.focusMessageId
            ? messages.findIndex((msg) => msg.id === props.focusMessageId
                || ('localId' in msg && msg.localId === props.focusMessageId))
            : -1;
        desiredEnd = Math.max(desiredEnd, focusIndex + 1);
        // An exact receipt may live in the oldest incomplete turn. Explicit
        // focus includes it immediately while normal paging stays turn-aligned.
        const end = Math.max(focusIndex + 1, windowEndForTurn(messages, desiredEnd, props.hasMoreOlder));
        const next = messages.slice(0, end);
        ...
        return next;
    }, [messages, oldestRenderedId, props.hasMoreOlder, props.focusMessageId, focusRequestKey, releasedFocusRequestKey]);
    ...
    const isGroupExpanded = useCallback((group: AgentWorkGroupItem) => (
        (focusRequestKey !== releasedFocusRequestKey && group.messages.some((msg) =>
            msg.id === props.focusMessageId || ('localId' in msg && msg.localId === props.focusMessageId)))
        || groupToggles.expanded.has(group.id)
        || ((group.hasPendingPermission || group.hasRunning) && !groupToggles.closed.has(group.id))
    ), [groupToggles, focusRequestKey, releasedFocusRequestKey, props.focusMessageId]);
    ...
    const exactMessageFocusTarget = React.useMemo(
        () => props.focusMessageId
            ? resolveMessageFocusTarget(listItems, props.focusMessageId)
            : null,
        [listItems, props.focusMessageId],
    );
    ...
    const handleFocusScrollToIndexFailed = React.useCallback((info: {
        ...
        const currentTargetIndex = pending
            ? resolveMessageFocusTarget(listItemsRef.current, pending.messageId).index
            : null;
        ...
        listRef.current?.scrollToOffset({ offset: plan.offset, animated: false });
        if (focusScrollRetryTimerRef.current) {
            clearTimeout(focusScrollRetryTimerRef.current);
        }
        focusScrollRetryTimerRef.current = setTimeout(() => {
            ...
            const refreshed = refreshMessageFocusScrollRetryState(
                latest,
                resolveMessageFocusTarget(listItemsRef.current, latest.messageId).index,
            );
            if (!refreshed) return;
            focusScrollRetryRef.current = refreshed;
            try {
                void Promise.resolve(listRef.current?.scrollToIndex({
    ...
    React.useEffect(() => {
        ...
        if (focusRequestKey === releasedFocusRequestKey || !listReadyRef.current) return;
        if (
        ...
        const retryFocus = () => {
            // A drag or a newer focus request can release this async scroll.
            if (focusScrollRetryRef.current?.messageId !== props.focusMessageId) return;
            handleFocusScrollToIndexFailed({
                index: target.index!,
                highestMeasuredFrameIndex: -1,
                averageItemLength: listItems.length > 0
                    ? scrollMetricsRef.current.contentHeight / listItems.length
                    : 0,
            });
        };
        try {
    ...
    }, [agentCopyTextByMessageId, inlineImagesByMessageId, props.metadata, props.sessionId, isGroupExpanded, handleToggleGroup]);
    ...
    const handleScroll = useCallback((e: NativeSyntheticEvent<NativeScrollEvent>) => {
        ...
        const next = exactMessageFocusAnchoredRef.current || distanceFromNewest > SCROLL_THRESHOLD;
    ...
    return (
        <View style={{ flex: 1 }}>
            <FlashList
                ...
                ref={listRef}
                data={listItems}
                // The list uses a native inverted anchor: item 0 is the newest
