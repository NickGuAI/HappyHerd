...
export function parsePatch(patch: string): RawFile[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Until the declared hunk is consumed, ---/+++ are source lines too.
        // For example, changing -- count; to ++ count; produces both prefixes.
        if (hunk && (oldNo < hunk.oldStart + hunk.oldLines || newNo < hunk.newStart + hunk.newLines)
            && appendHunkLine(line)) continue;
        ...
        if (line.startsWith('--- ')) {
            // Plain unified patches can start another file without diff --git.
            if (file && file.hunks.length > 0) pushFile();
