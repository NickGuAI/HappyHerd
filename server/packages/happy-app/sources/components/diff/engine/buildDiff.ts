...
const DEFAULTS: Required<BuildOptions> = {
    contextLines: 3,
    // Preserve syntax colors on ordinary diffs; the size budget below keeps
    // exceptionally large files inexpensive to build.
    syntax: true,
    intraline: true,
    tabWidth: 4,
...
function finish(files: DiffFile[], started: number): DiffDocument {
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__ && doc.buildMs > 4) {
