 ...
 */
import { HUNK_HEADER } from './parsePatch';
...
export function countPatchStats(patch: string): { additions: number; deletions: number } {
    ...
    while (index < patch.length) {
        ...
        if (first === '@' && patch.startsWith('@@', index)) {
            const header = HUNK_HEADER.exec(patch.slice(index, end));
            inHunk = header !== null;
            oldRemaining = header ? Number(header[2] ?? 1) : 0;
            newRemaining = header ? Number(header[4] ?? 1) : 0;
        } else if (inHunk) {
            const unfinished = oldRemaining > 0 || newRemaining > 0;
            if (!unfinished && (patch.startsWith('--- ', index) || patch.startsWith('+++ ', index))) {
                inHunk = false;
            } else if (first === '+') {
                additions++;
                newRemaining--;
                ...
                deletions++;
                oldRemaining--;
            } else if (first === ' ' || index === end) {
                oldRemaining--;
                newRemaining--;
            } else if (first === 'd' && patch.startsWith('diff ', index)) {
                inHunk = false;
            }
        }
        index = end + 1;
