...
export const DiffFileHeader = React.memo(function DiffFileHeader({
    ...
    const content = (
        ...
        >
            ...
            {file.kind === 'added' ? <Badge text={t('uiCopy.new')} color={palette.marker.add} /> : null}
            {file.kind === 'deleted' ? <Badge text={t('uiCopy.deleted')} color={palette.marker.del} /> : null}
            {file.kind === 'renamed' ? <Badge text={t('diff.renamed')} color={palette.textSecondary} /> : null}
            {file.additions > 0 ? (
    ...
    return <Pressable accessibilityRole="button" accessibilityLabel={file.path} aria-expanded={!collapsed} onPress={onToggle}>{content}</Pressable>;
});
