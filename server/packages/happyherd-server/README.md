...
HappyHerd Server is the synchronization backbone for secure Claude Code clients. It enables multiple devices to share encrypted conversations while maintaining complete privacy - the server never sees your messages, only encrypted blobs it cannot read.
...
Your Claude Code clients generate encryption keys locally and use HappyHerd Server as a secure relay. Messages are end-to-end encrypted before leaving your device. The server's job is simple: store encrypted blobs and sync them between your devices in real-time.

## Hosting

**You don't need to self-host!** Our free cloud HappyHerd Server at `happyherd-api.slopus.com` is just as secure as running your own. Since all data is end-to-end encrypted before it reaches our servers, we literally cannot read your messages even if we wanted to. The encryption happens on your device, and only you have the keys.

That said, HappyHerd Server is open source and self-hostable if you prefer running your own infrastructure. The security model is identical whether you use our servers or your own.
...
```bash
docker build -t happyherd-server -f Dockerfile .
```
...
Data persists in the `happyherd-data` Docker volume across container restarts.
...
```bash
# AWS CLI
aws s3api put-bucket-lifecycle-configuration --bucket happyherd-blobs \
...
# MinIO
mc ilm rule add myminio/happyherd-blobs \
  --expire-days 30 \
  --prefix "sessions/"
```
...
```bash
# AWS CLI
aws s3api put-bucket-encryption --bucket happyherd-blobs \
...
# MinIO
mc encrypt set sse-s3 myminio/happyherd-blobs
```
