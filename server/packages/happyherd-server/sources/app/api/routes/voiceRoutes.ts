...
import { VoiceConversationResponseSchema, VoiceUsageResponseSchema } from "@happyherd/wire";
...
function deriveElevenUserId(happyherdUserId: string): string {
    const hmac = crypto.createHmac("sha256", process.env.HANDY_MASTER_SECRET!);
    hmac.update(happyherdUserId);
