...
export async function runMigrations(opts: { pgliteDir: string; migrationsDir?: string } = { pgliteDir }) {
    ...
    candidates.push(
        ...
        path.join(process.cwd(), "packages", "happyherd-server", "prisma", "migrations"),
...
async function serve() {
    ...
    /* rename:preserve */
    const htmlConfig = process.env.HAPPYHERD_INJECT_HTML_CONFIG ?? process.env.HAPPY_INJECT_HTML_CONFIG;
    /* /rename:preserve */
    if (htmlConfig) {
        try {
            injectHtmlConfig = JSON.parse(htmlConfig);
        } catch {
...
function findStaticDir(): string | undefined {
    const candidates = [
        /* rename:preserve */ process.env.HAPPYHERD_STATIC_DIR ?? process.env.HAPPY_STATIC_DIR, /* /rename:preserve */
...
export function isStandaloneEntrypoint(invokedFile: string): boolean {
    ...
    // happyherd-server.exe when tests or tooling run cross-platform.
...
if (isDirectInvocation) {
    ...
    switch (command) {
        ...
        case "serve":
            ...
            break;
        default:
            console.log(`happyherd-server - portable distribution
