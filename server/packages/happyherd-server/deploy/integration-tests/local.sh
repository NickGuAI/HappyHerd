...
echo "=== HappyHerd Local Deployment (minikube) ==="
...
echo "Building happyherd-server:local image..."
docker build -t happyherd-server:local -f "$REPO_ROOT/Dockerfile.server" "$REPO_ROOT"
...
kubectl wait --for=condition=ready pod -l app=happyherd-postgres --timeout=60s

# Run migrations via a one-shot job
kubectl run happyherd-migrate --rm -i --restart=Never \
    --image=happyherd-server:local \
    --image-pull-policy=Never \
    --env="DATABASE_URL=postgresql://happy:happy@happyherd-postgres:5432/happy" \
    -- sh -c "cd /repo && npx prisma migrate deploy --schema=packages/happyherd-server/prisma/schema.prisma" \
    2>/dev/null || true
