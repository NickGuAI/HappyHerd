...
Canonical wire specification package for HappyHerd clients and services.
...
`@happyherd/wire` centralizes definitions for:
...
In this repository, consumer workspaces import `@happyherd/wire` through package exports that point at `dist/*`.

That means on a clean checkout:
1. Build wire first: `yarn workspace @happyherd/wire build`
...
yarn workspace @happyherd/wire build
yarn workspace @happyherd/wire test
```
