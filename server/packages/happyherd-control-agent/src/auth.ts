...
export async function authLogin(config: Config): Promise<void> {
    ...
    const qrData = `happyherd:///account?${encodeBase64Url(keypair.publicKey)}`;
    console.log('');
    qrcode.generate(qrData, { small: true }, (code: string) => {
        console.log(code);
    });
    console.log('## Authentication');
    console.log('- Action: Scan this QR code with the HappyHerd app');
...
export async function authStatus(
    ...
    const loginCommand = options.loginCommand ?? 'happyherd-control-agent auth login';
    console.log('## Authentication');
    if (creds) {
        console.log('- Status: Authenticated');
