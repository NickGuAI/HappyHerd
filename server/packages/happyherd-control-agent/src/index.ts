...
const SUPPORTED_AGENTS: SupportedAgent[] = ['claude', 'codex', 'gemini', 'agy'];
...
function ensureMachineCanResume(machine: DecryptedMachine): void {
    ...
    if (metadata.resumeSupport?.happyAgentAuthenticated === false) {
        throw new Error('Resume is unavailable on this machine. Run `happyherd-control-agent auth login` in that machine environment first.');
    }
...
program
    ...
    .description('CLI client for controlling HappyHerd Coder agents remotely')
    .version('0.1.0');
