...
const binPath = resolve(packageDir, 'bin', 'happyherd-control-agent.mjs');
const happyherdBinPath = resolve(repoRoot, 'packages', 'happyherd-cli', 'bin', 'happyherd.mjs');
const keepIntegrationEnv = ['1', 'true', 'yes'].includes((process.env.HAPPYHERD_AGENT_KEEP_ENV ?? '').toLowerCase());
...
function runHappyHerdCli(args: string[], env: NodeJS.ProcessEnv): string {
    return execFileSync(process.execPath, [
        '--no-warnings',
        '--no-deprecation',
        happyherdBinPath,
...
function createGitProject(envDir: string): { projectDir: string; worktreeDir: string } {
    const projectDir = join(envDir, 'happyherd-control-agent-test-project');
    const worktreeDir = join(projectDir, '.dev', 'worktree', 'feature-branch');

    mkdirSync(projectDir, { recursive: true });
    runCommand('git', ['init', '--initial-branch=main'], projectDir);
    runCommand('git', ['config', 'user.name', 'HappyHerd Agent Test'], projectDir);
    runCommand('git', ['config', 'user.email', 'happyherd-control-agent-tests@example.com'], projectDir);

    writeFile(join(projectDir, 'README.md'), '# HappyHerd Agent Test Project\n');
...
async function waitForSessionInList(sessionId: string, env: NodeJS.ProcessEnv): Promise<void> {
    ...
    }, 20_000, `session ${sessionId} to appear in happyherd-control-agent list`);
}
...
async function runAgentAuthLogin(env: NodeJS.ProcessEnv, approval: { serverUrl: string; token: string; secret: Uint8Array }): Promise<string> {
    return await new Promise<string>((resolvePromise, rejectPromise) => {
        ...
        const timeout = setTimeout(() => {
            ...
            settled = true;
            child.kill('SIGKILL');
            rejectPromise(new Error(`Timed out waiting for happyherd-control-agent auth login.\n${stdout}\n${stderr}`));
        }, 60_000);
        ...
        child.on('close', code => {
            if (code !== 0) {
                finish(new Error(`happyherd-control-agent auth login exited with code ${code}\n${stdout}\n${stderr}`));
                return;
            }
...
describe('happyherd-control-agent integration', { timeout: 180_000 }, () => {
    beforeAll(async () => {
        ...
        if (keepIntegrationEnv) {
            console.log(`[happyherd-control-agent integration] keeping environment: ${integrationEnvName}`);
            console.log(`[happyherd-control-agent integration] environment dir: ${integrationEnvDir}`);
        }
    });
    ...
    it('creates through native happyherd and accepts a first turn on the tracked session', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testProjectDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'native happyherd machine-session message';
        ...
        }>(runHappyHerdCli(['machine', 'list', '--json'], agentEnv));
    ...
    it('spawns in a git worktree and sends a message through happyherd-control-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testWorktreeDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'happyherd-control-agent worktree message';
        const spawnResult = parseJson<{
