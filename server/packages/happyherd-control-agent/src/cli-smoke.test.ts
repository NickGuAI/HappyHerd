/**
 * CLI smoke tests for happyherd-control-agent.
 *
...
describe('Smoke: Error handling', () => {
    ...
    describe('server error handling (via API error mapper)', () => {
        it('HTTP 401 maps to re-authenticate message', async () => {
            ...
            const errorMsg = 'Authentication expired. Run `happyherd-control-agent auth login` to re-authenticate.';
            expect(errorMsg).toContain('happyherd-control-agent auth login');
        });
...
describe('Smoke: Interop — dataKey vs legacy encryption', () => {
    ...
    it('session created with legacy encryption (happyherd-cli style) can be decrypted', () => {
        const creds = makeCredentials();
    ...
    it('messages encrypted with dataKey can be round-tripped', () => {
        const sessionKey = getRandomBytes(32);
        const messageContent = { role: 'user', content: { type: 'text', text: 'Hello from happyherd-control-agent' } };
