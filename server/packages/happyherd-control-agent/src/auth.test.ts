...
function makeTestConfig(): Config {
    const homeDir = mkdtempSync(join(tmpdir(), 'happyherd-control-agent-auth-test-'));
    return {
...
describe('auth', () => {
    ...
    describe('authStatus', () => {
        ...
        it('shows not authenticated when no credentials', async () => {
            ...
            expect(calls).toContain('- Action: Run `happyherd-control-agent auth login` to authenticate.');
        });
        ...
        it('allows a supported caller to name its public login command', async () => {
            ...
            expect(calls.join('\n')).not.toContain('happyherd-control-agent auth login');
        });
