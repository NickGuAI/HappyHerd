# HappyHerd CLI Codebase Overview

## Project Overview

HappyHerd CLI (`handy-cli`) is a command-line tool that wraps Claude Code to enable remote control and session sharing. It's part of a three-component system:
...
## Starting the Daemon
```bash
# From the happyherd-cli directory:
./bin/happyherd.mjs daemon start

# With custom server URL (for local development):
HAPPYHERD_SERVER_URL=http://localhost:3005 ./bin/happyherd.mjs daemon start

# Stop the daemon:
./bin/happyherd.mjs daemon stop

# Check daemon status:
./bin/happyherd.mjs daemon status
```

## Daemon Logs
- Daemon logs are stored in `~/.happyherd-dev/logs/` (or `$HAPPYHERD_HOME_DIR/logs/`)
