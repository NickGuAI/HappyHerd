...
describe('Claude Version Utils - Cross-Platform Detection', () => {
  ...
  describe('resolveClaudeEntrypoint', () => {
    ...
    beforeEach(() => {
      packageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'happyherd-claude-entrypoint-'));
    });
  ...
  describe('detectSourceFromPath', () => {
    ...
    describe('Homebrew installations', () => {
      ...
      it('should detect Homebrew on Linux', () => {
        const result = detectSourceFromPath('/home/example-user/.linuxbrew/bin/claude');
        expect(result).toBe('Homebrew');
      });
  ...
  describe('Integration scenarios', () => {
    ...
    it('should maintain 100% success rate on all standard installation patterns', () => {
      const standardPatterns = [
        ...
        { path: '/opt/homebrew/bin/claude', expected: 'Homebrew' },
        { path: '/home/example-user/.linuxbrew/bin/claude', expected: 'Homebrew' },
...
describe('HAPPYHERD_CLAUDE_PATH env var', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.HAPPYHERD_CLAUDE_PATH;
  });

  it('should use HAPPYHERD_CLAUDE_PATH when set', () => {
    process.env.HAPPYHERD_CLAUDE_PATH = testClaudePath;
    const result = findGlobalClaudeCliPath();
    expect(result?.source).toBe('HAPPYHERD_CLAUDE_PATH');
  ...
  it('should fall back to auto-discovery when env var not set', () => {
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('HAPPYHERD_CLAUDE_PATH');
  });

  it('should ignore env var if path does not exist', () => {
    process.env.HAPPYHERD_CLAUDE_PATH = '/nonexistent/path/claude';
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('HAPPYHERD_CLAUDE_PATH');
  });
});
