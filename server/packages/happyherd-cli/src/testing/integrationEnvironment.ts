...
export function applyEnvironmentToProcess(env: IntegrationEnvironment) {
    process.env.HAPPYHERD_SERVER_URL = `http://localhost:${env.serverPort}`;
    process.env.HAPPYHERD_WEBAPP_URL = `http://localhost:${env.expoPort}`;
    process.env.HAPPYHERD_HOME_DIR = join(env.envDir, 'cli', 'home');
    process.env.HAPPYHERD_PROJECT_DIR = env.projectPath;
    process.env.HAPPYHERD_VARIANT = 'dev';
    process.env.DEBUG = '1';
}
