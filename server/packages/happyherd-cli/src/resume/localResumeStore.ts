...
import {
    ...
    type ReconnectableHappyHerdSession,
} from './resolveHappyHerdSession';
    ...
    try {
        ...
        if (error instanceof AxiosError && error.response?.status === 401) {
            throw new LocalResumeSessionError(
                'HappyHerd session lookup authentication expired. Run `happyherd auth login --force` in this environment.',
                'unavailable',
            );
        }
        return null;
...
export async function resolveLocalReconnectableSession(sessionId: string): Promise<ReconnectableHappyHerdSession> {
    ...
    if (records.length === 0) {
        throw new LocalResumeSessionError(
            `Cannot resume HappyHerd session "${sessionId}" on this machine: no local session encryption data found at ${configuration.sessionsFile}. Start a new HappyHerd session on this machine to enable future resumes.`,
            'not_found',
        );
    }
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : `No HappyHerd session found matching "${sessionId}"`;
        throw new LocalResumeSessionError(
...
export async function backfillReconnectableSessionForMachine(
    ...
    if (!session.metadata.machineId) {
        throw new LocalResumeSessionError(
            `Cannot recover HappyHerd session "${session.id}" because it does not identify its originating machine.`,
            'unavailable',
        );
    }
    if (session.metadata.machineId !== machineId) {
        throw new LocalResumeSessionError(
            `Cannot recover HappyHerd session "${session.id}" because it belongs to another machine.`,
            'unavailable',
        );
    }
    ...
    if (!persistSession(session.id, persisted)) {
        throw new LocalResumeSessionError(
            `Cannot recover HappyHerd session "${session.id}" because its reconnect record could not be persisted.`,
            'unavailable',
        );
    }
