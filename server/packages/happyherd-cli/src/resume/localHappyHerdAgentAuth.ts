...
export type LocalHappyHerdAgentCredentials = {
    token: string;
    secret: Uint8Array;
    contentKeyPair: {
...
export function createLegacyRecoveryCredentials(token: string, secret: Uint8Array): LocalHappyHerdAgentCredentials {
    return {
        token,
        secret,
...
export function getLocalHappyHerdAgentCredentialPath(happyHomeDir: string = configuration.happyHomeDir): string {
    return join(happyHomeDir, 'agent.key');
}

export function readLocalHappyHerdAgentCredentials(
    happyHomeDir: string = configuration.happyHomeDir,
): LocalHappyHerdAgentCredentials | null {
    const credentialPath = getLocalHappyHerdAgentCredentialPath(happyHomeDir);
...
export function hasLocalHappyHerdAgentAuth(happyHomeDir: string = configuration.happyHomeDir): boolean {
    return readLocalHappyHerdAgentCredentials(happyHomeDir) !== null;
}

export function detectResumeSupport(happyHomeDir: string = configuration.happyHomeDir): ResumeSupport {
    const happyAgentAuthenticated = hasLocalHappyHerdAgentAuth(happyHomeDir);
    return {
