...
vi.mock('@/configuration', () => ({
    configuration: {
        sessionsFile: '/tmp/.happyherd/sessions.json',
        serverUrl: 'https://api.example.test',
        currentCliVersion: '1.1.10',
    },
}));

vi.mock('./resolveHappyHerdSession', async (importOriginal) => {
    const actual = await importOriginal<typeof import('./resolveHappyHerdSession')>();
    return {
        ...actual,
