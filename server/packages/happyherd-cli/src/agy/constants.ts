 ...
 *
 * Centralized constants for the agy integration: the binary name, HappyHerd's
 ...
 *   1. `HAPPYHERD_AGY_PATH` env override — an explicit absolute path to the binary.
...
export function findAgyBin(): string | undefined {
  const override = process.env.HAPPYHERD_AGY_PATH;
...
/**
 * Logical model names shown in HappyHerd. Gemini thinking variants are one model
...
/**
 * Resolve HappyHerd's model + effort picks to the exact display name accepted by
