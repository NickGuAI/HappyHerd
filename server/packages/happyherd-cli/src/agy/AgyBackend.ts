...
export interface AgyBackendOptions {
  ...
  model?: string;
  /** Initial HappyHerd effort key; used to resolve models with agy-specific variants. */
  effort?: string;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.on('error', (err: Error) => {
        ...
        const detail = (err as NodeJS.ErrnoException).code === 'ENOENT'
          ? `agy executable not found. Install the Antigravity CLI, or set HAPPYHERD_AGY_PATH to its absolute path (tried 'agy' on PATH and ~/.local/bin/agy).`
          : err.message;
