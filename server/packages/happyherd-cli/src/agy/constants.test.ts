...
describe('resolveAgyBin', () => {
  const orig = process.env.HAPPYHERD_AGY_PATH;
  ...
  afterEach(() => {
    if (orig === undefined) {
      delete process.env.HAPPYHERD_AGY_PATH;
    } else {
      process.env.HAPPYHERD_AGY_PATH = orig;
    }
  });

  it('uses HAPPYHERD_AGY_PATH when it points at an existing file', () => {
    // node's own binary is guaranteed to exist on every platform
    process.env.HAPPYHERD_AGY_PATH = process.execPath;
    expect(findAgyBin()).toBe(process.execPath);
    expect(resolveAgyBin()).toBe(process.execPath);
  });

  it('ignores HAPPYHERD_AGY_PATH when the target does not exist', () => {
    process.env.HAPPYHERD_AGY_PATH = '/nonexistent/path/to/agy-should-not-resolve';
...
describe('resolveAgyModelName', () => {
  it('maps Gemini 3.8 Flash and HappyHerd effort to agy display names', () => {
