...
beforeEach(async () => {
  ...
  await writeFile(path.join(process.env.HAPPYHERD_HOME_DIR, 'AGENTS.md'), '# Global\nAlways verify.\n');
  const commanderDir = path.join(process.env.HAPPYHERD_HOME_DIR, 'commanders', 'athena');
