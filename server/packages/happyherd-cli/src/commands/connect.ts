 ...
 * - connect gemini: Store Gemini API key in HappyHerd cloud
...
async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to HappyHerd cloud\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with HappyHerd'));
...
/**
 * Update local Gemini credentials file to keep in sync with HappyHerd cloud
 * This ensures the Gemini SDK uses the same account as HappyHerd
 */
