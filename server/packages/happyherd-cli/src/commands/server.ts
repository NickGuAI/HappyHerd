...
interface HappyHerdServerPackageArtifact {
    command: string;
    prefixArgs?: string[];
    cwd: string;
    bundled?: boolean;
...
function showHelp() {
    console.log(`
${chalk.bold('happyherd server')} - Run HappyHerd sync server + web app locally (self-host)
...
${chalk.bold('Notes:')}
  - Stores data in ${chalk.cyan('$HAPPYHERD_HOME_DIR/server-data/')}
  ...
  - Use ${chalk.cyan('--no-persist')} to run without modifying default HappyHerd settings
...
async function ensureSettingsWriteAllowed(opts: ServerOptions, serverUrl: string): Promise<void> {
    ...
    if (!process.stdin.isTTY || !process.stderr.isTTY) {
        console.error(chalk.red('Refusing to modify default HappyHerd settings from a non-interactive run.'));
 ...
 *
 * pkgroll bundles into dist/, so __dirname at runtime is .../happyherd-cli/dist; tools/ lives
 * at .../happyherd-cli/tools. In rare layouts (e.g. running un-built source via tsx from src/
 * commands/), tools/ sits at .../happyherd-cli/tools and __dirname is .../happyherd-cli/src/commands,
...
function resolveInstalledServerPackage(): ServerArtifacts | undefined {
    try {
        const serverPackage = require_(SERVER_PACKAGE_NAME) as {
            resolveServerArtifact?: () => HappyHerdServerPackageArtifact | undefined;
        };
