...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('happyherd auth')} - Authentication management

${chalk.bold('Usage:')}
  happyherd auth login [--force]    Authenticate with HappyHerd
  happyherd auth logout             Remove authentication and machine data
  happyherd auth status             Show authentication status
  happyherd auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'happyherd auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  console.log(chalk.blue('This will log you out of HappyHerd'));
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use HappyHerd again'));
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "happyherd auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "happyherd auth login --force" to fix this'));
  }
