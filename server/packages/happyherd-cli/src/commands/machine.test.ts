...
import { getLocalHappyHerdAgentCredentialPath } from '@/resume/localHappyHerdAgentAuth';
...
function rigMachine(id = 'rig-machine'): DecryptedMachine {
  return machine(id, {
    metadata: {
      ...
      homeDir: '/Users/rig',
      happyHomeDir: '/Users/rig/.happyherd/rig',
      machineKind: 'rig',
      rigOnly: true,
...
describe('machine and session command parsing', () => {
  ...
  it('routes account-control auth through the configured HappyHerd home', async () => {
    ...
    const expectedConfig = {
      ...
      credentialPath: getLocalHappyHerdAgentCredentialPath(configuration.happyHomeDir),
    };
  ...
  it('requires all creation inputs and rejects unknown, missing, or duplicate flags', () => {
    ...
    ])).toThrow('Unsupported HappyHerd CLI daemon provider "unknown"');
  });
});
...
describe('remote tracked session creation', () => {
  ...
  it('fails closed on Rig before applying the HappyHerd CLI daemon schema or spawning', async () => {
    const rig = rigMachine();
...
describe('account credential authority', () => {
  it('makes auth status, logout, and machine-command access agree on agent.key only', async () => {
    const homeDir = mkdtempSync(join(tmpdir(), 'happyherd-machine-auth-'));
    const credentialPath = getLocalHappyHerdAgentCredentialPath(homeDir);
    const authConfig = {
