...
import {
  getLocalHappyHerdAgentCredentialPath,
  readLocalHappyHerdAgentCredentials,
  type LocalHappyHerdAgentCredentials,
} from '@/resume/localHappyHerdAgentAuth';
import {
...
export type MachineKind = 'happyherd-cli-daemon' | 'rig' | 'unknown';
...
supported only for machines running the native HappyHerd CLI daemon; receipts mark
...
New Device in the HappyHerd app. The account-control key is stored only in the
configured HappyHerd home and remains separate from normal HappyHerd CLI auth.`;
}
...
HappyHerd CLI daemon providers: ${DAEMON_PROVIDERS.join(', ')}
...
The selected machine must run a native HappyHerd CLI daemon that advertises the
...
export function parseSessionCreateOptions(args: string[]): SessionCreateOptions {
  ...
  if (!DAEMON_PROVIDERS.includes(providerValue as Provider)) {
    throw new Error(`Unsupported HappyHerd CLI daemon provider "${providerValue}". Expected one of: ${DAEMON_PROVIDERS.join(', ')}`);
  }
...
function parseMetadata(machine: DecryptedMachine): MachineMetadata {
  ...
  if (!parsed.success) {
    throw new Error(`Machine ${machine.id} does not advertise valid HappyHerd CLI daemon capability metadata`);
  }
  return parsed.data;
}
...
function requireConfirmedMachineSessionTarget(machine: DecryptedMachine): MachineMetadata {
  ...
  if (!supportsConfirmedMachineSessions(metadata)) {
    throw new Error(
      `Machine ${machine.id} does not advertise target-confirmed machine-session protocol version ${HAPPYHERD_MACHINE_SESSION_PROTOCOL_VERSION}; upgrade and restart its HappyHerd CLI daemon`,
    );
  }
  return metadata;
}
  ...
  return {
    ...
    machines: [...machines].sort((left, right) => left.id.localeCompare(right.id)).map((machine) => {
      ...
      return {
        ...
        sessionCreateSupported: kind === 'happyherd-cli-daemon' && supportsConfirmedMachineSessions(metadata),
        happyCliVersion: identity.happyCliVersion,
        homeDir: identity.homeDir,
        availableProviders: kind === 'happyherd-cli-daemon'
          ? DAEMON_PROVIDERS.filter((provider) => metadata?.cliAvailability?.[provider] === true)
          : [],
        providers: kind === 'happyherd-cli-daemon' ? safeCatalogs(metadata) : {},
      };
    }),
  };
}
...
export async function createDefaultClient(options: {
  homeDir?: string;
  readCredentials?: (homeDir: string) => LocalHappyHerdAgentCredentials | null;
  create?: (credentials: LocalHappyHerdAgentCredentials) => MachineControlClient;
} = {}): Promise<MachineControlClient> {
  const homeDir = options.homeDir ?? configuration.happyHomeDir;
  const credentials = (options.readCredentials ?? readLocalHappyHerdAgentCredentials)(homeDir);
  if (!credentials) {
    throw new Error('Account-level machine control is not linked. Run `happyherd machine auth login` and approve the one-time link in the HappyHerd app.');
  }
  return options.create?.(credentials) ?? new HappyHerdControlClient({
...
function accountControlConfig(homeDir: string = configuration.happyHomeDir): AccountControlConfig {
  return {
    ...
    homeDir,
    credentialPath: getLocalHappyHerdAgentCredentialPath(homeDir),
  };
}
...
export async function handleMachineCommand(
  ...
  if (action === 'pair') {
    ...
    if (flags.json === true) outputFor(dependencies)(JSON.stringify(receipt));
      ...
      outputFor(dependencies)('Enter this code in Connections -> Add device using the same HappyHerd account and server.');
    }
    return;
  }
...
export async function handleSessionCommand(
  ...
  if (options.local) {
    ...
    else outputFor(dependencies)(`Created HappyHerd session ${created.sessionId} on ${created.machine.host} (${created.machine.id})`);
    return;
  }
  const client = await controlCall(() => clientFor(dependencies));
  ...
  outputFor(dependencies)(`Created HappyHerd session ${created.session.id} on ${metadata?.host ?? machine.id} (${machine.id})`);
