...
function resolveSideChatSource(parent: ParentSession): SideChatSource {
  ...
  if (!flavor) {
    if (advertisedFlavor) {
      throw new Error(`HappyHerd session ${parent.id} uses unsupported provider "${advertisedFlavor}".`);
    }
    throw new Error(`HappyHerd session ${parent.id} is missing provider metadata.`);
  }
  if (!nonEmptyString(metadata.machineId)) {
    throw new Error(`HappyHerd session ${parent.id} is missing owning machine metadata.`);
  }
  if (!nonEmptyString(metadata.path)) {
    throw new Error(`HappyHerd session ${parent.id} is missing working directory metadata.`);
  }
...
export async function createChildSideChat(
  ...
  if (machine.id !== source.machineId) {
    throw new Error(`Owning machine ${source.machineId} for HappyHerd session ${source.sessionId} was not found.`);
  }
  if (!machine.active) {
    throw new Error(`Owning machine ${source.machineId} for HappyHerd session ${source.sessionId} is offline.`);
  }
  ...
  if (!nonEmptyString(spawnResult.sessionId)) {
    throw new Error('Side-chat spawn returned an invalid HappyHerd session ID.');
  }
...
HappyHerd session and preserves its parent lineage. Inspect, pause, and resume are
