...
export interface CreateSessionMetadataOptions {
    ...
    /** HappyHerd session id this session was forked from. */
    parentSessionId?: string;
    /** HappyHerd message id used as the fork rewind point. */
...
export function providerContinuationMetadataFromEnvironment(): Pick<Metadata, 'continuedFromSessionId'> {
    const continuedFromSessionId = process.env.HAPPYHERD_CONTINUED_FROM_SESSION_ID?.trim();
...
export function sideChatMetadataFromEnvironment(): Pick<Metadata, 'parentSessionId' | 'isSideChat'> {
    const parentSessionId = process.env.HAPPYHERD_FORKED_FROM_SESSION_ID?.trim();
    const isSideChat = process.env.HAPPYHERD_SIDE_CHAT === '1';
    delete process.env.HAPPYHERD_FORKED_FROM_SESSION_ID;
    delete process.env.HAPPYHERD_SIDE_CHAT;
