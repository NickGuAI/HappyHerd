/**
 * Cross-platform HappyHerd CLI spawning utility
 * 
 * ## Background
 * 
 ...
 * with no warning output when using HappyHerd.
 * 
 * ## The Wrapper Strategy
 * 
 * We created a wrapper script `bin/happyherd.mjs` with a shebang `#!/usr/bin/env node`.
 ...
 * 2. Shell directly executes `bin/happyherd.mjs` (shebang: `#!/usr/bin/env node`)
 * 3. `bin/happyherd.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 * 
 * **Windows:**
 * 1. User runs the `happyherd` command
 * 2. NPM wrapper (`happyherd.cmd`) calls `node bin/happyherd.mjs`
 * 3. `bin/happyherd.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 * 
 * ## The Spawning Problem
 * 
 * When our code needs to spawn HappyHerd cli as a subprocess (for daemon processes),
 * we were trying to execute `bin/happyherd.mjs` directly. This fails on Windows
...
/**
 * Spawn the HappyHerd CLI with the given arguments in a cross-platform way.
 * 
 * This function bypasses the wrapper script (bin/happyherd.mjs) and spawns the
 * actual CLI entrypoint (dist/index.mjs) directly with Node.js, ensuring
 * compatibility across all platforms including Windows.
 * 
 * @param args - Arguments to pass to the HappyHerd CLI
 * @param options - Spawn options (same as child_process.spawn)
 * @returns ChildProcess instance
 */
export function spawnHappyHerdCLI(args: string[], options: SpawnOptions = {}): ChildProcess {
  ...
  // Log the public command while retaining the internal bin/happyherd.mjs entrypoint.
  const fullCommand = `happyherd ${args.join(' ')}`;
  logger.debug(`[SPAWN HAPPYHERD CLI] Spawning: ${fullCommand} in ${directory}`);
  ...
  if (!existsSync(entrypoint)) {
    ...
    logger.debug(`[SPAWN HAPPYHERD CLI] ${errorMessage}`);
