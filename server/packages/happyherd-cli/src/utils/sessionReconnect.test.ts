...
import { configureHappyHerdSessionReconnect, loadOrCreateHappyHerdSession } from './sessionReconnect';

describe('session reconnect initialization', () => {
  afterEach(() => vi.unstubAllEnvs());

  it('creates an ordinary new HappyHerd session without reconnect behavior', async () => {
    ...
    await expect(loadOrCreateHappyHerdSession({ api: api as any, sessionTag: 'tag', metadata, state }))
      .resolves.toEqual({
        response,
        reconnecting: false,
        queueMessageIds: [],
  ...
  it('refreshes the exact encrypted HappyHerd session and restores queued replay ids', async () => {
    vi.stubEnv('HAPPYHERD_RECONNECT_SESSION_ID', 'existing-session');
    vi.stubEnv('HAPPYHERD_RECONNECT_ENCRYPTION_KEY', Buffer.alloc(32, 7).toString('base64'));
    vi.stubEnv('HAPPYHERD_RECONNECT_ENCRYPTION_VARIANT', 'dataKey');
    vi.stubEnv('HAPPYHERD_RECONNECT_SEQ', '9');
    vi.stubEnv('HAPPYHERD_RECONNECT_METADATA_VERSION', '4');
    vi.stubEnv('HAPPYHERD_RECONNECT_AGENT_STATE_VERSION', '3');
    vi.stubEnv('HAPPYHERD_RECONNECT_QUEUE_MESSAGE_ID', 'resume-seed');
    ...
    const result = await loadOrCreateHappyHerdSession({
      api: api as any,
      sessionTag: 'ignored',
      metadata,
    ...
    configureHappyHerdSessionReconnect(session as any, result as any);
