...
/** Attach a fresh provider process to an existing encrypted HappyHerd session when requested by the daemon. */
export async function loadOrCreateHappyHerdSession(input: {
  api: ApiClient;
  sessionTag: string;
  metadata: Metadata;
  state: AgentState;
  ...
  return {
    ...
    priorityQueueMessageId: process.env.HAPPYHERD_FRESH_PROVIDER_RECONNECT === '1'
      ? process.env.HAPPYHERD_RECONNECT_QUEUE_MESSAGE_ID ?? null
      : null,
  };
}

export function configureHappyHerdSessionReconnect(
