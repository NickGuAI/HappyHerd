 ...
 * Ensures consistent tmux handling across happyherd-cli with proper session naming
 */
...
export class TmuxUtilities {
    ...
    public static readonly DEFAULT_SESSION_NAME = "happyherd";
    ...
    async spawnInTmux(
        ...
        try {
            ...
            // - undefined: Use first existing session or create "happyherd"
            // - empty string: Use first existing session or create "happyherd"
            ...
            if (!sessionName) {
                ...
                if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
                    ...
                    // No sessions exist, create "happyherd"
                    sessionName = 'happyherd';
                    logger.debug(`[TMUX] No existing sessions, using default: ${sessionName}`);
                }
            }

            const windowName = options.windowName || `happyherd-${Date.now()}`;
