...
describe('createSessionMetadata', () => {
    ...
    it('sets fresh provider-continuation lineage from the daemon handoff', () => {
        ...
        expect(metadata.continuedFromSessionId).toBe('happyherd-source');
    ...
    it('sets metadata.isSideChat when the session is a side chat', () => {
        const { metadata } = createSessionMetadata({
            ...
            parentSessionId: 'happyherd-parent',
            isSideChat: true,
        });

        expect(metadata.isSideChat).toBe(true);
        expect(metadata.parentSessionId).toBe('happyherd-parent');
    });
    ...
    it.each(['gemini', 'grok', 'dsh', 'agy'] as const)(
        ...
        (flavor) => {
            vi.stubEnv('HAPPYHERD_FORKED_FROM_SESSION_ID', 'happyherd-parent');
            vi.stubEnv('HAPPYHERD_SIDE_CHAT', '1');
    ...
    it('consumes side-chat lineage so a later ordinary provider launch cannot inherit it', () => {
        ...
        expect(process.env.HAPPYHERD_FORKED_FROM_SESSION_ID).toBeUndefined();
        expect(process.env.HAPPYHERD_SIDE_CHAT).toBeUndefined();
    });
