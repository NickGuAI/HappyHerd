...
import { checkIfDaemonRunningAndCleanupStaleState, isDaemonRunningCurrentlyInstalledHappyHerdVersion, stopDaemon } from './daemon/controlClient'
import { getLatestDaemonLog } from './ui/logger'
import { killRunawayHappyHerdProcesses } from './daemon/doctor'
...
(async () => {
  ...
  // Top-level version is a HappyHerd CLI query, not a Claude passthrough. Return
...
`)
      ...
      const result = await killRunawayHappyHerdProcesses()
    ...
    if (geminiSubcommand === 'project' && args[2] === 'set' && args[3]) {
      ...
      try {
        ...
        // Try to get current user email from HappyHerd cloud token
    ...
    if (daemonSubcommand === 'list') {
      ...
      const child = spawnHappyHerdCLI(['daemon', 'start-sync'], {
        detached: true,
        stdio: 'ignore',
...
`)
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-h' || arg === '--help') {
        ...
        unknownArgs.push(arg)
      } else if (arg === '--happyherd-starting-mode') {
        ...
        // HappyHerd-specific flag to disable chrome even if default is on
      } else if (arg === '--settings') {
        // Intercept --settings flag - HappyHerd uses this internally for session hooks
        const settingsValue = args[++i] // consume the value
        console.warn(chalk.yellow(`⚠️  Warning: --settings is used internally by HappyHerd for session tracking.`))
...
${chalk.bold('Examples:')}
  ...
  happyherd resume cmmij8      Resume a previous session by HappyHerd session ID
  ...
  happyherd --no-sandbox       Disable HappyHerd sandbox for this session
  ...
  happyherd machine auth login Link account-wide machine control in the HappyHerd app
...
${chalk.bold('Options:')}
  ...
  -t <title>      Notification title (optional, defaults to "HappyHerd")
...
`)
  ...
  try {
    ...
    // Use custom title or default to "HappyHerd"
    const notificationTitle = title || 'HappyHerd'
