...
describe('quota-triggered same-session rotation', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'happyherd-credential-rotation-'));
  ...
  it('stops and resumes the same HappyHerd session on the next account', async () => {
    const calls: string[] = [];
    ...
    const result = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-1', provider: 'codex', account: 'one', limitedUntil: 500,
    }, { paths, now: () => now, stopProvider, resumeProvider, onAccountSwitched });

    expect(result).toEqual({ type: 'rotated', account: 'two' });
    expect(stopProvider).toHaveBeenCalledWith('happyherd-session-1');
    expect(resumeProvider).toHaveBeenCalledWith('happyherd-session-1');
    expect(onAccountSwitched).toHaveBeenCalledWith({
      sessionId: 'happyherd-session-1',
      provider: 'codex',
      fromAccount: 'one',
      toAccount: 'two',
    });
  ...
  it('surfaces the limit, waits for the first reset, then resumes when all accounts are limited', async () => {
    ...
    await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-2', provider: 'codex', account: 'one', limitedUntil: 500,
    }, { paths, now: () => now, stopProvider, resumeProvider });
    const waitUntil = vi.fn(async (timestamp: number) => { now = timestamp; });

    const result = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-2', provider: 'codex', account: 'two', limitedUntil: 700,
    }, {
      paths,
      now: () => now,
      stopProvider,
    ...
    expect(stopProvider).toHaveBeenLastCalledWith('happyherd-session-2');
    expect(resumeProvider).toHaveBeenLastCalledWith('happyherd-session-2');
    expect(onAccountSwitched).toHaveBeenCalledWith({
      sessionId: 'happyherd-session-2',
      provider: 'codex',
      fromAccount: 'two',
      toAccount: 'one',
    });
  ...
  it('does not announce an ignored limit notice', async () => {
    ...
    const ignored = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-ignored', provider: 'codex', account: 'missing', limitedUntil: 500,
    }, {
      paths,
      now: () => now,
  ...
  it('does not announce when stopping the limited provider fails', async () => {
    ...
    await expect(rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-stop-failed', provider: 'codex', account: 'one', limitedUntil: 500,
    }, {
      paths,
      now: () => now,
  ...
  it('does not announce when the replacement provider fails to resume', async () => {
    const onAccountSwitched = vi.fn(async () => {});
    await expect(rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-failed', provider: 'codex', account: 'one', limitedUntil: 500,
    }, {
      paths,
      now: () => now,
      stopProvider: vi.fn(async () => {}),
  ...
  it('surfaces the limit but does not announce when waiting resumes the same account', async () => {
    ...
    const sameAccount = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-same', provider: 'codex', account: 'one', limitedUntil: 500,
    }, {
      paths,
      now: () => now,
      stopProvider,
  ...
  it('reports an unchanged account when an immediate replacement does not switch', async () => {
    const onAccountSwitched = vi.fn(async () => {});
    const result = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-unchanged', provider: 'codex', account: 'one', limitedUntil: 500,
    }, {
      paths,
      now: () => now,
      stopProvider: vi.fn(async () => {}),
  ...
  it('announces the account reported by the resumed provider when selection changes during restart', async () => {
    ...
    const result = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-race', provider: 'codex', account: 'one', limitedUntil: 500,
    }, {
      paths,
      now: () => now,
      stopProvider: vi.fn(async () => {}),
    ...
    expect(onAccountSwitched).toHaveBeenCalledWith({
      sessionId: 'happyherd-session-race',
      provider: 'codex',
      fromAccount: 'one',
      toAccount: 'three',
    });
  ...
  it('resolves a renamed account by stable id before recording its limit', async () => {
    ...
    const result = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-renamed',
      provider: 'codex',
      account: 'one',
      accountId: first.id,
      credentialVersion: first.credentialVersion,
  ...
  it('refreshes an old-revision session without limiting the new credential', async () => {
    ...
    const result = await rotateProviderSessionAfterLimit({
      sessionId: 'happyherd-session-old-revision',
      provider: 'codex',
      account: 'one',
      accountId: first.id,
      credentialVersion: first.credentialVersion,
