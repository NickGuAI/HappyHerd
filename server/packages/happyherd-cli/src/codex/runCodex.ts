...
export async function runCodex(opts: {
    ...
    try {
        ...
        console.error('  \x1b[36mhappyherd claude\x1b[0m\n');
        process.exit(1);
    }
    ...
    // HappyHerd change-title MCP server.
    const happyherdServer = agentSession ? null : await startHappyHerdServer(session);
    ...
    const bridgeEntrypoint = join(projectPath(), 'bin', 'happyherd-mcp.mjs');
    ...
    if (happyherdServer) {
        mcpServers.happyherd = {
            command: process.execPath,
            args: ['--no-warnings', '--no-deprecation', bridgeEntrypoint, '--url', happyherdServer.url]
        };
    }
    ...
    try {
        ...
        const forkCodexThreadId = process.env.HAPPYHERD_FORK_CODEX_THREAD_ID;
        ...
        while (!shouldExit) {
            ...
            try {
                ...
                const sandboxManagedByHappyHerd = client.sandboxEnabled;
                const executionPolicy = resolveCodexExecutionPolicy(
                    message.mode.permissionMode,
                    sandboxManagedByHappyHerd,
                );
        ...
        // Stop HappyHerd MCP server
        logger.debug('[codex]: happyherdServer.stop');
        happyherdServer?.stop();
