...
describe('buildCodexTurnPrompt', () => {
    ...
    it('sends normal follow-up turns without HappyHerd scaffolding', () => {
        const prompt = buildCodexTurnPrompt({
            message: 'continue',
...
describe('stripHappyHerdSystemBlocks', () => {
    ...
    it('recovers a multi-line user message wrapped only by the title instruction', () => {
        ...
        expect(stripHappyHerdSystemBlocks(prompt)).toBe('line one\n\nline two');
    });

    it('leaves plain text without markers untouched', () => {
        expect(stripHappyHerdSystemBlocks('just a normal message')).toBe('just a normal message');
    });
});
