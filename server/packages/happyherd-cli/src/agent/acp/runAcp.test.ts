...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    mockStartHappyHerdServer: vi.fn(),
    mockProjectPath: vi.fn(() => '/tmp/happyherd'),
...
vi.mock('@/daemon/run', () => ({
  initialMachineMetadata: { host: 'host', platform: 'darwin', happyCliVersion: 'test', homeDir: '/tmp', happyHomeDir: '/tmp/.happyherd', happyLibDir: '/tmp/happyherd' },
}));
...
vi.mock('@/claude/utils/startHappyHerdServer', () => ({
  startHappyHerdServer: mocks.mockStartHappyHerdServer,
}));
...
describe('runAcp', () => {
  ...
  beforeEach(() => {
    ...
    mocks.mockStartHappyHerdServer.mockResolvedValue({
  ...
  it.each([
    ...
    async (permissionMode, requiresUserInput, expectedDecision) => {
      vi.stubEnv('HAPPYHERD_RECONNECT_SESSION_ID', 'resumed-session');
      vi.stubEnv('HAPPYHERD_RECONNECT_ENCRYPTION_KEY', 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=');
      vi.stubEnv('HAPPYHERD_RECONNECT_ENCRYPTION_VARIANT', 'legacy');
      vi.stubEnv('HAPPYHERD_RECONNECT_SEQ', '12');
      vi.stubEnv('HAPPYHERD_RECONNECT_METADATA_VERSION', '4');
      vi.stubEnv('HAPPYHERD_RECONNECT_AGENT_STATE_VERSION', '5');
  ...
  it('replays the explicit archived turn during raw Grok ACP reconnect', async () => {
    ...
    vi.stubEnv('HAPPYHERD_RECONNECT_METADATA_VERSION', '4');
    vi.stubEnv('HAPPYHERD_RECONNECT_AGENT_STATE_VERSION', '5');
    vi.stubEnv('HAPPYHERD_RECONNECT_QUEUE_MESSAGE_ID', 'archived-next-turn');
  ...
  it('executes a seeded DSH side-chat turn without loading the retired provider session', async () => {
    vi.stubEnv('HAPPYHERD_RECONNECT_SESSION_ID', 'dsh-side-chat');
    vi.stubEnv('HAPPYHERD_RECONNECT_ENCRYPTION_KEY', 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=');
    vi.stubEnv('HAPPYHERD_RECONNECT_ENCRYPTION_VARIANT', 'legacy');
    vi.stubEnv('HAPPYHERD_RECONNECT_SEQ', '12');
    vi.stubEnv('HAPPYHERD_RECONNECT_METADATA_VERSION', '4');
    vi.stubEnv('HAPPYHERD_RECONNECT_AGENT_STATE_VERSION', '5');
    vi.stubEnv('HAPPYHERD_RECONNECT_QUEUE_MESSAGE_ID', 'dsh-resume-seed');
  ...
  it('wires backend messages through mapper into session envelopes', async () => {
    ...
    expect(consoleLines()).toEqual(expect.arrayContaining([
      'HappyHerd Session ID: session-1',
  ...
  it('updates session metadata with ACP config options (models and operating modes)', async () => {
    ...
    const baseMetadata = {
      ...
      happyLibDir: '/repo/.happyherd/lib',
      happyToolsDir: '/repo/.happyherd/tools',
    };
