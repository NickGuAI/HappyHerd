...
import { classifyDshHardLimit, classifyGrokHardLimit } from '@/credentialPool/providerLimits';
...
export async function runAcp(opts: {
  ...
  if (reconnectSessionId && reconnectKeyBase64 && reconnectVariant) {
    response = await api.refreshSessionForReconnect({
      ...
      seq: Number.parseInt(process.env.HAPPYHERD_RECONNECT_SEQ || '0', 10),
      encryptionKey: decodeBase64(reconnectKeyBase64),
      encryptionVariant: reconnectVariant,
      metadata,
      metadataVersion: Number.parseInt(process.env.HAPPYHERD_RECONNECT_METADATA_VERSION || '0', 10),
      agentState: state,
      agentStateVersion: Number.parseInt(process.env.HAPPYHERD_RECONNECT_AGENT_STATE_VERSION || '0', 10),
    });
  ...
  if (response) {
    logAcp('muted', `HappyHerd Session ID: ${response.id}`);
  }
  ...
  // switch from HappyHerd message metadata.
  ...
  const mcpServers = {
    happyherd: {
      command: join(projectPath(), 'bin', 'happyherd-mcp.mjs'),
      args: ['--url', happyherdServer.url],
    },
  };
  ...
  try {
    ...
    while (!shouldExit) {
      ...
      try {
        ...
        if (opts.agentName === 'grok' || opts.agentName === 'dsh') {
          const hardLimit = opts.agentName === 'grok'
            ? classifyGrokHardLimit(error)
            : classifyDshHardLimit(error);
          if (hardLimit) {
    ...
    try {
      ...
      logger.debug(`[${opts.agentName}] Failed to stop HappyHerd MCP server:`, error);
    }
