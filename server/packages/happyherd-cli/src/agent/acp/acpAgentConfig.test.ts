...
describe('resolveAcpLaunchConfig', () => {
  it.each([
    ...
    'bypassPermissions',
    'plan',
  ])('forwards GrokBuild launch mode %s without leaking HappyHerd lifecycle flags', (permissionMode) => {
    ...
    expect(resolved.args).not.toContain('--happyherd-starting-mode');
...
describe('sanitizeGrokChildEnvironment', () => {
  it('keeps process essentials and Grok auth without leaking arbitrary application credentials', () => {
    expect(sanitizeGrokChildEnvironment({
      ...
      HAPPYHERD_RECONNECT_ENCRYPTION_KEY: 'happyherd-secret',
