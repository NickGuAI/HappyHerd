/**
 * Resolves the CLAUDE_CODE_ENTRYPOINT value HappyHerd should use when launching
 ...
 * NOT itself an SDK entrypoint. The result is that every HappyHerd-driven session
 * disappears from a plain `claude --resume` picker — which is exactly the
 * symptom reported in slopus/happyherd#1202 ("No conversations found to resume").
 *
 * Mitigation
 * ----------
 * Pre-set the env to a Claude-recognised entrypoint that is NOT in the SDK
 * filter set. "remote_mobile" matches HappyHerd's role as a remote-mobile control
 * surface for Claude Code, is part of Claude Code's own allowlist, and keeps
 * HappyHerd-touched sessions visible in `claude --resume`.
 *
 * If the caller already exported CLAUDE_CODE_ENTRYPOINT to something specific,
 * we respect that — the user/operator wins.
 */

export const HAPPYHERD_DEFAULT_ENTRYPOINT = 'remote_mobile' as const

export function resolveHappyHerdEntrypoint(currentValue: string | undefined): string {
    if (currentValue && currentValue.length > 0) {
        return currentValue
    }
    return HAPPYHERD_DEFAULT_ENTRYPOINT
}
