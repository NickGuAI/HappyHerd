...
describe('getProjectPath', () => {
    ...
    describe('Claude Code path normalization parity', () => {
        ...
        // HappyHerd must match this exactly, otherwise session files won't be found.
        // See: https://github.com/slopus/happy/issues/563

        it('should replace @ symbols with hyphens (Google Drive paths)', () => {
            process.env.CLAUDE_CONFIG_DIR = '/test/home/.claude';
            const workingDir = '/Users/adam/Library/CloudStorage/GoogleDrive-user@example.com/projects';
            const result = getProjectPath(workingDir);
            expect(result).toBe(join('/test/home/.claude', 'projects', '-Users-adam-Library-CloudStorage-GoogleDrive-user-example-com-projects'));
        });
