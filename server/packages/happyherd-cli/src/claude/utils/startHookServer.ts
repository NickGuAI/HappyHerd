 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► OS temp/happyherd-hooks-<random>/session-hook-<pid>.json
 ...
 * File watching has race conditions when multiple HappyHerd processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between HappyHerd process and Claude session.
 */
