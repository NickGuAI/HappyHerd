...
import { findAllHappyHerdProcesses } from '@/daemon/doctor'
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        ...
        HAPPYHERD_PROJECT_ROOT: process.env.HAPPYHERD_PROJECT_ROOT,
        DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING: process.env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING,
        NODE_ENV: process.env.NODE_ENV,
        DEBUG: process.env.DEBUG,
        workingDirectory: process.cwd(),
        processArgv: process.argv,
        happyherdDir: configuration?.happyHomeDir,
...
export async function runDoctorCommand(): Promise<void> {
    ...
    const wrapperPath = join(projectRoot, 'bin', 'happyherd.mjs');
    ...
    console.log(`HAPPYHERD_HOME_DIR: ${env.HAPPYHERD_HOME_DIR ? chalk.green(env.HAPPYHERD_HOME_DIR) : chalk.gray('not set')}`);
    console.log(`HAPPYHERD_SERVER_URL: ${env.HAPPYHERD_SERVER_URL ? chalk.green(env.HAPPYHERD_SERVER_URL) : chalk.gray('not set')}`);
    ...
    console.log(`Documentation: ${chalk.blue('https://github.com/NickGuAI/HappyHerd')}`);

    // ── Concise useful info last (visible without scrolling) ──

    // Basic info
    console.log(chalk.bold('\n📋 Basic Information'));
    console.log(`HappyHerd CLI Version: ${chalk.green(packageJson.version)}`);
    console.log(`Platform: ${chalk.green(process.platform)} ${process.arch}`);
    console.log(`Node.js Version: ${chalk.green(process.version)}`);

    // Configuration
    console.log(chalk.bold('\n⚙️  Configuration'));
    console.log(`HappyHerd Home: ${chalk.blue(configuration.happyHomeDir)}`);
