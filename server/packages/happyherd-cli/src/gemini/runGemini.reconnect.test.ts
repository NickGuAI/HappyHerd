...
vi.mock('@/claude/utils/startHappyHerdServer', () => ({
  startHappyHerdServer: mocks.startHappyHerdServer,
}));

vi.mock('@/projectPath', () => ({
  projectPath: vi.fn(() => '/tmp/happyherd'),
}));
...
vi.mock('@/ui/logger', () => ({
  logger: { debug: vi.fn(), getLogPath: vi.fn(() => '/tmp/happyherd.log') },
}));
...
describe('runGemini fresh-provider reconnect', () => {
  ...
  beforeEach(() => {
    ...
    mocks.startHappyHerdServer.mockResolvedValue({
      url: 'http://127.0.0.1:9876',
      stop: vi.fn(),
    });

    vi.stubEnv('HAPPYHERD_RECONNECT_SESSION_ID', 'gemini-side-chat');
    ...
    vi.stubEnv('HAPPYHERD_RECONNECT_QUEUE_MESSAGE_ID', 'gemini-resume-handoff');
