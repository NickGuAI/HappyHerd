 ...
 * through HappyHerd CLI. It manages the agent lifecycle, session state, and
 * communication with the HappyHerd server and mobile app.
 */
...
import { configureHappyHerdSessionReconnect, loadOrCreateHappyHerdSession } from '@/utils/sessionReconnect';
...
export async function runGemini(opts: {
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from HappyHerd cloud');
  ...
  const reconnect = await loadOrCreateHappyHerdSession({ api, sessionTag, metadata, state });
  const response = reconnect.response;

  // Handle server unreachable case - create offline stub with hot reconnection
  let session: ApiSessionClient;
  // Permission handler declared here so it can be updated in onSessionSwap callback
  // (assigned later after HappyHerd server setup)
  ...
  configureHappyHerdSessionReconnect(session, reconnect);
  ...
  //
  // Start HappyHerd MCP server and create Gemini backend
  //

  const happyherdServer = await startHappyHerdServer(session);
  const bridgeCommand = join(projectPath(), 'bin', 'happyherd-mcp.mjs');
  const mcpServers = {
    happyherd: {
    ...
    switch (msg.type) {
      ...
      case 'tool-result':
        ...
        if (msg.toolName === 'change_title' || 
            ...
            msg.toolName === 'happyherd__change_title') {
    ...
    while (!shouldExit) {
      ...
      try {
        ...
        pendingChangeTitle = message.message.includes('change_title') || 
                             message.message.includes('happyherd__change_title');
