...
describe('sessionEnvironment', () => {
    ...
    it('keeps explicit fork values after removing stale ambient values', () => {
        const childEnv = buildSessionChildEnvironment(contaminatedEnvironment(), {
            HAPPYHERD_FORKED_FROM_SESSION_ID: 'new-parent-session',
            HAPPYHERD_FORKED_FROM_MESSAGE_ID: 'new-parent-message',
            HAPPYHERD_FORK_CODEX_THREAD_ID: 'new-codex-thread',
            HAPPYHERD_SIDE_CHAT: '1',
        ...
        expect(childEnv).not.toHaveProperty('HAPPYHERD_RECONNECT_SESSION_ID');
    });
    ...
    it('replaces stale reconnect state with the values for the resumed session', () => {
        const childEnv = buildSessionChildEnvironment(contaminatedEnvironment(), {
            HAPPYHERD_RECONNECT_SESSION_ID: 'new-session',
            HAPPYHERD_RECONNECT_ENCRYPTION_KEY: 'new-key',
            HAPPYHERD_RECONNECT_ENCRYPTION_VARIANT: 'dataKey',
            HAPPYHERD_RECONNECT_SEQ: '12',
            HAPPYHERD_RECONNECT_METADATA_VERSION: '13',
            HAPPYHERD_RECONNECT_AGENT_STATE_VERSION: '14',
            HAPPYHERD_RECONNECT_QUEUE_MESSAGE_ID: 'heartbeat-occurrence',
        ...
        expect(childEnv).not.toHaveProperty('HAPPYHERD_FORK_CODEX_THREAD_ID');
        expect(childEnv).not.toHaveProperty('CODEX_THREAD_ID');
    });
    ...
    it('unsets inherited tmux values without removing an explicit fork value', () => {
        const explicitEnv = { HAPPYHERD_FORK_CODEX_THREAD_ID: 'new-codex-thread' };
        const keysToUnset = sessionEnvironmentKeysToUnset(explicitEnv);
        const command = wrapTmuxCommandWithSessionEnvironmentSanitizer('node happyherd.mjs codex', explicitEnv);

        expect(keysToUnset).not.toContain('HAPPYHERD_FORK_CODEX_THREAD_ID');
        expect(keysToUnset).toContain('CODEX_THREAD_ID');
        expect(command).toMatch(/^unset /);
        expect(command).toContain('CODEX_THREAD_ID');
        expect(command).not.toContain('unset HAPPYHERD_FORK_CODEX_THREAD_ID');
        expect(command).toMatch(/node happyherd\.mjs codex$/);
    });
});
