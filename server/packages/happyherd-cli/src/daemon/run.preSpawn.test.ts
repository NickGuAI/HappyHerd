...
type Handlers = {
  ...
  onHappyHerdSessionWebhook: (sessionId: string, metadata: Metadata) => void;
...
vi.mock('@/utils/spawnHappyHerdCLI', () => ({ spawnHappyHerdCLI: mocks.spawnHappyHerdCLI }));
...
vi.mock('@/resume/localHappyHerdAgentAuth', () => ({ detectResumeSupport: vi.fn(() => ({})) }));
vi.mock('@/agentContext/commanderContext', () => ({
  agentContextRoot: () => process.env.HAPPYHERD_HOME_DIR!,
...
vi.mock('@/daemon/happyherdTerminalBoot', () => ({ startHappyHerdTerminalDaemon: vi.fn() }));
...
beforeEach(async () => {
  ...
  vi.stubEnv('HAPPYHERD_HOME_DIR', join(root, '.happyherd'));
  vi.stubEnv('HAPPYHERD_MISSING_PRESPAWN_TEST_VAR', undefined);
  await mkdir(process.env.HAPPYHERD_HOME_DIR!, { recursive: true });
  await writeFile(join(process.env.HAPPYHERD_HOME_DIR!, 'AGENTS.md'), '# Test');
  ...
  mocks.spawnHappyHerdCLI.mockReturnValue({ pid: 4321, on: vi.fn(), kill: vi.fn() });
...
async function expectScheduledPreparationFailureThenRecovery({
  ...
  mocks.handlers!.onHappyHerdSessionWebhook(recoveredSessionId, {
    path: root, host: 'test-host', hostPid: 4321,
    homeDir: root, happyHomeDir: process.env.HAPPYHERD_HOME_DIR!, happyLibDir: root, happyToolsDir: root,
  });
  await vi.waitFor(async () => {
    const { runs } = await service.history(automation.id);
...
describe('daemon pre-spawn rejection evidence', () => {
  ...
  it('does not infer retry safety from a catch-all exception after process creation', async () => {
    mocks.spawnHappyHerdCLI.mockReturnValue({ pid: 4321, on: () => { throw new Error('listener setup failed'); } });
  ...
  it('records terminal failures on consecutive scheduled ticks and starts after quota recovers', async () => {
    ...
    tick();
    await vi.waitFor(() => expect(mocks.spawnHappyHerdCLI).toHaveBeenCalledOnce());
    mocks.handlers!.onHappyHerdSessionWebhook('recovered-session', {
      path: root, host: 'test-host', hostPid: 4321,
      homeDir: root, happyHomeDir: process.env.HAPPYHERD_HOME_DIR!, happyLibDir: root, happyToolsDir: root,
    });
    await vi.waitFor(async () => {
