# HappyHerd CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages HappyHerd sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.
...
2. Spawns detached process via `spawnHappyHerdCLI(['daemon', 'start-sync'], { detached: true })`
...
4. `startDaemon()` performs startup:
   ...
   - Heartbeat loop: every 60s (or HAPPYHERD_DAEMON_HEARTBEAT_INTERVAL) checks for version updates and prunes dead sessions
...
3. If mismatch detected:
   - Spawns new daemon via `spawnHappyHerdCLI(['daemon', 'start'])`
...
3. `spawnSession()`:
   ...
   - Spawns detached HappyHerd process with `--happyherd-starting-mode remote --started-by daemon`
   - Adds to `pidToTrackedSession` map
   - Sets up 10-second awaiter for session webhook
4. New HappyHerd process:
...
2. HappyHerd process calls `notifyDaemonSessionStarted()`
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'happyherd directly...'`
...
`happyherd doctor` uses `ps aux | grep` to find all HappyHerd processes:
- Production: matches `happyherd.mjs`, `happyherd`, legacy `happyherd` or `happy-coder`, and `dist/index.mjs`
...
1. `findRunawayHappyHerdProcesses()` filters for likely orphans
2. `killRunawayHappyHerdProcesses()`:
   - Sends SIGTERM
   - Waits 1 second
...
```json
{
  ...
  "metadata": "base64(encrypted({
    ...
    'homeDir': '/Users/john',
    'happyHomeDir': '/Users/john/.happyherd'
  }))",
