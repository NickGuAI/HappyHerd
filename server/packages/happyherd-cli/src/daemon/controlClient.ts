...
async function daemonPost(path: string, body?: any, timeoutOverride?: number): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = timeoutOverride
      ?? (process.env.HAPPYHERD_DAEMON_HTTP_TIMEOUT ? parseInt(process.env.HAPPYHERD_DAEMON_HTTP_TIMEOUT) : 10_000);
 ...
 * For instance we are not handling the case where we upgraded happyherd,
 ...
 */
export async function isDaemonRunningCurrentlyInstalledHappyHerdVersion(): Promise<boolean> {
