...
const { mockConfiguration, mockSpawn } = vi.hoisted(() => ({
  mockConfiguration: { bootHappyHerdAgent: false } as { bootHappyHerdAgent: boolean },
  mockSpawn: vi.fn(),
}));
...
import {
  buildHappyHerdTerminalBootCommand,
  HAPPYHERD_TERMINAL_BOOT_TIMEOUT_MS,
  startHappyHerdTerminalDaemon,
} from './happyherdTerminalBoot';

describe('HappyHerd Terminal daemon boot', () => {
  it('starts nothing for somebody who never asked for HappyHerd Agent', () => {
    const child = startHappyHerdTerminalDaemon();
  ...
  it('builds a node command for the exported daemon entrypoint', () => {
    expect(buildHappyHerdTerminalBootCommand('/installed/happy-terminal/dist/main.js', '/usr/bin/node'))
      .toEqual({
  ...
  it('uses a bounded best-effort startup timeout', () => {
    expect(HAPPYHERD_TERMINAL_BOOT_TIMEOUT_MS).toBe(120_000);
  });
  ...
  it('hides the daemon child window on Windows', () => {
    ...
    mockConfiguration.bootHappyHerdAgent = true;
    mockSpawn.mockReturnValue(child);

    expect(startHappyHerdTerminalDaemon()).toBe(child);
