 ...
 *
 * This file boots one authenticated HappyHerd environment and restarts the daemon
...
describe('Daemon Integration Tests', { timeout: 180_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnHappyHerdCLI(['daemon', 'start'], {
      stdio: 'ignore',
    });
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      happyHomeDir: '/test/happyherd-home',
      happyLibDir: '/test/happyherd-lib',
      happyToolsDir: '/test/happyherd-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.startedBy).toBe('happyherd directly - likely by user from terminal');
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    // Spawn a real happyherd process that looks like it was started from terminal
    const terminalHappyHerdProcess = spawnHappyHerdCLI([
    ...
    if (!terminalHappyHerdProcess || !terminalHappyHerdProcess.pid) {
      throw new Error('Failed to spawn terminal happyherd process');
    }
    ...
    const terminalSession = sessions.find(
      (s: any) => s.pid === terminalHappyHerdProcess.pid
    );
    ...
    expect(terminalSession.startedBy).toBe('happyherd directly - likely by user from terminal');
    ...
    try {
      terminalHappyHerdProcess.kill('SIGTERM');
    } catch (e) {
   ...
   * 6. Daemon spawns new daemon via spawnHappyHerdCLI(['daemon', 'start'])
   ...
   * 
   * This simulates what happens during `npm upgrade @happyherd/cli`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/happyherd/ with NEW version files
  ...
  it.skip('[skipped] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.HAPPYHERD_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when @happyherd/cli is uninstalled
