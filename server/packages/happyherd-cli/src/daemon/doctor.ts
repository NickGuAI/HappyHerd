...
/**
 * Find all HappyHerd CLI processes (including current process)
 */
export async function findAllHappyHerdProcesses(): Promise<Array<{ pid: number, command: string, type: string }>> {
  try {
    ...
    for (const proc of processes) {
      ...
      // Check if it's a HappyHerd process
      const isHappyHerd = name.includes('happyherd') ||
                      name === 'node' && (cmd.includes('happyherd-cli') || cmd.includes('dist/index.mjs')) ||
                      cmd.includes('happyherd.mjs') ||
                      cmd.includes('happy-coder') || // legacy npm package name
                      cmd.includes('/happyherd/') ||
                      (cmd.includes('tsx') && cmd.includes('src/index.ts') && cmd.includes('happyherd-cli'));
      
      if (!isHappyHerd) continue;
...
/**
 * Find all runaway HappyHerd CLI processes that should be killed
 */
export async function findRunawayHappyHerdProcesses(): Promise<Array<{ pid: number, command: string }>> {
  const allProcesses = await findAllHappyHerdProcesses();
...
/**
 * Kill all runaway HappyHerd CLI processes
 */
export async function killRunawayHappyHerdProcesses(): Promise<{ killed: number, errors: Array<{ pid: number, error: string }> }> {
  const runawayProcesses = await findRunawayHappyHerdProcesses();
