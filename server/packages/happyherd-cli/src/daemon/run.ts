...
import { cleanupDaemonState, isDaemonRunningCurrentlyInstalledHappyHerdVersion, listDaemonSessions, stopDaemon } from './controlClient';
...
import { detectResumeSupport } from '@/resume/localHappyHerdAgentAuth';
...
import { startHappyHerdTerminalDaemon } from './happyherdTerminalBoot';
...
function trackedSessionWithAutomationProvenance(
  ...
  metadata: Metadata | undefined = session.happyherdSessionMetadataFromLocalWebhook,
...
// Suffix host with `-dev` for the HAPPYHERD_VARIANT=dev variant so the dev daemon
// is visually distinct from the stable one in the machine list (they otherwise
// share the same hostname and look identical).
const hostSuffix = process.env.HAPPYHERD_VARIANT === 'dev' ? '-dev' : '';
...
export async function startDaemon(): Promise<void> {
  ...
  const runningDaemonVersionMatches = await isDaemonRunningCurrentlyInstalledHappyHerdVersion();
  ...
  try {
    // HappyHerd Agent is a machine-level service shared by the mobile app and
    // HappyHerd Terminal. Start it concurrently and keep this daemon boot path
    // independent from its install/download/network state.
    startHappyHerdTerminalDaemon();
    ...
    const persistedSession = (sessionId: string): AutomationTrackedSession | undefined => {
      ...
      return trackedSessionWithAutomationProvenance({
        ...
        happyherdSessionMetadataFromLocalWebhook: saved.metadata,
        encryption: {
    ...
    for (const session of handoffSessions) {
      ...
      pidToTrackedSession.set(session.pid, trackedSessionWithAutomationProvenance({
        ...
        pid: session.pid,
      }, reconnect?.happyherdSessionMetadataFromLocalWebhook));
    }
    ...
    const onChildExited = async (pid: number): Promise<void> => {
      ...
      const exitedBeforeWebhook = pidToPreWebhookExitAwaiter.has(pid);
      try {
    ...
    // A reconnecting provider can report its existing HappyHerd session before
    ...
    const onHappyHerdSessionWebhook = (sessionId: string, sessionMetadata: Metadata, encryption?: SessionEncryptionData) => {
      ...
      // Registration cannot replace another live owner, or repurpose a child
      // reserved for a different reconnect target.
      const reserved = pidToTrackedSession.get(pid);
      if (reserved?.happySessionId && reserved.happySessionId !== sessionId) return;
      if ([...pidToTrackedSession.values()].some((candidate) =>
        candidate.pid !== pid && candidate.happySessionId === sessionId
        && !hasProviderProcessExited(candidate.pid))) return;
      if (stoppingPids.has(pid)) return;
      ...
      if (!hadPendingSpawnAwaiter) registeringPids.delete(pid);
      ...
      if (existingSession) {
        ...
        existingSession.happyherdSessionMetadataFromLocalWebhook = sessionMetadata;
        ...
        const trackedSession = trackedSessionWithAutomationProvenance({
          ...
          happySessionId: sessionId,
          happyherdSessionMetadataFromLocalWebhook: sessionMetadata,
          encryption,
          pid
        }, sessionMetadata);
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        if (options.parentSessionId) {
          extraEnv.HAPPYHERD_FORKED_FROM_SESSION_ID = options.parentSessionId;
        }
        if (options.forkedFromMessageId) {
          extraEnv.HAPPYHERD_FORKED_FROM_MESSAGE_ID = options.forkedFromMessageId;
        }
        if (options.continuedFromSessionId) {
          extraEnv.HAPPYHERD_CONTINUED_FROM_SESSION_ID = options.continuedFromSessionId;
        }
        if (options.isSideChat) {
          extraEnv.HAPPYHERD_SIDE_CHAT = '1';
        }
        if (options.isSuperSession) {
          extraEnv.HAPPYHERD_SUPER_SESSION = '1';
        }
        // For fork: spawned HappyHerd CLI needs to know which Claude JSONL to
        // backfill into the fresh HappyHerd session row. Without this, the
        ...
        if (options.resumeClaudeSessionId) {
          extraEnv.HAPPYHERD_FORK_CLAUDE_SESSION_ID = options.resumeClaudeSessionId;
        }
        if (options.resumeCodexThreadId) {
          extraEnv.HAPPYHERD_FORK_CODEX_THREAD_ID = options.resumeCodexThreadId;
        }
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          const launchArgs = [
            agent,
            '--happyherd-starting-mode', 'remote',
            '--started-by', 'daemon',
          ];
          ...
          const windowName = `happyherd-${Date.now()}-${agent}`;
            ...
            return new Promise((resolve) => {
              ...
              pidToAwaiter.set(tmuxResult.pid!, (completedSession) => {
                ...
                if (!persistedMachineSessionSettingsMatch(
                  completedSession.happyherdSessionMetadataFromLocalWebhook,
        ...
        if (!useTmux) {
          ...
          // Resume ids attach the new HappyHerd session to a pre-existing provider
          ...
          providerLaunchAttempted = true;
          return spawnTrackedHappyHerdProcess({
            args,
            cwd: directory,
    ...
    const spawnTrackedHappyHerdProcess = ({
      args,
      cwd,
      env,
      ...
      if (!happyherdProcess.pid) {
        logger.debug('[DAEMON RUN] Failed to spawn process - no PID returned');
        return Promise.resolve({
          type: 'error',
          errorMessage: 'Failed to spawn HappyHerd process - no PID returned',
          retrySafe: true,
        });
      }

      logger.debug(`[DAEMON RUN] Spawned process with PID ${happyherdProcess.pid}`);

      const trackedSession: AutomationTrackedSession = {
        startedBy: 'daemon',
        automationId: automation?.id,
        automationRunId: automation?.runId,
        pid: happyherdProcess.pid,
        childProcess: happyherdProcess,
        directoryCreated,
        message,
        ...reconnect,
      };

      pidToTrackedSession.set(happyherdProcess.pid, trackedSession);
      if (reconnect) registeringPids.add(happyherdProcess.pid);

      happyherdProcess.on('exit', (code, signal) => {
        logger.debug(`[DAEMON RUN] Child PID ${happyherdProcess.pid} exited with code ${code}, signal ${signal}`);
        if (happyherdProcess.pid) {
          void onChildExited(happyherdProcess.pid);
        }
      });

      happyherdProcess.on('error', (error) => {
        logger.debug(`[DAEMON RUN] Child process error:`, error);
        if (happyherdProcess.pid && hasProviderProcessExited(happyherdProcess.pid)) {
          void onChildExited(happyherdProcess.pid);
        } else if (happyherdProcess.pid) {
          logger.debug(`[DAEMON RUN] Child PID ${happyherdProcess.pid} still exists after process error; keeping session active`);
        }
      });

      logger.debug(`[DAEMON RUN] Waiting for session webhook for PID ${happyherdProcess.pid}`);
      ...
      return new Promise((resolve) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          pidToAwaiter.delete(happyherdProcess.pid!);
          pidToPreWebhookExitAwaiter.delete(happyherdProcess.pid!);
          logger.debug(`[DAEMON RUN] Session webhook timeout for PID ${happyherdProcess.pid}`);
          resolve({
            type: 'error',
            errorMessage: `Session webhook timeout for PID ${happyherdProcess.pid}`
          });
        }, 15_000);

        if (automation || reconnect) {
          pidToPreWebhookExitAwaiter.set(happyherdProcess.pid!, () => {
            if (settled) return;
            settled = true;
            clearTimeout(timeout);
            pidToAwaiter.delete(happyherdProcess.pid!);
            resolve({
              type: 'error',
              errorMessage: `Provider PID ${happyherdProcess.pid} exited before registering a HappyHerd session`,
            });
          });
        }

        pidToAwaiter.set(happyherdProcess.pid!, (completedSession) => {
    ...
    async function finalizeExitedAutomationSession(session: AutomationTrackedSession): Promise<void> {
      ...
      if (!session.happySessionId) {
        await automations.confirmRunDidNotStart({
          ...
          message: `Provider PID ${session.pid} exited before registering a HappyHerd session.`,
        });
        return;
      }
      ...
      const localMetadata = session.happyherdSessionMetadataFromLocalWebhook;
      ...
      session.happyherdSessionMetadataFromLocalWebhook = freshMetadata;
      await automations.confirmRunTermination({
        automationId: run.automationId,
        runId: run.id,
        sessionId: run.sessionId!,
      ...
      try {
        ...
        for (const run of activeRuns) {
          ...
          if (!session) {
            ...
            const metadata = reconnect?.happyherdSessionMetadataFromLocalWebhook;
    ...
    type DaemonResumeSessionOptions = {
      ...
      /** Side-chat-only fresh provider restart attached to the same encrypted HappyHerd session. */
    ...
    const resumeSession = (
      ...
      const pending = resumesInFlight.get(happySessionId);
    ...
    const resumeConflict = (sessionId: string, saved: PersistedSession | undefined): SpawnSessionResult | undefined => {
      for (const [pid, session] of pidToTrackedSession) {
        if (session.happySessionId !== sessionId) continue;
        // Exit/deactivation remains owned by the existing lifecycle, including
        // its async reconciliation. Until it completes retain the reservation.
        if (hasProviderProcessExited(pid)) {
          void onChildExited(pid);
          return { type: 'error', errorMessage: `Session ${sessionId} is finishing its previous process. Try again.` };
        }
        if (stoppingPids.has(pid) || registeringPids.has(pid)) {
          return { type: 'error', errorMessage: `Session ${sessionId} is still starting or stopping. Try again after its process settles.` };
        }
        return { type: 'error', errorMessage: `Session ${sessionId} is already running with PID ${pid}.` };
      }
      const finished = sessionIdToFinishedSession.get(sessionId);
      const confirmedFinished = finished && saved?.metadata?.hostPid === finished.pid;
      if (!confirmedFinished && hasPersistedProcessConflict(saved, bootTimeMs)) {
        return { type: 'error', errorMessage: `Session ${sessionId} may still have a running process from a previous daemon. Wait for it to exit or register before retrying.` };
      }
      return undefined;
    };

    const resumeSessionAttempt = async (
      happySessionId: string,
      options?: DaemonResumeSessionOptions,
    ): Promise<SpawnSessionResult> => {
      try {
        // Capture the local PID provenance before refreshing metadata. A server
        // snapshot cannot make a pre-boot PID current by changing savedAt.
        const savedOwner = readPersistedSessions()[happySessionId];
        const conflict = resumeConflict(happySessionId, savedOwner);
        ...
        if (!tracked) {
          ...
          if (!tracked) {
            throw new Error(`Recovered HappyHerd session ${resolvedSessionId} could not be indexed.`);
          }
        }
        if (!tracked.happyherdSessionMetadataFromLocalWebhook) {
        ...
        const authoritative = (await api.inspectSessionAuthoritative({
          ...
          metadata: tracked.happyherdSessionMetadataFromLocalWebhook,
        ...
        const refreshedPersisted: PersistedSession = {
          ...
          // Only a local registration dates process ownership. Recovery alone
          // must not turn an old server hostPid into a current-boot owner.
          savedAt: savedOwner?.savedAt ?? 0,
        };
        if (!persistSession(resolvedSessionId, refreshedPersisted)) {
          throw new Error(`Cannot recover HappyHerd session "${resolvedSessionId}" because its reconnect record could not be persisted.`);
        }
        persisted[resolvedSessionId] = refreshedPersisted;
        tracked.happyherdSessionMetadataFromLocalWebhook = authoritative.metadata;
        ...
        const launch = freshProvider
          ? {
            args: [freshProvider, ...(freshSideChatProvider || freshProvider === 'claude'
              ? ['--happyherd-starting-mode', 'remote'] : []), '--started-by', 'daemon'],
            cwd: metadata.path,
          }
          : buildResumeLaunch(
        ...
        const lateConflict = resumeConflict(resolvedSessionId, savedOwner);
        ...
        const resumed = await spawnTrackedHappyHerdProcess({
          ...
          env: buildSessionChildEnvironment(ambientEnvironment, {
            ...
            HAPPYHERD_RECONNECT_SESSION_ID: resolvedSessionId,
            HAPPYHERD_RECONNECT_ENCRYPTION_KEY: encodeBase64(tracked.encryption.encryptionKey),
            HAPPYHERD_RECONNECT_ENCRYPTION_VARIANT: tracked.encryption.encryptionVariant,
            HAPPYHERD_RECONNECT_SEQ: String(tracked.encryption.seq),
            HAPPYHERD_RECONNECT_METADATA_VERSION: String(tracked.encryption.metadataVersion),
            HAPPYHERD_RECONNECT_AGENT_STATE_VERSION: String(tracked.encryption.agentStateVersion),
            ...(options?.replayQueueMessageId
              ? { HAPPYHERD_RECONNECT_QUEUE_MESSAGE_ID: options.replayQueueMessageId }
              : {}),
      ...
      for (const [pid, session] of pidToTrackedSession.entries()) {
        if (session.happySessionId === sessionId ||
          ...
          if (session.startedBy === 'daemon' && session.childProcess) {
            // Signal the whole process group, not just the HappyHerd CLI parent.
    ...
    const changeGrokPermissionMode = async (
      ...
      if (!live?.happyherdSessionMetadataFromLocalWebhook || !live.encryption) {
        throw new Error(`Grok session ${request.sessionId} is not running on this daemon`);
      }

      const metadata = live.happyherdSessionMetadataFromLocalWebhook;
    ...
    const loadHeartbeatTarget = async (happySessionId: string) => {
      ...
      if (!tracked?.happyherdSessionMetadataFromLocalWebhook || !tracked.encryption) {
    ...
    const onProviderLimited = (notice: ProviderLimitNotice): boolean => {
      if (notice.accountId) {
        const tracked = findTrackedSessionById(notice.sessionId);
        const metadata = tracked?.happyherdSessionMetadataFromLocalWebhook;
        if (
        ...
        try {
          result = await rotateProviderSessionAfterLimit(managedNotice, {
            ...
            resumeProvider: async (sessionId) => {
              ...
              const providerAccount = resumed?.happyherdSessionMetadataFromLocalWebhook?.providerAccount;
              if (!providerAccount) {
            ...
            onAccountSwitched: async ({ sessionId, provider, fromAccount, toAccount }) => {
              const resumed = findTrackedSessionById(sessionId);
              if (!resumed?.happyherdSessionMetadataFromLocalWebhook || !resumed.encryption) {
              ...
              await api.postSessionEvent({
                ...
                metadata: resumed.happyherdSessionMetadataFromLocalWebhook,
    ...
    const activeCredentialAccounts = new CredentialAccountManager({
      isLegacyAccountInUse: (target) => [...pidToTrackedSession.values()].some((session) => {
        ...
        const metadata = session.happyherdSessionMetadataFromLocalWebhook;
      ...
      archiveMetadata: async (sessionId) => {
        ...
        try {
          ...
          await client.updateMetadata((metadata) => ({
            ...
            archivedBy: 'cli',
            archiveReason: 'Side chat closed from HappyHerd CLI',
          }), 10_000);
          return { success: true };
        } catch (error) {
          return { success: false, message: error instanceof Error ? error.message : String(error) };
    ...
    // 4. Write heartbeat
    const heartbeatIntervalMs = parseInt(process.env.HAPPYHERD_DAEMON_HEARTBEAT_INTERVAL || '60000');
    ...
    const restartOnStaleVersionAndHeartbeat = setInterval(async () => {
      ...
      if (bundleReplaced) {
        ...
        // isDaemonRunningCurrentlyInstalledHappyHerdVersion() === true, and exits —
        ...
        try {
          spawnHappyHerdCLI(['daemon', 'start'], {
            detached: true,
            stdio: 'ignore',
