...
const testMetadata = {
    ...
    happyToolsDir: '/home/user/.happyherd/tools'
};
...
describe('Api server error handling', () => {
    ...
    describe('getOrCreateSession', () => {
        it('should return null when HappyHerd server is unreachable (ECONNREFUSED)', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('⚠️  HappyHerd server unreachable')
            );

            consoleSpy.mockRestore();
        });

        it('should return null when HappyHerd server cannot be found (ENOTFOUND)', async () => {
        ...
        it('should return null when HappyHerd server times out (ETIMEDOUT)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    describe('refreshSessionForReconnect', () => {
        ...
        it('fails closed when the reconnect target is absent', async () => {
            ...
            await expect(api.refreshSessionForReconnect({
                ...
                agentState: {},
                agentStateVersion: 0,
            })).rejects.toThrow('Cannot refresh HappyHerd session missing-session for reconnect');
        });
    });
