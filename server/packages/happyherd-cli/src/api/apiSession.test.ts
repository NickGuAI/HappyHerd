...
describe('ApiSessionClient v3 messages API migration', () => {
    ...
    let testHappyHerdHomeDir: string;
    ...
    beforeEach(() => {
        ...
        testHappyHerdHomeDir = mkdtempSync(join(tmpdir(), 'happyherd-api-session-'));
        (configuration as { happyHomeDir: string }).happyHomeDir = testHappyHerdHomeDir;
        socketHandlers = {};
        session = makeSession();
        mockSocket = {
            connected: true,
    ...
    it('restores a crash-pending local usage event and cursor before replaying it', async () => {
        ...
        const outbox = new DurableUsageOutbox(testHappyHerdHomeDir, session.id);
        await outbox.write({
            report,
