...
if (!parentPort) {
    throw new Error('happyherd worker must be started via worker_threads')
}
...
type HappyHerdEncryptionContext = {
    anonId: string
...
const send = (m: HappyHerdWorkerMessage) => port.postMessage(m)

let credentials: StoredCredentials | null = null
let client: HappyHerdAuthenticatedClient | null = null
let linkFlow: LinkFlow | null = null
let state: HappyHerdStateSnapshot = {
    status: 'starting',
...
class HappyHerdAuthenticatedClient {
    private constructor(
        private readonly cfg: WorkerConfig,
        private readonly auth: StoredCredentials,
        private readonly encryption: HappyHerdEncryptionContext,
    ) {}

    static async create(
        cfg: WorkerConfig,
        auth: StoredCredentials,
    ): Promise<HappyHerdAuthenticatedClient> {
        const secret = decodeBase64(auth.secret, 'base64url')
        const encryption = await createEncryptionContext(secret)
        return new HappyHerdAuthenticatedClient(cfg, auth, encryption)
    }
...
port.on('message', (msg: HappyHerdWorkerRequestWithId) => {
    Promise.resolve()
        .then(() => handle(msg))
        .catch((err) => {
...
async function boot(): Promise<void> {
    try {
        ...
        client = credentials ? await HappyHerdAuthenticatedClient.create(config, credentials) : null
        setState(snapshotForCredentials(credentials))
    } catch (err) {
        setError(`Failed to load HappyHerd credentials: ${errString(err)}`)
    }
}

async function handle(msg: HappyHerdWorkerRequestWithId): Promise<void> {
...
async function startLinkDevice(requestId: string): Promise<void> {
    ...
    try {
        ...
        const flow: LinkFlow = {
            keypair,
            publicKey,
            authUrl: `happyherd:///account?${publicKey}`,
            startedAt: Date.now(),
            cancelled: false,
        }
        ...
        try {
            ...
            if (data.state === 'authorized') {
                if (typeof data.token !== 'string' || typeof data.response !== 'string') {
                    throw new Error('HappyHerd auth response is missing token data')
                }
                const encrypted = decodeBase64(data.response)
                const secret = decryptBox(encrypted, flow.keypair.secretKey)
                if (!secret || secret.length !== 32) {
                    throw new Error('Failed to decrypt HappyHerd account credentials')
                }
                linkFlow = null
            ...
            if (!flow.cancelled && linkFlow === flow) {
                linkFlow = null
                setError(`HappyHerd link failed: ${errString(err)}`)
            }
            return
        }
        await delay(1000)
...
async function storeAuthenticated(token: string, secret: Uint8Array): Promise<void> {
    ...
    client = await HappyHerdAuthenticatedClient.create(config, credentials)
    setState(snapshotForCredentials(credentials))
}

async function createEncryptionContext(masterSecret: Uint8Array): Promise<HappyHerdEncryptionContext> {
...
async function authGetToken(secret: Uint8Array): Promise<string> {
    ...
    if (typeof data.token !== 'string') {
        throw new Error('HappyHerd authentication did not return a token')
    }
    return data.token
}
...
async function apiJson(path: string, body: unknown): Promise<Record<string, unknown>> {
    ...
    if (!response.ok) {
        ...
        throw new Error(`HappyHerd API ${response.status}: ${text || response.statusText}`)
    }
...
function snapshotForCredentials(auth: StoredCredentials | null): HappyHerdStateSnapshot {
    if (!auth) {
        return {
            ...stateBase(),
            status: 'unconfigured',
...
function setState(next: HappyHerdStateSnapshot): void {
    state = next
...
function respond(requestId: string, next: HappyHerdStateSnapshot, value?: unknown): void {
