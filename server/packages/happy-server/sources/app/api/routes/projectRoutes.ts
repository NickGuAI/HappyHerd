...
export function projectRoutes(app: Fastify) {
    ...
    app.delete('/v1/projects/:projectId/avatar', {
        ...
        await emitProjectUpdate(userId, project);
        // Keep opaque candidates until the project is deleted. Another client
        // can have uploaded the next avatar before this removal committed; a
        // prefix sweep here could delete that candidate immediately before its
        // activation stores the new authoritative ref.
        return reply.send(serializeProject(project));
    });
