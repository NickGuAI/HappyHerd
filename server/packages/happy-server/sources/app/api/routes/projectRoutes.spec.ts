...
describe('projectRoutes', () => {
    ...
    it('keeps uploaded candidates when removing the active avatar', async () => {
        app = await createApp();
        const created = await app.inject({ method: 'POST', url: '/v1/projects', headers: { 'x-user-id': 'u1' }, payload: { externalId: 'a', metadata: 'one' } });
        const projectId = created.json().id;

        const activeUpload = await app.inject({ method: 'POST', url: `/v1/projects/${projectId}/avatar/request-upload`, headers: { 'x-user-id': 'u1' }, payload: { size: 3 } });
        const activeRef = activeUpload.json().ref as string;
        const activeFile = activeRef.slice(activeRef.lastIndexOf('/') + 1);
        await app.inject({ method: 'PUT', url: `/v1/projects/${projectId}/avatar/${activeFile}`, headers: { 'x-user-id': 'u1', 'content-type': 'application/octet-stream' }, payload: Buffer.from('old') });
        await app.inject({ method: 'PATCH', url: `/v1/projects/${projectId}/avatar`, headers: { 'x-user-id': 'u1' }, payload: { ref: activeRef, preview: 'old-preview' } });

        // A second client can upload its replacement before it observes the
        // first client's avatar removal.
        const candidateUpload = await app.inject({ method: 'POST', url: `/v1/projects/${projectId}/avatar/request-upload`, headers: { 'x-user-id': 'u1' }, payload: { size: 3 } });
        const candidateRef = candidateUpload.json().ref as string;
        const candidateFile = candidateRef.slice(candidateRef.lastIndexOf('/') + 1);
        await app.inject({ method: 'PUT', url: `/v1/projects/${projectId}/avatar/${candidateFile}`, headers: { 'x-user-id': 'u1', 'content-type': 'application/octet-stream' }, payload: Buffer.from('new') });

        const removed = await app.inject({ method: 'DELETE', url: `/v1/projects/${projectId}/avatar`, headers: { 'x-user-id': 'u1' } });
        expect(removed.statusCode).toBe(200);
        expect(removed.json().avatar).toBeNull();
        expect(filesMock.deleteProjectAvatars).not.toHaveBeenCalled();
        expect(state.uploads.get(candidateRef)).toEqual(Buffer.from('new'));

        const activated = await app.inject({ method: 'PATCH', url: `/v1/projects/${projectId}/avatar`, headers: { 'x-user-id': 'u1' }, payload: { ref: candidateRef, preview: 'new-preview' } });
        expect(activated.statusCode).toBe(200);
        expect(activated.json().avatar.ref).toBe(candidateRef);
    });
