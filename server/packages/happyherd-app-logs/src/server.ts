...
// Resolve happyherd home dir — same pattern as happyherd-cli/src/configuration.ts
let happyherdHome: string;
const configuredHome = /* rename:preserve */ process.env.HAPPYHERD_HOME_DIR ?? process.env.HAPPY_HOME_DIR; /* /rename:preserve */
if (configuredHome) {
    happyherdHome = configuredHome.replace(/^~/, homedir());
} else {
    happyherdHome = join(homedir(), '.happy');
}

const logsDir = join(happyherdHome, 'app-logs');
