...
const root = path.resolve('/prepared/happyherd-server-self-host');
...
for (const override of [undefined, '/operator/explicit-engine.node']) {
  test(`standalone command passes ${override ? 'explicit' : 'artifact-declared'} engine to server`, () => {
    ...
    vm.runInNewContext(readFileSync(path.join(__dirname, 'bin/happyherd-server.cjs'), 'utf8'), {
      console,
      process: { argv: ['node', 'happyherd-server', 'serve'],
