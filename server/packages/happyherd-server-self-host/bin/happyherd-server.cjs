...
if (!artifact) {
  console.error('Could not locate the HappyHerd server package runtime.');
  process.exit(1);
}
...
if (artifact.webappDir && !env.HAPPYHERD_STATIC_DIR) {
  env.HAPPYHERD_STATIC_DIR = artifact.webappDir;
}
