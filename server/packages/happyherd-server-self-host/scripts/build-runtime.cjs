 ...
 * around packages/happyherd-server, which stays private and is what production
 * deploys. We reach into the sibling at build time only (same pattern as
 * happyherd-cli/scripts/bundle-webapp.cjs reaching into happyherd-app).
 *
 * Produces:
 *   dist/standalone.mjs  — bun bundle of ../happyherd-server/sources/standalone.ts
...
const serverPackage = path.resolve(root, '..', 'happyherd-server');
