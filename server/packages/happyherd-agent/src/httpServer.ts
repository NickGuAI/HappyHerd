...
import type { SkillCallRequest } from './types';
...
export class BridgeHttpServer {
  ...
  constructor(options: {
    ...
    const allowedFamilies = new Set(broker.families());
    ...
    this.server = createServer(async (request, response) => {
      try {
        ...
        if (request.method === 'POST' && url.pathname === '/mcp') {
          ...
          if (typeof body.family !== 'string' || !allowedFamilies.has(body.family)) {
