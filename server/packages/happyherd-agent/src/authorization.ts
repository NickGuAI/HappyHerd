...
function nonEmptyString(value: unknown, label: string, maxLength = 8_192): string {
  if (typeof value !== 'string' || value.length === 0 || value.length > maxLength) {
    throw new Error(`Invalid service authorization response: ${label}`);
  }
  return value;
}
...
function parseExpiry(value: unknown): number {
  ...
  if (!Number.isFinite(parsed)) {
    throw new Error('Invalid service authorization response: delegation.expiresAt');
  }
  return parsed;
}
...
function parseGrant(
  ...
  if (!actor || typeof actor !== 'object' || Array.isArray(actor)) {
    throw new Error('Invalid service authorization response: actor');
  }
  if (!delegation || typeof delegation !== 'object' || Array.isArray(delegation)) {
    throw new Error('Invalid service authorization response: delegation');
  }
  ...
  if (discordUserId !== message.authorDiscordId) {
    throw new Error('Invalid service authorization response: actor/source mismatch');
  }
  if (raw.mode !== requestedMode) {
    throw new Error('Invalid service authorization response: mode mismatch');
  }
  if (!Array.isArray(raw.scopes) || raw.scopes.some((scope) => typeof scope !== 'string')) {
    throw new Error('Invalid service authorization response: scopes');
  }
  const expiresAt = parseExpiry(delegationRecord.expiresAt);
  if (expiresAt <= now || expiresAt > now + 15 * 60_000) {
    throw new Error('Invalid service authorization response: delegation expiry');
  }
  const resources = raw.resources;
  if (!resources || typeof resources !== 'object' || Array.isArray(resources)) {
    throw new Error('Invalid service authorization response: resources');
  }

  return {
    decision: 'allow',
    actor: {
      subjectId: nonEmptyString(actorRecord.subjectId, 'actor.subjectId', 128),
      discordUserId,
    },
    mode: requestedMode,
...
export class ServiceAuthorizationClient implements ActorAuthorizer {
  private readonly options: Required<Pick<ServiceAuthorizationClientOptions, 'fetchImpl' | 'now'>>
    & Omit<ServiceAuthorizationClientOptions, 'fetchImpl' | 'now'>;

  constructor(options: ServiceAuthorizationClientOptions) {
    this.options = {
      ...options,
    ...
    try {
      response = await this.options.fetchImpl(url, {
        ...
        headers: {
          ...
          'x-happyherd-agent-id': this.options.agentId,
          'x-happyherd-agent-timestamp': timestamp,
          'x-happyherd-agent-nonce': nonce,
          'x-happyherd-agent-content-sha256': bodyHash,
          'x-happyherd-agent-signature': signature,
        },
        body,
        signal: AbortSignal.timeout(10_000),
      });
    } catch (error) {
      throw new Error(`Service authorization unavailable: ${error instanceof Error ? error.message : String(error)}`);
    }

    if (!response.ok) {
      throw new Error(`Service authorization failed closed with HTTP ${response.status}`);
    }
