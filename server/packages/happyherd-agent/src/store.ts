...
function parseState(raw: string): BridgeState {
  ...
  if (!isRecord(parsed) || parsed.schemaVersion !== 1) {
    throw new Error('Unsupported or corrupt HappyHerd Agent bridge state');
  }
  if (!isRecord(parsed.surfaces) || !isRecord(parsed.inbound)) {
    throw new Error('Corrupt HappyHerd Agent bridge state collections');
  }
...
export class BridgeStore {
  ...
  static async open(stateDir: string): Promise<BridgeStore> {
    ...
    if (!directoryStats.isDirectory() || (directoryStats.mode & 0o077) !== 0) {
      throw new Error('HappyHerd Agent state directory must be a mode-0700 directory');
    }
    ...
    try {
      ...
      if ((fileStats.mode & 0o077) !== 0) {
        throw new Error('HappyHerd Agent state file must be mode 0600');
      }
    } catch (error) {
    ...
    return this.mutate(() => {
      ...
      if (current) {
        if (
          ...
          throw new Error('A Discord DM surface cannot change its linked subject');
        }
