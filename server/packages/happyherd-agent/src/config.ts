...
export type BridgeConfig = {
  ...
  discordTokenNotBefore: number | null;
...
function parsePort(value: string | undefined): number {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new Error('HAPPYHERD_AGENT_PORT must be an integer from 1 to 65535');
  }
  return port;
}
...
function parseAuthorizationPath(raw: string): string {
  if (!raw.startsWith('/') || raw.startsWith('//')) {
    throw new Error('HAPPYHERD_AGENT_AUTHORIZATION_PATH must be an origin-relative path');
  }
  const parsed = new URL(raw, 'https://service.invalid');
  if (parsed.origin !== 'https://service.invalid' || parsed.search || parsed.hash) {
    throw new Error('HAPPYHERD_AGENT_AUTHORIZATION_PATH must not change origin or include query/fragment data');
  }
...
function assertDedicatedPaths(config: Pick<BridgeConfig, 'happyHomeDir' | 'agentWorkspace' | 'stateDir'>): void {
  ...
  for (const [label, candidate] of [
    ...
    if (personalRoots.some((root) => candidate === root || candidate.startsWith(`${root}/`))) {
      throw new Error(`${label} must use a dedicated service path, not ${candidate}`);
    }
  }
...
export function loadBridgeConfig(env: NodeJS.ProcessEnv = process.env): BridgeConfig {
  ...
  const listenHost = env.HAPPYHERD_AGENT_HOST?.trim() || '127.0.0.1';
  const listenPort = parsePort(env.HAPPYHERD_AGENT_PORT);
  const serviceApiBaseUrl = parseUrl(required(env, 'HAPPYHERD_AGENT_SERVICE_API_URL'), 'HAPPYHERD_AGENT_SERVICE_API_URL', production);
  const brokerUrl = parseUrl(
    env.HAPPYHERD_AGENT_BROKER_URL ?? `http://127.0.0.1:${listenPort}/mcp`,
    'HAPPYHERD_AGENT_BROKER_URL',
    production,
  );
  const tokenNotBeforeRaw = env.HAPPYHERD_AGENT_DISCORD_TOKEN_NOT_BEFORE?.trim();
  const discordTokenNotBefore = tokenNotBeforeRaw ? Date.parse(tokenNotBeforeRaw) : null;
  if (tokenNotBeforeRaw && !Number.isFinite(discordTokenNotBefore)) {
    throw new Error('HAPPYHERD_AGENT_DISCORD_TOKEN_NOT_BEFORE must be an ISO-8601 timestamp');
  }

  const config: BridgeConfig = {
    discordApplicationId: required(env, 'HAPPYHERD_AGENT_DISCORD_APPLICATION_ID'),
    discordBotTokenFile: absolutePath(env, 'HAPPYHERD_AGENT_DISCORD_TOKEN_FILE'),
    discordTokenRotationReceiptFile: env.HAPPYHERD_AGENT_DISCORD_TOKEN_ROTATION_RECEIPT_FILE
      ? absolutePath(env, 'HAPPYHERD_AGENT_DISCORD_TOKEN_ROTATION_RECEIPT_FILE')
      : null,
    discordTokenNotBefore,
    toolManifestFile: absolutePath(env, 'HAPPYHERD_AGENT_TOOL_MANIFEST_FILE'),
    serviceApiBaseUrl,
    authorizationPath: parseAuthorizationPath(
      env.HAPPYHERD_AGENT_AUTHORIZATION_PATH?.trim() || '/api/internal/discord/authorize',
    ),
    ...
    allowedGuildIds: parseCsv(env.HAPPYHERD_AGENT_ALLOWED_GUILD_IDS),
    allowedChannelIds: parseCsv(env.HAPPYHERD_AGENT_ALLOWED_CHANNEL_IDS),
    listenHost,
    listenPort,
    brokerUrl,
  ...
  if (
    ...
    || !['127.0.0.1', 'localhost', '[::1]', 'happyherd-agent-broker.localhost'].includes(broker.hostname)
    || brokerPort !== config.listenPort
    || broker.pathname !== '/mcp'
  ) {
    throw new Error('HAPPYHERD_AGENT_BROKER_URL must be this bridge’s loopback /mcp endpoint');
  }
  const secretFiles = [
    config.discordBotTokenFile,
    config.serviceSigningSecretFile,
    config.transportSecretFile,
  ...
  if (new Set(secretFiles).size !== secretFiles.length) {
    throw new Error('Discord, rotation receipt, signing, and bridge transport material must use separate files');
  }
  if (production && (config.allowedGuildIds.size === 0 || config.allowedChannelIds.size === 0)) {
    throw new Error('Production requires explicit Discord guild and channel allowlists');
  }
  if (production && broker.hostname !== 'happyherd-agent-broker.localhost') {
    throw new Error('Production broker must use the sandbox-proxied loopback alias');
  }
  if (production && !config.discordTokenRotationReceiptFile) {
    throw new Error('Production requires HAPPYHERD_AGENT_DISCORD_TOKEN_ROTATION_RECEIPT_FILE');
  }
  if (production && config.discordTokenNotBefore === null) {
    throw new Error('Production requires HAPPYHERD_AGENT_DISCORD_TOKEN_NOT_BEFORE');
  }
  return config;
}
...
export async function verifyDiscordTokenRotationReceipt(options: {
  ...
  if (
    ...
    || (options.notBefore !== null && options.notBefore !== undefined && rotatedAt < options.notBefore)
    || rotatedAt > now + 5 * 60_000
  ) {
    throw new Error('Discord token rotation receipt is outside the configured validity window');
  }
