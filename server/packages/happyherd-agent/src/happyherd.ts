...
export interface HappyHerdSessionRuntime {
...
export class HappyHerdRuntime implements HappyHerdSessionRuntime {
  ...
  constructor(
    ...
    control: ControlClient = HappyHerdControlClient.fromEnvironment(),
  ) {
    this.config = config;
  ...
  async isMachineReady(): Promise<boolean> {
    try {
      const machine = await this.control.resolveMachine(this.config.happyherdMachineId);
  ...
  async ensureSession(binding: SurfaceBinding): Promise<{ sessionId: string; sequence: number }> {
    ...
    if (!session) {
      session = await this.control.spawnCodexSession({
        machineId: this.config.happyherdMachineId,
