...
async function stateDirectory(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'example-bridge-store-'));
...
describe('BridgeStore', () => {
  ...
  it('keeps a DM bound to one organization actor and one capability', async () => {
    ...
    await expect(store.bindSurface(binding({ subjectId: 'member-2' })))
      .rejects.toThrow('cannot change its linked subject');
  });
