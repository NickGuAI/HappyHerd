...
export function buildPmaiBrokerDispatcher(
  env: NodeJS.ProcessEnv = process.env,
): ProxyAgent | undefined {
  if (env.PMAI_BROKER_PROXY_REQUIRED !== '1') return undefined;
  if (env.SANDBOX_RUNTIME !== '1') {
    throw new Error('PMAI broker proxying is allowed only inside the HappyHerd sandbox');
  }
