...
export interface SessionPlace {
    ...
    name: string;
    path: string;
    /** The project this place belongs to, when a HappyHerd Agent session has named one. */
    projectId?: string;
}
...
/**
 * Everywhere the picker may start a HappyHerd Agent session, newest knowledge winning.
 *
 * HappyHerd Agent runs beside HappyHerd CLI rather than replacing it, so the directories a person already
 * works in belong to the computer, not to whichever daemon happened to open them. Both machines of
 * a pair are read, and a place named by a HappyHerd Agent session wins over the same path derived from
 ...
 *
 * Read from the sessions HappyHerd Agent published rather than from git, so a workspace is named the
 ...
 *
 * HappyHerd gives each daemon its own machine, and HappyHerd Agent names the other half of its pair. A
 * person picking HappyHerd Agent should still see the directories their HappyHerd CLI sessions established.
 */
export function pairedMachineIds(
    ...
    // The pointer is written by HappyHerd Agent only, so the pairing is also read backwards.
