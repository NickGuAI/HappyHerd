...
function newScreenBoundary() {
    ...
    const scope = {
        ...
        getEffortLevelsForModel: () => [], validateNewSessionLaunchSelection: () => null,
        supportsImageAttachmentsForFlavor: () => true, workspaceEntries: [], setWorkspaceEntries: vi.fn(),
...
describe('chat composer callback boundary', () => {
    function chatBoundary() {
        const composer = { getMessage: vi.fn(() => 'original'), clearMessage: vi.fn(), clearSentMessage: vi.fn() };
