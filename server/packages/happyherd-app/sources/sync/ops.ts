...
import type { HappyHerdAgentSpawnTarget } from './happyherdAgentSpawn';
import {
...
export interface SpawnSessionOptions {
    ...
    modelId?: string;
    effort?: string;
    /** Durable project/workspace destination for HappyHerd Agent's native RPC. */
    happyherdAgentTarget?: HappyHerdAgentSpawnTarget;
    /**
     * If set, the daemon spawns the agent with `--resume <id>` so the new
     * HappyHerd session attaches to a pre-existing on-disk Claude conversation
     * file. Used by the session fork / duplicate flow.
     */
    resumeClaudeSessionId?: string;
    /**
     * If set, the daemon spawns Codex with `--resume <id>` so the new HappyHerd
     * session attaches to an app-server thread created by fork / duplicate.
     */
    resumeCodexThreadId?: string;
    /** HappyHerd session id this fork was branched from (lineage). */
    parentSessionId?: string;
    /** HappyHerd message id used as the rewind point (only set for "duplicate"). */
    forkedFromMessageId?: string;
    /** Source HappyHerd session for a fresh cross-provider continuation. */
...
export async function machineSpawnNewSession(options: SpawnSessionOptions): Promise<SpawnSessionResult> {
    ...
    const { machineId, directory, approvedNewDirectoryCreation = false, token, agent, permissionMode, modelMode, effortLevel, commanderId, clientRequestId, providerId, modelId, effort, happyherdAgentTarget, resumeClaudeSessionId, resumeCodexThreadId, parentSessionId, forkedFromMessageId, continuedFromSessionId } = options;

    try {
        if (agent === 'rig' && !clientRequestId) {
            throw new Error('Rig session creation requires a client request ID');
        }
        if (happyherdAgentTarget && agent !== 'rig') {
            throw new Error('HappyHerd Agent catalog targets require the HappyHerd Agent harness');
        }
 ...
 * the new Claude session UUID. Caller then spawns a fresh HappyHerd session
 * with `resumeClaudeSessionId` set to that UUID to attach a new HappyHerd
...
export async function machineCreateDirectory(
    machineId: string,
    request: import('@happyherd/wire').WorkspaceCreateDirectoryRequest,
): Promise<import('@happyherd/wire').WorkspaceCreateDirectoryResponse> {
    try {
        return await apiSocket.machineRPC<
            import('@happyherd/wire').WorkspaceCreateDirectoryResponse,
            import('@happyherd/wire').WorkspaceCreateDirectoryRequest
 ...
 *   2. Spawn a new HappyHerd session on the same machine with
 ...
 * RPC into env vars, then into the new HappyHerd session's metadata at start
