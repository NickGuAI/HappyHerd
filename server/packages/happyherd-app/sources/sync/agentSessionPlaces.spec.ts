...
describe('where a HappyHerd Agent session may be started', () => {
    it('never offers a bot folder for a second conversation', () => {
        ...
        const botSession = session({
            machineId: RIG,
            path: '/home/user/HappyHerd/Bots/assistant',
            bot,
        ...
        expect(collectSessionPlaces({ machineIds: [RIG], sessions: [session({
            machineId: RIG, path: '/home/user/HappyHerd/Bots/assistant', bot,
        })] })).toEqual([]);
    ...
    it('offers the directories legacy sessions established, on either machine of the pair', () => {
        const places = collectSessionPlaces({
            machineIds: [RIG, CLI],
            sessions: [
                session({ machineId: CLI, path: '/home/example-user/projects/happyherd' }),
                session({ machineId: RIG, path: '/home/example-user/projects/rig' }),
            ],
        });
        expect(places.map((p) => p.path).sort()).toEqual([
            '/home/example-user/projects/happyherd',
    ...
    it('prefers the project name HappyHerd Agent knows over the bare path', () => {
        const places = collectSessionPlaces({
            machineIds: [RIG, CLI],
            sessions: [
...
describe('pairing the two machines of one computer', () => {
    it('reads the pointer HappyHerd Agent writes', () => {
