 ...
 *
 * HappyHerd gives every daemon a machine of its own, so a computer running both HappyHerd CLI and HappyHerd
...
export interface MachineChoice {
    /**
     * What a draft stores, which is HappyHerd CLI's machine whenever this computer has one.
     *
     * HappyHerd Agent's display name carries a suffix that only means anything when both halves are on
    ...
    machineIds: string[];
    /** The daemon HappyHerd CLI runs here, if it runs one. */
    happyherdMachine: Machine | null;
    /** The daemon HappyHerd Agent runs here, if it runs one. */
...
/** The host name without the daemon suffix HappyHerd Agent adds to tell the pair apart. */
function getComputerName(choice: { happyherdMachine: Machine | null; rigMachine: Machine | null }): string {
    const named = choice.happyherdMachine ?? choice.rigMachine;
    if (!named) return 'Unknown machine';
    // HappyHerd Agent has no sibling to be distinguished from once the pair is offered as one row,
    // so it is named by its host rather than by the display name it publishes.
    if (choice.happyherdMachine === null && named.metadata?.host) return named.metadata.host;
 ...
 * computer are only ever connected through the machine they have in common: two HappyHerd Agent
 * registrations naming the same HappyHerd CLI are the same laptop, but neither names the other. Asking
 * one hop from whichever machine was seen first splits that laptop in two and puts the HappyHerd CLI
 ...
 *
 * A pairing is only ever the pointer HappyHerd Agent publishes, read in both directions. Two daemons
...
export function collectMachineChoices(machines: readonly Machine[]): MachineChoice[] {
    ...
    for (const machine of machines) {
        ...
        // One computer can accumulate several HappyHerd Agent registrations — a daemon started from a
        // different data directory mints a new machine identity and claims the same HappyHerd CLI
        ...
        const happyherdMachine = preferLiveliest(group.filter((member) => !isRigMachine(member.metadata)));
        choices.push({
            id: (happyherdMachine ?? rigMachine ?? machine).id,
            name: getComputerName({ happyherdMachine, rigMachine }),
            machineIds: ids,
            happyherdMachine,
            rigMachine,
 ...
 *
 * A computer with no HappyHerd CLI daemon cannot run Claude Code however much the picker would like
 * to offer it, and HappyHerd Agent's own machine publishes no CLI availability at all — so treating a
...
export function machineChoiceAgentAvailable(
    ...
    if (agent === 'rig') {
        return isHarnessAvailable({
            availability: choice.happyherdMachine?.metadata?.cliAvailability,
            happyherdAgentAvailable: choice.rigMachine !== null,
            key: agent,
        });
    }
    const happyherd = choice.happyherdMachine;
    if (!happyherd) return false;
    return isHarnessAvailable({
        availability: happyherd.metadata?.cliAvailability,
        happyherdAgentAvailable: choice.rigMachine !== null,
        key: agent,
    });
}
 ...
 *
 * Common harnesses stay visible but disabled when unavailable. GrokBuild,
 * Antigravity, and HappyHerd Agent stay absent until this computer reports them available.
 */
export function machineChoiceAgentVisible(
    choice: MachineChoice | null,
    agent: NewSessionAgentType,
): boolean {
    return (agent !== 'grok' && agent !== 'dsh' && agent !== 'agy' && agent !== 'rig') || machineChoiceAgentAvailable(choice, agent);
}
...
export function resolveChoiceAgent(
    ...
    if (machineChoiceAgentAvailable(choice, agent)) {
        // Older HappyHerd CLI machines are trusted for common harnesses. Antigravity
        // never reaches this branch without an explicit installation report.
        return agent === 'rig' || !choice.happyherdMachine?.metadata?.cliAvailability
            ? agent
            : resolveMachineAgent(agent, choice.happyherdMachine.metadata.cliAvailability);
    }
 ...
 *
 * HappyHerd Agent remains available for existing sessions and sync while its
 * creation surface is experimental. A saved HappyHerd Agent draft therefore
...
export function resolveAgentMachine(
    ...
    return agent === 'rig' ? choice.rigMachine : choice.happyherdMachine;
}

/**
 * The daemon that can create a git worktree for a new session.
 *
 * HappyHerd Agent may publish `worktrees: false` while the HappyHerd CLI daemon paired
 * with it still exposes the machine-level git RPC. In that case the CLI daemon
 * creates the checkout and HappyHerd Agent starts the session inside the resulting
...
export function resolveWorktreeCreationMachine(
    ...
    if (agent === 'rig') {
        const happyherdMachine = choice.happyherdMachine;
        if (happyherdMachine && isMachineOnline(happyherdMachine)) {
            return happyherdMachine;
        }
    }
