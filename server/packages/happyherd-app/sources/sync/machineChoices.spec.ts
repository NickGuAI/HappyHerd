...
function rig(id = RIG, sibling = CLI, options?: { active?: boolean; activeAt?: number }) {
    return machine(id, {
        host: 'laptop.local',
        displayName: 'laptop.local — HappyHerd Agent',
        machineKind: 'rig',
        rigOnly: true,
...
describe('offering one computer rather than one daemon', () => {
    it('collapses a HappyHerd CLI and HappyHerd Agent pair into a single choice', () => {
        const choices = collectMachineChoices([cli(), rig()]);
        expect(choices).toHaveLength(1);
        expect(choices[0].machineIds.sort()).toEqual([CLI, RIG].sort());
    });

    it('names the computer by its host, dropping the HappyHerd Agent suffix', () => {
        expect(collectMachineChoices([cli(), rig()])[0].name).toBe('laptop.local');
    });

    it('stores the HappyHerd CLI machine id, which is what older drafts already hold', () => {
    ...
    it('offers a HappyHerd Agent with no HappyHerd CLI beside it, under its own host name', () => {
...
describe('choosing between several HappyHerd Agent registrations on one computer', () => {
    ...
    // Machines arrive newest first, and a HappyHerd Agent registration is younger than the HappyHerd CLI
    ...
    it('is one computer even when the agents are seen before what they point at', () => {
        ...
        expect(choices[0].happyherdMachine?.id).toBe(CLI);
    });
...
describe('what a computer can actually run', () => {
    it('offers HappyHerd Agent only where a HappyHerd Agent daemon is registered', () => {
    ...
    it('refuses a CLI agent on a computer that runs only HappyHerd Agent', () => {
    ...
    it('only shows detected-only harnesses and HappyHerd Agent when available on the machine', () => {
    ...
    it('offers HappyHerd for new sessions only when experiments are enabled', () => {
        const paired = collectMachineChoices([cli(), rig()])[0];
    ...
    it('never resolves a new session to HappyHerd from a stale draft when experiments are off', () => {
        const rigOnly = collectMachineChoices([rig(RIG, 'missing-sibling')])[0];
...
describe('choosing where to create a worktree', () => {
    it('uses HappyHerd CLI for a HappyHerd Agent workspace when the pair is online', () => {
        const choice = collectMachineChoices([cli(), rig()])[0];

        expect(resolveWorktreeCreationMachine(choice, 'rig', false)?.id).toBe(CLI);
    });

    it('uses HappyHerd Agent directly when it supports worktrees and has no CLI pair', () => {
        const choice = collectMachineChoices([rig(RIG, 'missing-sibling')])[0];
    ...
    it('does not offer an offline CLI as HappyHerd Agent worktree support', () => {
...
describe('a computer that is asleep', () => {
    // Its projects have not moved, so it stays pickable and its HappyHerd Agent stays selectable.
    ...
    it('still offers HappyHerd Agent, which the send path refuses later with a reason', () => {
