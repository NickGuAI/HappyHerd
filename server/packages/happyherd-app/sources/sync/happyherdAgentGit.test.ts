...
import {
    getHappyHerdAgentGitState,
    readHappyHerdAgentGitFile,
    supportsHappyHerdAgentGit,
    type HappyHerdAgentGitFile,
    type HappyHerdAgentGitState,
} from './happyherdAgentGit';
...
function file(overrides: Partial<HappyHerdAgentGitFile> = {}): HappyHerdAgentGitFile {
    return {
        path: 'src/file.ts',
        status: 'modified',
        staged: false,
...
function gitState(files: HappyHerdAgentGitFile[] = []): HappyHerdAgentGitState {
    return {
        facts: {
...
describe('HappyHerd Agent Git capability gating', () => {
    it('requires native V1, readable files, and every Git RPC method', () => {
        expect(supportsHappyHerdAgentGit(nativeMetadata)).toBe(true);
        expect(supportsHappyHerdAgentGit({
            ...nativeMetadata,
    ...
    it('does not call RPC for unsupported, V0, or legacy sessions', async () => {
        for (const metadata of [
            ...
            await expect(getHappyHerdAgentGitState(SESSION_ID)).rejects.toThrow(/not available/);
        }
        expect(mocks.sessionRPC).not.toHaveBeenCalled();
    });
});

describe('HappyHerd Agent Git state', () => {
    it('uses an exact session-scoped empty gitState request and preserves committed files', async () => {
        ...
        await expect(getHappyHerdAgentGitState(SESSION_ID)).resolves.toEqual(gitState([committed, staged]));
    ...
    it('throws the native failure message and rejects malformed or unknown responses', async () => {
        ...
        await expect(getHappyHerdAgentGitState(SESSION_ID)).rejects.toThrow('Git is unavailable for this workspace.');

        mocks.sessionRPC.mockResolvedValueOnce({ success: true, git: { comparison: 'unknown' } });
        await expect(getHappyHerdAgentGitState(SESSION_ID)).rejects.toThrow('invalid Git state');
    });
});

describe('HappyHerd Agent Git file reads', () => {
    it('reads a modified file from the exact base and working-tree paths in parallel', async () => {
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file())).resolves.toEqual({
            kind: 'text',
            oldText: 'old text\n',
            newText: 'new text\n',
        });
    ...
    it('uses previousPath for renames and skips the absent side for add/delete', async () => {
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({
            path: 'new/name.ts',
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ status: 'added' }))).resolves.toEqual({
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ status: 'deleted' }))).resolves.toEqual({
    ...
    it('preserves a UTF-8 byte order mark so adding or removing one is a visible change', async () => {
        ...
        const result = await readHappyHerdAgentGitFile(SESSION_ID, BASE, file());
    ...
    it('returns explicit messages for unsupported and non-image binary files without reading', async () => {
        ...
        for (const status of ['conflicted', 'submodule', 'type_changed'] as const) {
            await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ status }))).resolves.toMatchObject({
                kind: 'message',
            });
        }
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ path: 'archive.dat', binary: true }))).resolves.toEqual({
    ...
    it('surfaces native missing and local oversize failures as human errors', async () => {
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file())).rejects.toThrow(
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ status: 'added' }))).rejects.toThrow(
    ...
    it('rejects malformed read responses and preserves transport errors', async () => {
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ status: 'added' }))).rejects.toThrow('invalid file response');

        const transportError = new Error('The computer did not respond');
        mocks.sessionRPC.mockReset();
        mocks.sessionRPC.mockRejectedValueOnce(transportError);
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ status: 'added' }))).rejects.toBe(transportError);
    });
    ...
    it('waits for the other started side after one RPC rejects', async () => {
        ...
        let settled = false;
        const reading = readHappyHerdAgentGitFile(SESSION_ID, BASE, file());
    ...
    it('reports binary-looking text and empty images without fabricating text', async () => {
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file({ path: 'empty.png', status: 'added', binary: true }))).resolves.toEqual({
    ...
    it('allows 20,000 combined lines, ignores a trailing newline, and caps larger text', async () => {
        ...
        await expect(readHappyHerdAgentGitFile(SESSION_ID, BASE, file())).resolves.toMatchObject({
