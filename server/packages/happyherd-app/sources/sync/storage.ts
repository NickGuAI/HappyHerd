...
/**
 * A session the agent retired, or a HappyHerd CLI session that has ended. Rig
...
export interface SessionRowData {
    ...
    hasUnread: boolean;
    // Native project identity supplied by Rig. HappyHerd CLI project cards derive
...
export type SessionListViewItem =
    ...
    | { type: 'projects-header'; source: 'rig' | 'happyherd' | 'personal' }
    | { type: 'project'; source: 'rig' | 'happyherd' | 'personal'; project: ProjectGroupData }
...
function buildSessionListViewData(
    ...
    const happyherdSessions: Session[] = [];
    ...
    topLevelSessions.forEach(session => {
        ...
        if (projectId && projects[projectId]?.kind === 'personal') {
            ...
            happyherdSessions.push(session);
        }
    });
    ...
    sortProjectSessions(happyherdSessions);
    ...
    const happyherdProjects = buildPathProjectGroups(
        happyherdSessions,
        toRow,
        isSessionActive,
        'happyherd',
    );
    if (happyherdProjects.length > 0) {
        listData.push({ type: 'projects-header', source: 'happyherd' });
        for (const project of happyherdProjects) {
            listData.push({ type: 'project', source: 'happyherd', project });
        }
    }
