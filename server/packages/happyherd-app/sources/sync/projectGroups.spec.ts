...
describe('buildProjectGroups', () => {
    it('gathers every worktree of a repo under one project', () => {
        const groups = buildProjectGroups([
            session({ id: 'a', projectId: 'p1', projectName: 'happyherd' }),
        ...
        expect(groups[0].name).toBe('happyherd');
    ...
    it('carries the project identity onto each row', () => {
        const groups = buildProjectGroups([
            session({ id: 'a', projectId: 'p1', projectName: 'happyherd', workspaceId: 'w1', workspaceName: 'feature' }),
        ], toRow, isActive);
...
describe('buildPathProjectGroups', () => {
    it('groups HappyHerd sessions by machine and working directory', () => {
        const groups = buildPathProjectGroups([
            session({ id: 'a', machineId: 'machine-1', path: '/projects/happyherd' }),
            session({ id: 'b', machineId: 'machine-1', path: '/projects/happyherd', active: false }),
        ], toRow, isActive, 'happyherd');
    ...
    it('does not merge the same path from different machines', () => {
        const groups = buildPathProjectGroups([
            session({ id: 'a', machineId: 'machine-1', path: '/projects/happyherd' }),
            session({ id: 'b', machineId: 'machine-2', path: '/projects/happyherd' }),
        ], toRow, isActive, 'happyherd');
    ...
    it('nests HappyHerd worktrees under their repository project', () => {
        const groups = buildPathProjectGroups([
            session({ id: 'worktree', path: '/projects/happyherd/.dev/worktree/simpler-home' }),
            session({ id: 'main', path: '/projects/happyherd' }),
        ], toRow, isActive, 'happyherd');
        ...
        expect(groups[0]).toMatchObject({
            name: 'happyherd',
            workspaces: [
                { id: '', name: null, sessions: [{ id: 'main' }] },
                {
                    id: '/projects/happyherd/.dev/worktree/simpler-home',
                    name: 'simpler-home',
    ...
    it('keeps worktrees with the same name separate across repositories', () => {
        const groups = buildPathProjectGroups([
            session({ id: 'happyherd', path: '/projects/happyherd/.dev/worktree/review' }),
            session({ id: 'website', path: '/projects/website/.dev/worktree/review' }),
        ], toRow, isActive, 'happyherd');

        expect(groups.map(group => group.name)).toEqual(['happyherd', 'website']);
    });
});

describe('filterProjectGroupSessions', () => {
    it('hides archived rows inside a HappyHerd project and refreshes its count', () => {
        const project: ProjectGroupData = {
            id: 'happyherd-project',
            name: 'happyherd',
            machineId: 'machine-1',
            sessionCount: 2,
            activeCount: 1,
