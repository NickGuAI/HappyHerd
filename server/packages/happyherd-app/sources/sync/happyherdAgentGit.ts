...
const HAPPYHERD_AGENT_GIT_RPC_METHODS = ['gitState', 'readFile', 'readFileAtRevision'] as const;
const HAPPYHERD_AGENT_GIT_REVISION_PATTERN = /^(?:[0-9a-f]{40}|[0-9a-f]{64})$/;
const HAPPYHERD_AGENT_GIT_MAX_BYTES = 512 * 1024;
const HAPPYHERD_AGENT_GIT_MAX_TEXT_LINES = 20_000;

const HAPPYHERD_AGENT_GIT_FILE_STATUSES = [
    'added',
    'conflicted',
    'copied',
    'deleted',
...
const HappyHerdAgentGitFactsSchema = z.object({
...
export const HappyHerdAgentGitFileSchema = z.object({
    path: z.string().min(1),
    previousPath: z.string().min(1).optional(),
    status: z.enum(HAPPYHERD_AGENT_GIT_FILE_STATUSES),
    staged: z.boolean(),
    unstaged: z.boolean(),
    binary: z.boolean(),
...
export type HappyHerdAgentGitFile = z.infer<typeof HappyHerdAgentGitFileSchema>;

export const HappyHerdAgentGitStateSchema = z.object({
    facts: HappyHerdAgentGitFactsSchema,
    comparison: z.enum(['ready', 'unavailable']),
    base: z.string().regex(HAPPYHERD_AGENT_GIT_REVISION_PATTERN).nullable(),
    changedFiles: z.number().int().nonnegative(),
    insertions: z.number().int().nonnegative(),
    deletions: z.number().int().nonnegative(),
    countsExact: z.boolean(),
    conflicted: z.boolean(),
    files: z.array(HappyHerdAgentGitFileSchema),
...
export type HappyHerdAgentGitState = z.infer<typeof HappyHerdAgentGitStateSchema>;

const HappyHerdAgentGitFailureSchema = z.object({
...
const HappyHerdAgentGitStateResponseSchema = z.union([
    z.object({
        success: z.literal(true),
        git: HappyHerdAgentGitStateSchema,
    }).passthrough(),
    HappyHerdAgentGitFailureSchema,
]);

const HappyHerdAgentGitReadFileResponseSchema = z.union([
    z.object({
        success: z.literal(true),
        content: z.string(),
...
const HappyHerdAgentGitReadFileAtRevisionResponseSchema = z.union([
    z.object({
        success: z.literal(true),
        content: z.string(),
    }).passthrough(),
...
export type HappyHerdAgentFileContent =
...
function requireSupportedSession(sessionId: string): void {
    if (!supportsHappyHerdAgentGit(sessionMetadata(sessionId))) {
        throw new Error('Git changes are not available for this session.');
    }
}

function parseGitRevision(value: string): boolean {
    return HAPPYHERD_AGENT_GIT_REVISION_PATTERN.test(value);
}
...
export function supportsHappyHerdAgentGit(metadata: Metadata | null | undefined): boolean {
    if (!isRigMetadataV1(metadata)) return false;
    const capabilities = metadata?.capabilities;
    return capabilities?.files?.read === true
        && Array.isArray(capabilities.rpcMethods)
        && HAPPYHERD_AGENT_GIT_RPC_METHODS.every((method) => capabilities.rpcMethods.includes(method));
}

/** Fetches and validates one native Git snapshot for a session. */
export async function getHappyHerdAgentGitState(sessionId: string): Promise<HappyHerdAgentGitState> {
    ...
    const parsed = parseFailureOrThrow(
        response,
        HappyHerdAgentGitStateResponseSchema,
        'HappyHerd Agent returned an invalid Git state.',
    );
    if (!parsed.success) throw new Error(parsed.error);
    return parsed.git;
}

function beforePathFor(file: HappyHerdAgentGitFile): string | null {
    return file.status === 'added' || file.status === 'untracked'
        ? null
        : file.previousPath ?? file.path;
}

function afterPathFor(file: HappyHerdAgentGitFile): string | null {
    return file.status === 'deleted' ? null : file.path;
}

function unsupportedFileMessage(file: HappyHerdAgentGitFile): string | null {
    switch (file.status) {
...
function hasTooManyTextLines(oldText: string, newText: string): boolean {
    const oldLines = countTextLinesWithin(oldText, HAPPYHERD_AGENT_GIT_MAX_TEXT_LINES);
    if (oldLines === null) return true;
    return countTextLinesWithin(newText, HAPPYHERD_AGENT_GIT_MAX_TEXT_LINES - oldLines) === null;
}
...
async function readSide(
    ...
    const schema = side === 'after'
        ? HappyHerdAgentGitReadFileResponseSchema
        : HappyHerdAgentGitReadFileAtRevisionResponseSchema;
    const parsed = schema.safeParse(response);
    if (!parsed.success) throw new Error('HappyHerd Agent returned an invalid file response.');
...
export async function readHappyHerdAgentGitFile(
    sessionId: string,
    gitBase: string,
    file: HappyHerdAgentGitFile,
): Promise<HappyHerdAgentFileContent> {
    requireSupportedSession(sessionId);

    const parsedFile = HappyHerdAgentGitFileSchema.safeParse(file);
    if (!parsedFile.success) throw new Error('HappyHerd Agent returned an invalid Git file.');
    const change = parsedFile.data;

    if (!parseGitRevision(gitBase)) {
        throw new Error('HappyHerd Agent returned an invalid Git comparison base.');
    }
    ...
    if (change.binary) {
        ...
        for (const [side, content] of [
            ...
            if (!inspected) {
                throw new Error('HappyHerd Agent returned invalid file contents.');
            }
            if (inspected.decodedBytes > HAPPYHERD_AGENT_GIT_MAX_BYTES) {
    ...
    if (oldBytes.decodedBytes > HAPPYHERD_AGENT_GIT_MAX_BYTES || newBytes.decodedBytes > HAPPYHERD_AGENT_GIT_MAX_BYTES) {
        throw new Error('This file is too large to preview on the phone.');
    }
