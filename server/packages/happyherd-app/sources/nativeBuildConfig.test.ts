...
function readNativeConfig(variant: string, overrides: Record<string, string> = {}) {
    ...
    for (const key of ['HAPPYHERD_APP_BUNDLE_ID', 'HAPPYHERD_EAS_PROJECT_ID', 'HAPPYHERD_EAS_OWNER', 'HAPPYHERD_APP_LINK_HOST', 'HAPPYHERD_IOS_BUILD_NUMBER', 'APPLE_TEAM_ID']) {
        delete env[key];
    }
...
describe('native distribution ownership', () => {
    ...
    it('uses the configured distribution account, build number, links, and matching OTA channel', () => {
        const config = readNativeConfig('preview', {
            HAPPYHERD_APP_BUNDLE_ID: 'com.example.app',
            HAPPYHERD_EAS_PROJECT_ID: '00000000-0000-4000-8000-000000000000',
            HAPPYHERD_EAS_OWNER: 'example-owner',
            HAPPYHERD_IOS_BUILD_NUMBER: '42',
        ...
        expect(readNativeConfig('production', { HAPPYHERD_APP_LINK_HOST: 'app.example.com' }).ios.associatedDomains)
