...
export type ModeOption = {
    ...
    unavailable?: boolean;
    isDefault?: boolean;
    /** First HappyHerd CLI version that can parse this hardcoded mode. */
...
// permissionModeLabels.ts. They are HappyHerd's own vocabulary, not a quote of each
...
// more. `default` is HappyHerd's stricter baseline — `untrusted` + workspace-write,
...
// Gemini effort is deliberately not encoded into separate model rows. HappyHerd's
// existing effort picker carries `low`/`medium`/`high` independently, and happyherd-cli
...
// separate model display names. HappyHerd presents them through its effort picker;
// happyherd-cli performs the final model-name mapping at the provider boundary.
...
/**
 * Active HappyHerd CLI sessions use the same machine-advertised model catalog as
 * New Session. Rig sessions keep their session-owned dynamic catalog, while
 * older/offline HappyHerd sessions retain the legacy session fallback.
 */
