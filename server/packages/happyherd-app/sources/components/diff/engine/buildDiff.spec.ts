...
describe('parsePatch', () => {
    ...
    it.each([
        ['git', 'diff --git a/counter.ts b/counter.ts'],
        ['generic diff', 'diff -u a/counter.ts b/counter.ts'],
        ['unified', ''],
    ])('keeps header-shaped hunk content and subsequent file boundaries in %s patches', (_format, prefix) => {
        const patch = [
            ...(prefix ? [prefix] : []),
            '--- a/counter.ts',
            '+++ b/counter.ts',
        ...
        const files = parsePatch(patch);
        expect(files.map((file) => file.path)).toEqual(['counter.ts', 'new.ts', 'old.ts']);
        expect(files.map((file) => file.kind)).toEqual(['modified', 'added', 'deleted']);
        expect(files[0].oldPath).toBe('counter.ts');
        expect(files[0].hunks.map((hunk) => hunk.lines)).toEqual([
            [{ type: 'del', text: '-- count;', oldNo: 1 }, { type: 'add', text: '++ count;', newNo: 1 }],
            [{ type: 'del', text: '-- another;', oldNo: 10 }, { type: 'add', text: '++ another;', newNo: 10 }],
        ]);
        expect(countPatchStats(patch)).toEqual({ additions: 3, deletions: 3 });
        const doc = buildDiffFromPatch(patch);
        expect(lineRows(doc.files[0].rows).map(textOf)).toEqual(['-- count;', '++ count;', '-- another;', '++ another;']);
        expect({ additions: doc.additions, deletions: doc.deletions }).toEqual(countPatchStats(patch));
    });

    it('preserves decrement-to-increment source in an actual git patch', () => {
        const root = mkdtempSync(join(tmpdir(), 'happyherd-diff-hunk-'));
        try {
            mkdirSync(join(root, 'before'));
            mkdirSync(join(root, 'after'));
            writeFileSync(join(root, 'before/counter.ts'), '-- count;\n');
            writeFileSync(join(root, 'after/counter.ts'), '++ count;\n');
            let patch = '';
            try {
                patch = execFileSync('git', ['diff', '--no-index', '--', 'before/counter.ts', 'after/counter.ts'], { cwd: root, encoding: 'utf8' });
            } catch (error) {
            ...
            expect(doc.files[0].path).toBe('after/counter.ts');
            expect(doc.files[0].oldPath).toBe('before/counter.ts');
            expect(lineRows(doc.files[0].rows).map(textOf)).toEqual(['-- count;', '++ count;']);
            expect(countPatchStats(patch)).toEqual({ additions: 1, deletions: 1 });
            expect({ additions: doc.additions, deletions: doc.deletions }).toEqual(countPatchStats(patch));
        } finally {
