...
describe('native syntax transport', () => {
    beforeEach(() => {
        ...
        vi.stubGlobal('__happyherdDiffSyntax', undefined);
    ...
    it('creates one runtime lazily, initializes Prism in scheduled work, and sends strings both ways', async () => {
        ...
        expect(bridge.create.mock.calls[0][0]).toEqual({ name: 'happyherd-diff-syntax' });
        expect((globalThis as any).__happyherdDiffSyntax).toBeUndefined();
    ...
    it('rejects cleanly when the tokenizer or native runtime fails', async () => {
        const worker = createSyntaxWorker();
        vi.stubGlobal('__happyherdDiffSyntax', () => { throw new Error('bad grammar'); });
