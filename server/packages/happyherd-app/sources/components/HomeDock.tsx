...
export const HomeDock = React.memo(({
    ...
    const selectedMachine = resolveAgentMachine(selectedChoice, resolvedAgentType)
        ?? selectedChoice?.happyherdMachine
    ...
    // HappyHerd Agent calls these workspaces, and names them; git calls them worktrees.
    const picksWorkspaces = selectedProjectId !== null;
    const createsNativeHappyHerdAgentWorkspace = agentType === 'rig'
        && picksWorkspaces
        && rigCreation !== null;
    ...
    const canCreateWorktree = createsNativeHappyHerdAgentWorkspace || worktreeCreationMachine !== null;
    ...
    const worktreeOptions = React.useMemo<ModeOption[]>(() => {
        if (!canCreateWorktree && !picksWorkspaces) {
            return [{
                key: '__none__',
                name: 'No worktree',
                description: t("uiCopy.notSupportedByValue", { value1: getHarnessName(agentType) }),
            }];
        }
                    ...
                    <Animated.View style={[styles.focusedComposerActions, focusedActionsRevealStyle]}>
                        ...
                        <Animated.View style={primaryActionFlashStyle}>
                            <BubblePressable
                                ...
                                accessibilityRole="button"
                                accessibilityLabel={primaryAction === 'stop'
                                    ...
                                    <View style={styles.startProgressContent}>
                                        ...
                                        {primaryAction === 'stop' && (
                                            <Animated.Text
                                                ...
                                                numberOfLines={1}
                                            >
                                                {t('newSession.interruptPressStop')}
                                            </Animated.Text>
                                        )}
                                    </View>
                                </View>
