...
const virtualModules: Record<string, string> = {
    ...
    '@/modal': `export const Modal = { prompt: async () => '  happyherd://terminal/review  ' };`,
...
describe('EmptyMainScreen onboarding reachability', () => {
    ...
    it.each(['light', 'dark'])('scrolls German native onboarding to camera, manual URL and archive actions on a short screen: %s', async theme => {
        ...
        await manual.click();
        expect(await page.evaluate(() => (globalThis as any).__CONNECTED_URL__)).toBe('happyherd://terminal/review');
        ...
        const commandColor = await page.getByText(de.uiCopy.npmIGHappyHerd, {exact:true}).evaluate(element => getComputedStyle(element).color);
