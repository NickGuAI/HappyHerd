...
export const ProviderContinuationSheet = React.memo(function ProviderContinuationSheet({
    ...
    const [loading, continueWithProvider] = useHappyHerdAction(async () => {
        if (!session || !sourceProvider || !targetProvider || !canContinue) {
            throw new HappyHerdError(t('session.providerContinuationUnavailable'), false);
        }
        ...
        if (!messagesLoaded) {
            try {
                ...
                throw new HappyHerdError(t('session.providerContinuationHandoffFailed'), false);
            }
        }
