...
import { HappyHerdAgentDiffView } from '@/components/HappyHerdAgentDiffView';
...
export const AllFilesDiffView = React.memo(function AllFilesDiffView(props: AllFilesDiffViewProps) {
    ...
    return isRigMetadata(session.metadata)
        ? <HappyHerdAgentDiffView key={props.sessionId} {...props} metadata={session.metadata} />
...
const LegacyAllFilesDiffView = React.memo(function LegacyAllFilesDiffView({
    ...
    React.useEffect(() => {
        ...
        // Commit signatures only with their results. A whitespace/context change
        // can cancel this batch; its replacement must still fetch those files.
        const requestedSignatures = new Map(toFetch.map((file) => [file.fullPath, fileSignature(file)]));

        (async () => {
            const fetched = await Promise.all(
                toFetch.map(async (file): Promise<FileDiffResult> => {
                    if (!sessionPath) {
                        return { file, content: null, error: t('files.failedToRead') };
                    }
                    ...
                    try {
                            ...
                            const [head, working] = await Promise.all([
                                file.status === 'untracked' || file.status === 'added'
                                    ...
                                    : sessionBash(sessionId, {
                                        command: buildGitShowBase64Command(file.oldPath ?? gitDiffPath),
                                        cwd: sessionPath,
                                        timeout: 8000,
                        ...
                        if (file.status === 'untracked') {
                            ...
                            if (!res.success || res.content === undefined) {
                                return { file, content: null, error: res.error || t('files.failedToRead') };
                            }
                            let contents: string;
                            try {
                        ...
                        return { file, content: null, error: err instanceof Error ? err.message : t('files.failedToRead') };
                    }
                })
            );
            ...
            for (const r of fetched) {
                fetchedSignatures.current.set(r.file.fullPath, requestedSignatures.get(r.file.fullPath)!);
            }
            setHasLoadedOnce(true);
        })();
