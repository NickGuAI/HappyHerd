...
vi.mock('@/sync/happyherdAgentGit', () => ({
    supportsHappyHerdAgentGit: (metadata: typeof rigMetadataFixture) => metadata.rigMetadataVersion === 1
        && ['gitState', 'readFile', 'readFileAtRevision'].every((method) => metadata.capabilities?.rpcMethods.includes(method)),
    getHappyHerdAgentGitState: mocks.getGit,
    readHappyHerdAgentGitFile: mocks.readFile,
}));
...
import { HappyHerdAgentDiffView } from './HappyHerdAgentDiffView';
...
function element(overrides: Record<string, unknown> = {}) {
    return React.createElement(HappyHerdAgentDiffView, { sessionId: 'session', metadata, onHeaderRightSlotChange: publishHeader, ...overrides });
}
...
describe('HappyHerd Agent Changes view', () => {
    ...
    it('shows upgrade guidance for an older daemon instead of clean state or a shell fallback', async () => {
        ...
        expect(text()).toContain('Update HappyHerd Agent');
