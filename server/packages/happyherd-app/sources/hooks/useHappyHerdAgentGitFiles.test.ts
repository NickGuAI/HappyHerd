...
import type { HappyHerdAgentGitFile } from '@/sync/happyherdAgentGit';

const { readFile } = vi.hoisted(() => ({ readFile: vi.fn() }));
vi.mock('@/sync/happyherdAgentGit', () => ({ readHappyHerdAgentGitFile: readFile }));
import { useHappyHerdAgentGitFiles } from './useHappyHerdAgentGitFiles';

const base = 'a'.repeat(40);
const files: HappyHerdAgentGitFile[] = ['a.ts', 'b.ts', 'c.ts'].map((path) => ({
    path, status: 'modified', staged: false, unstaged: false, binary: false,
}));
let current: ReturnType<typeof useHappyHerdAgentGitFiles>;
let renderer: ReturnType<typeof create> | undefined;
function Harness({ sessionId = 'session' }: { sessionId?: string }) {
    current = useHappyHerdAgentGitFiles(sessionId, base, files);
    return null;
}
...
describe('on-demand HappyHerd Agent file reads', () => {
