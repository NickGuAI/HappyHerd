...
export function useStartSessionFromDraft() {
    ...
    const startSession = React.useCallback(async (
        ...
        // changes cannot send HappyHerd Agent work to HappyHerd CLI (or vice versa).
        ...
        try {
            happyherdAgentTarget = rigCreation
                ? resolveHappyHerdAgentSpawnTarget({
                    projectId: selectedProjectId,
        ...
        // Native Project workspace creation belongs to HappyHerd Agent. An
        // ordinary git worktree may instead belong to the paired HappyHerd CLI,
        ...
        const isCurrentTarget = () => {
            ...
            return ownsCreatedSession && !run.canceled
                && latest.agentType === draft.agentType
                ...
                && latest.permissionMode === draft.permissionMode
                && latest.effortLevel === draft.effortLevel;
        };
        ...
        try {
            ...
            if (initialPrompt || attachments.length > 0) {
                const receipt = await sync.sendMessage(sessionId, initialPrompt, {
                    source: 'new_session',
                    attachments,
                    signal: run.controller.signal,
                    isCurrent: isCurrentTarget,
                    onAccepted: () => { run.accepted = true; },
