...
import {
    ...
    type HappyHerdAgentFileContent,
    type HappyHerdAgentGitFile,
} from '@/sync/happyherdAgentGit';

export type HappyHerdAgentFileResult =
    | { content: HappyHerdAgentFileContent; error: null }
    | { content: null; error: string };

/** One displayed Git snapshot owns its requested files; the viewer remounts on refresh. */
export function useHappyHerdAgentGitFiles(sessionId: string, base: string, files: HappyHerdAgentGitFile[]) {
    const [requested, setRequested] = React.useState<ReadonlySet<string>>(() => new Set());
    const [results, setResults] = React.useState<ReadonlyMap<string, HappyHerdAgentFileResult>>(() => new Map());
    ...
    React.useEffect(() => {
        ...
        for (const path of requested) {
            ...
            const finish = (result: HappyHerdAgentFileResult) => {
                if (generation.current !== currentGeneration) return;
                inFlight.current.delete(path);
                setResults((previous) => new Map(previous).set(path, result));
            };
            void readHappyHerdAgentGitFile(sessionId, base, file).then(
