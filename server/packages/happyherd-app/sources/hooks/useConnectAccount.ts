...
import { t } from '@/text';
import { normalizeProductLink } from '@/utils/normalizeProductLink';
...
export function useConnectAccount(options?: UseConnectAccountOptions) {
    ...
    const processAuthUrl = React.useCallback(async (url: string) => {
        ...
        if (!url.startsWith('happyherd:///account?')) {
        ...
        setIsLoading(true);
        try {
            const tail = url.slice('happyherd:///account?'.length);
    ...
    React.useEffect(() => {
        if (CameraView.isModernBarcodeScannerAvailable) {
            const subscription = CameraView.onModernBarcodeScanned(async (event) => {
                ...
                if (normalizeProductLink(event.data).startsWith('happyherd:///account?')) {
