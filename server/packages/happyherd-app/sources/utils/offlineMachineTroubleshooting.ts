 ...
 * HappyHerd logs instead of giving generic networking advice.
 */
export function buildOfflineMachineTroubleshooting(
    ...
    const projectName = session?.metadata?.project?.name?.trim()
        ...
        || 'HappyHerd';
    const happyHomeDir = choice?.happyherdMachine?.metadata?.happyHomeDir?.trim()
        || choice?.rigMachine?.metadata?.happyHomeDir?.trim()
        || session?.metadata?.happyHomeDir?.trim()
        || '~/.happyherd';
    const aiPrompt = `In ${happyHomeDir}, diagnose why HappyHerd cannot reach "${machineName}" for project "${projectName}".`;
