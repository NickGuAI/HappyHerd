...
import { getShellControl, happyherdToolDisplay } from './happyherdToolDisplay';
...
export function isTerminalToolName(name: string): boolean {
    return TERMINAL_TOOL_NAMES.has(name) || happyherdToolDisplay[name]?.category === 'terminal';
}
...
export function getToolSummaryCategory(toolName: string): ToolSummaryCategory {
    const happyherdCategory = happyherdToolDisplay[toolName]?.category;
    if (happyherdCategory) return happyherdCategory;
...
export function getToolActivityLabel(tool: Pick<ToolCall, 'name' | 'title' | 'input' | 'description'>): string {
    ...
    const action = happyherdToolDisplay[tool.name]?.title
...
export function getToolDisplayTitle(tool: Pick<ToolCall, 'name' | 'title'>): string {
    return tool.title?.trim() || happyherdToolDisplay[tool.name]?.title || tool.name;
}
...
function getToolActivityAction(category: ToolSummaryCategory, toolName: string): string {
    switch (category) {
        ...
        case 'search':
            return t('toolGroup.searchedAction');
        case 'web':
