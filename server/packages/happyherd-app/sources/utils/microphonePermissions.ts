...
export function showMicrophonePermissionDeniedAlert(canAskAgain: boolean = false) {
  const title = t('uiCopy.microphoneAccessRequired');
  const message = canAskAgain
    ? t('uiCopy.microphoneGrantPrompt')
    : t('uiCopy.microphoneEnableSettings');

  if (Platform.OS === 'web') {
    // Web: Show browser-specific instructions
    Modal.alert(
      title,
      t("uiCopy.pleaseAllowMicrophoneAccessInYourBrowserSettingsYouMay"),
      [{ text: 'OK' }]
    );
  } else {
