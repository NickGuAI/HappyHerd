...
/**
 * The coding agents HappyHerd can start a session with, and what to call them in
 * the UI. `rig` is the wire/CLI id of HappyHerd's own agent; it is only ever shown
 * as "HappyHerd". `agy` is Antigravity's binary name for the same reason.
 */
...
export function isHarnessAvailable({
    availability,
    happyherdAgentAvailable,
    key,
}: {
    availability?: HarnessAvailability | null;
    happyherdAgentAvailable: boolean;
    key: NewSessionAgentType;
}): boolean {
    if (key === 'rig') return happyherdAgentAvailable;
 ...
 * catalog. GrokBuild and Antigravity are exceptions to both fallbacks: they are
 * only listed after an explicit installation report. A retired harness is also
...
export function listAvailableHarnesses({
    ...
    const keys = HARNESS_ORDER.filter((key) => (
        ...
        || isHarnessAvailable({ availability, happyherdAgentAvailable, key })
    ));
