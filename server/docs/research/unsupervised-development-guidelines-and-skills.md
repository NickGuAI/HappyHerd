...
HappyHerd already has the runtime pieces needed for long-running and partially unsupervised agent work, but the guidance for how to operate, validate, and trust those workflows is scattered. This note captures:
...
## What HappyHerd documents today
...
- [`../../packages/happyherd-app/CLAUDE.md`](../../packages/happyherd-app/CLAUDE.md)
- [`../../packages/happyherd-server/CLAUDE.md`](../../packages/happyherd-server/CLAUDE.md)
...
## How unsupervised development currently works in HappyHerd
...
Important consequence: HappyHerd already has a protocol-level understanding of subagents and tool-spawned sidechains. The missing layer is not transport. The missing layer is process: when unsupervised operation is allowed, how it is validated, and what counts as evidence that it is safe and working.

## What is missing

There is no single document that answers these operator questions:

- What counts as an approved unsupervised workflow in HappyHerd?
...
That means HappyHerd has the runtime for autonomous behavior, but not the operating manual.
...
The most important ideas worth copying into HappyHerd are:
...
This matches HappyHerd's needs well. HappyHerd wraps multiple agents already, so a portable procedural layer is more valuable than yet another provider-specific prompt convention.
...
This lines up with HappyHerd's existing protocol support for `subagent` event streams. The runtime already understands subagent identity; the missing piece is guidance for when and how to use that capability well.

## Implications for HappyHerd

The practical conclusion is:

- HappyHerd does **not** need to invent subagent transport first.
- HappyHerd does **not** primarily need more architecture docs.
- HappyHerd **does** need an operator/process layer for autonomous work.
...
3. `docs/skills-and-subagents.md`
   ...
   - how HappyHerd should interoperate with existing agent skill directories

## Recommended design direction

If HappyHerd decides to adopt skills more explicitly, the most compatible path is likely:
...
- keep HappyHerd focused on orchestration, observability, and validation rather than becoming a separate skill runtime

That approach keeps HappyHerd aligned with the broader ecosystem while preserving its real value: remote control, daemonized execution, protocol normalization, and session observability across providers.

## Bottom line

HappyHerd currently has the substrate for unsupervised development, but not the handbook.
