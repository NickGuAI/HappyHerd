...
This document describes the HappyHerd CLI (`packages/happyherd-cli`) and its daemon. The CLI is both an interactive tool and a background session manager that keeps machine state in sync with the server.

## System overview

```mermaid
graph TB
    subgraph "HappyHerd CLI"
        Entry[src/index.ts]
        API[API Client]
...
- **Persistence/config:** `src/persistence.ts` + `src/configuration.ts` manage local state in `~/.happyherd`.
...
graph LR
    ...
    subgraph "Environment Overrides"
        direction TB
        E1[HAPPYHERD_HOME_DIR]
        E2[HAPPYHERD_SERVER_URL]
        E3[HAPPYHERD_WEBAPP_URL]
        E4[HAPPYHERD_VARIANT]
        E5[HAPPYHERD_EXPERIMENTAL]
        E6[HAPPYHERD_DISABLE_CAFFEINATE]
    end

    E1 -.-> settings & access & daemon & logs
```

Local state lives under `~/.happyherd` (or `HAPPYHERD_HOME_DIR`):
...
- `HAPPYHERD_SERVER_URL` and `HAPPYHERD_WEBAPP_URL` override defaults.
- `HAPPYHERD_VARIANT`, `HAPPYHERD_EXPERIMENTAL`, `HAPPYHERD_DISABLE_CAFFEINATE` control behavior.
...
- CLI entry: `packages/happyherd-cli/src/index.ts`
- Daemon: `packages/happyherd-cli/src/daemon`
- Control server/client: `packages/happyherd-cli/src/daemon/controlServer.ts`, `packages/happyherd-cli/src/daemon/controlClient.ts`
- API clients: `packages/happyherd-cli/src/api`
- Persistence: `packages/happyherd-cli/src/persistence.ts`
- Config: `packages/happyherd-cli/src/configuration.ts`
