...
This is the short version of what matters most for HappyHerd.

| Vendor | Core transport | Transcript shape | Subagents / tasks | Permissions | Sandbox story | Sync / remote story | HappyHerd takeaway |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
- OpenCode is the most attractive end-to-end reference for HappyHerd right now.
...
## Suggested design direction for HappyHerd
...
- Evaluate Superset's Electric SQL cloud→local sync pattern and cloud DB command queue as alternatives to HappyHerd's current sync approach.
- Consider Superset's host-service extraction pattern (injectable providers, manifest-based durability) for HappyHerd's CLI/server split.
