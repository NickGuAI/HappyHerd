...
**File:** `packages/happyherd-app/sources/sync/storageTypes.ts` (line 119)
...
**File:** `packages/happyherd-app/sources/hooks/useCLIDetection.ts` — **DELETE**
...
| File | Reason |
|------|--------|
| `packages/happyherd-app/sources/sync/profileUtils.ts` | Built-in profile definitions & docs |
| `packages/happyherd-app/sources/sync/profileSync.ts` | Profile sync service |
| `packages/happyherd-app/sources/components/ProfileEditForm.tsx` | Profile editor form |
| `packages/happyherd-app/sources/components/EnvironmentVariablesList.tsx` | Env var list component |
| `packages/happyherd-app/sources/components/EnvironmentVariableCard.tsx` | Env var card component |
| `packages/happyherd-app/sources/hooks/useEnvironmentVariables.ts` | Queries daemon env vars for profiles |
| `packages/happyherd-app/sources/hooks/envVarUtils.ts` | Env var substitution utils |
| `packages/happyherd-app/sources/hooks/useCLIDetection.ts` | Replaced by daemon-side detection |
| `packages/happyherd-app/sources/app/(app)/settings/profiles.tsx` | Profile settings page |

### 2.2 Files to EDIT — remove profile references

**`packages/happyherd-app/sources/sync/settings.ts`**
...
**`packages/happyherd-app/sources/components/AgentInput.tsx`**
...
**`packages/happyherd-app/sources/components/SettingsView.tsx`**
- Remove: profiles settings Item (lines 325-330)

**`packages/happyherd-app/sources/sync/ops.ts`**
...
| File | Reason |
|------|--------|
| `packages/happyherd-app/sources/app/(app)/new/index.tsx` | Main wizard page — will be replaced with new simpler screen |
| `packages/happyherd-app/sources/app/(app)/new/pick/machine.tsx` | Machine picker sub-screen (uses SearchableListSelector with favorites) |
| `packages/happyherd-app/sources/app/(app)/new/pick/path.tsx` | Path picker sub-screen (uses SearchableListSelector with favorites) |
| `packages/happyherd-app/sources/app/(app)/new/pick/profile-edit.tsx` | Profile edit sub-screen |
| `packages/happyherd-app/sources/components/NewSessionWizard.tsx` | Legacy wizard component |
| `packages/happyherd-app/sources/utils/tempDataStore.ts` | Temp data between wizard screens |
...
**`packages/happyherd-app/sources/app/(app)/_layout.tsx`**
- Remove: Stack.Screen entries for `new/index`, `new/pick/machine`, `new/pick/path`, `new/pick/profile-edit` (lines 300-327)

**`packages/happyherd-app/sources/components/MainView.tsx`** — keep `router.push('/new')` (route stays the same)

**`packages/happyherd-app/sources/components/SidebarView.tsx`** — keep `router.push('/new')`

**`packages/happyherd-app/sources/components/HomeHeader.tsx`** — keep `router.push('/new')`

**`packages/happyherd-app/sources/components/CommandPalette/CommandPaletteProvider.tsx`** — keep `router.push('/new')`

**`packages/happyherd-app/sources/components/EmptySessionsTablet.tsx`** — keep `router.push('/new')`
...
**`packages/happyherd-app/sources/app/(app)/new/index.tsx`** — replace with stub that:
...
1. **Typecheck:** `yarn typecheck` in `happyherd-app` and `happyherd-cli` — must pass with no errors
