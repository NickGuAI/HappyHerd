...
- [x] Create `packages/happyherd-cli/src/sessionProtocol/types.ts` with:
...
- [x] Add `sendSessionProtocolMessage(envelope)` to `ApiSessionClient` in `packages/happyherd-cli/src/api/apiSession.ts`
