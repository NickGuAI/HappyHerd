...
## Overview
Create a portable, self-contained distribution of happyherd-server as a single Bun-compiled binary. It runs without Redis (already has in-memory event bus), uses PGlite for embedded PostgreSQL, and local filesystem for file storage. CLI provides `happyherd-server migrate` and `happyherd-server serve` commands.
...
- [ ] Add `@electric-sql/pglite`, `pglite-prisma-adapter` to happyherd-server dependencies
...
- [ ] Add `build:standalone` script to `package.json`: `bun build ./sources/standalone.ts --compile --outfile happyherd-server`
...
- [ ] Test: `./happyherd-server migrate` creates and migrates a PGlite database
- [ ] Test: `./happyherd-server serve` starts the server with PGlite

### Task 7: Verify end-to-end
- [ ] Build the binary
- [ ] Run `./happyherd-server migrate` — verify database is created in `./data/`
- [ ] Run `./happyherd-server serve` — verify server starts, health endpoint responds
...
- Consider adding `happyherd-server init` command for first-time setup
