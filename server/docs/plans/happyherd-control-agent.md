# happyherd-control-agent CLI Tool

## Overview
A new standalone CLI tool (`happyherd-control-agent`) in `packages/happyherd-control-agent` that acts as a dedicated client for controlling HappyHerd Coder agents remotely. Unlike `happyherd-cli` which both runs and controls agents, `happyherd-control-agent` only controls them — listing machines, spawning sessions on a machine, creating sessions, sending messages, reading history, monitoring state, and stopping sessions.

This is a completely separate client from `happyherd-cli`. It has its own authentication flow (account auth via QR code, same as device linking in the mobile app), its own credential storage (`~/.happyherd/agent.key`), and is written from scratch with no code sharing.

## Context
- **Existing system**: Monorepo with `happyherd-cli` (agent runtime + control), `happyherd-server` (Fastify + PostgreSQL + Redis), `happyherd-app` (React Native mobile)
- **Server API**: REST endpoints at `https://api.cluster-fluster.com` + Socket.IO at `/v1/updates`
- **Authentication**: Uses account auth flow (`/v1/auth/account/request` + `/v1/auth/account/response`) — generates ephemeral keypair, displays QR code (`happyherd:///account?[base64url-publicKey]`), user scans with existing HappyHerd mobile app to approve, receives encrypted account secret
- **Credential storage**: `~/.happyherd/agent.key` (separate from happyherd-cli's `~/.happyherd/access.key`)
...
- **Integration tests**: Use the real environment bootstrap (`yarn env:up:authenticated`) and exercise the live server + daemon + CLI stack. Do not use mocked acceptance coverage for `happyherd-control-agent spawn`.
...
- [x] Create `packages/happyherd-control-agent/` directory with `package.json` (name: `happyherd-control-agent`, type: module, bin: `./bin/happyherd-control-agent.mjs`)
- [x] Create `tsconfig.json` with strict mode, path aliases (`@/` → `src/`), ESM output
- [x] Create `bin/happyherd-control-agent.mjs` entry point wrapper (mirrors happyherd-cli pattern: spawns node with `--no-warnings`)
...
- [x] Create `src/config.ts` — reads `HAPPYHERD_SERVER_URL` (default: `https://api.cluster-fluster.com`), `HAPPYHERD_HOME_DIR` (default: `~/.happyherd`), derives credential file path as `${happyHomeDir}/agent.key`
- [x] Create `src/credentials.ts`:
  ...
  - `readCredentials(config)` — parses `~/.happyherd/agent.key` JSON `{ token, secret }`, decodes secret from base64, derives contentKeyPair via `deriveContentKeyPair(secret)`. Returns `Credentials` or `null` if file missing.
  - `writeCredentials(config, token, secret)` — writes `{ token, secret: base64(secret) }` to `~/.happyherd/agent.key`
  - `clearCredentials(config)` — deletes `~/.happyherd/agent.key`
  - `requireCredentials(config)` — calls `readCredentials`, throws with "Run `happyherd-control-agent auth login` first" if null
...
### Task 4: Authentication command (`happyherd-control-agent auth`)
- [x] Create `src/auth.ts` implementing the account auth flow:
  ...
  3. Generate QR code data: `happyherd:///account?` + base64url(keypair.publicKey)
  4. Display QR code in terminal using `qrcode-terminal`
  5. Print instructions: "Scan this QR code with the HappyHerd app (Settings → Account → Link New Device)"
...
- [x] Add `happyherd-control-agent auth login` subcommand that runs the flow above
- [x] Add `happyherd-control-agent auth logout` subcommand that calls `clearCredentials()`
- [x] Add `happyherd-control-agent auth status` subcommand that reads credentials and prints auth status (authenticated / not authenticated)
...
- [x] Create `src/index.ts` using `commander` with program name `happyherd-control-agent`
- [x] `happyherd-control-agent list` — calls `listSessions`, displays table: ID (truncated), name/summary, path, status (active/inactive), last active time. With `--json` outputs raw JSON. With `--active` filters to active only.
- [x] `happyherd-control-agent status <session-id>` — fetches session via list + filter by ID prefix, connects Socket.IO to get live state, displays: session ID, metadata (path, host, lifecycle state), agent state (idle/busy, pending requests count), last message preview. With `--json` outputs raw JSON. Disconnects after displaying.
...
- [x] `happyherd-control-agent create --tag <tag> [--path <path>]` — creates new session with given tag and metadata (path defaults to cwd, host to hostname). Prints session ID. With `--json` outputs full session JSON.
- [x] `happyherd-control-agent send <session-id> <message>` — resolves session key, connects Socket.IO, sends user message (encrypted with AES-256-GCM), optionally waits for idle with `--wait`, and supports `--yolo` to send `permissionMode=yolo`. Disconnects after. Prints confirmation. With `--json` outputs message details.
...
- [x] `happyherd-control-agent history <session-id>` — fetches messages via HTTP, resolves session encryption key (dataKey or legacy), decrypts each message, displays in chronological order with role/timestamp. With `--json` outputs raw JSON. With `--limit <n>` limits output.
- [x] `happyherd-control-agent stop <session-id>` — connects Socket.IO, sends `session-end` event, disconnects. Prints confirmation.
- [x] `happyherd-control-agent wait <session-id> [--timeout <seconds>]` — connects Socket.IO, waits for agent idle state (no pending requests, not controlled by user), prints when idle or times out (default 300s). Exit code 0 on idle, 1 on timeout.
...
- [x] Verify interop: session created by happyherd-control-agent is visible and controllable from mobile app
- [x] Verify interop: session created by happyherd-cli can be listed and history read by happyherd-control-agent
- [x] Run full test suite (unit tests)
- [x] Run linter — all issues must be fixed

### Task 11: [Final] Update documentation
- [x] Add README.md to `packages/happyherd-control-agent/` with usage examples for all commands
- [x] Update root README if it references packages

### Task 12: Machines and spawn
- [x] Add `happyherd-control-agent machines [--active] [--json]`
- [x] Add machine record decryption using the existing account content key derivation and record data key pattern
- [x] Add `happyherd-control-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]`
- [x] Reuse the existing machine RPC contract (`spawn-happy-session`) without encryption shortcuts
- [x] Add a real integration test that boots `yarn env:up:authenticated`, authenticates `happyherd-control-agent`, lists machines, spawns via the real daemon RPC path, and verifies the live session

## Technical Details

### CLI Commands Summary
```
happyherd-control-agent auth login                          # Authenticate via QR code (scanned by HappyHerd mobile app)
happyherd-control-agent auth logout                         # Clear stored credentials
happyherd-control-agent auth status                         # Show authentication status

happyherd-control-agent machines [--active] [--json]        # List machines
happyherd-control-agent list [--active] [--json]            # List all sessions
happyherd-control-agent spawn --machine <machine-id> [--path <path>] [--agent <agent>] [--create-dir] [--json]  # Spawn on a machine
happyherd-control-agent status <session-id> [--json]        # Get live session state
happyherd-control-agent create --tag <tag> [--path <path>] [--json]  # Create new session
happyherd-control-agent send <session-id> <message> [--yolo] [--wait] [--json]  # Send message
happyherd-control-agent history <session-id> [--limit <n>] [--json]    # Read message history
happyherd-control-agent stop <session-id>                   # Stop a session
happyherd-control-agent wait <session-id> [--timeout <s>]   # Wait for agent to become idle
```

### Authentication Flow (Account Auth)
```
happyherd-control-agent                          HappyHerd Server                    HappyHerd Mobile App
     |                                    |                               |
     ...
     |   happyherd:///account?[base64url-key] |                               |
     |                                    |                               |
     |                                    |  <-- User scans QR code ------+
     |                                    |                               |
...
### Credential File Format (`~/.happyherd/agent.key`)
```json
{
...
**For new sessions (created by happyherd-control-agent):**
...
## Post-Completion
**Manual verification:**
- Test full auth flow: run `happyherd-control-agent auth login`, scan QR with HappyHerd app, verify credentials saved
...
- Test `history` command for sessions created by both `happyherd-control-agent` and `happyherd-cli`
- Test cross-client interop: messages from happyherd-control-agent readable by mobile app and vice versa

**Distribution:**
- Package can be published to npm as `happyherd-control-agent`
- Alternatively, users install from monorepo via `yarn workspace happyherd-control-agent build`
