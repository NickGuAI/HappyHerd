# `happyherd server` — bundled self-host mode

## Goal

Ship a zero-network, fully self-hosted HappyHerd in the regular `npm i -g happyherd` package. One foreground command — `happyherd server` — runs the sync server + web app on `localhost` and writes the local URL into `~/.happyherd/settings.json`. Every other HappyHerd process (daemon, `happyherd claude`, etc.) reads that URL from settings. No analytics, no external endpoints, no extra installs.
...
#   HappyHerd server listening at http://127.0.0.1:3005
...
### 1. `happyherd server` runs the server synchronously in the foreground
...
interface Settings {
  ...existing...
  serverUrl?: string;   // e.g. 'http://127.0.0.1:3005' — when set, used by all happyherd processes
}
```

That's the entire persistence surface. No nested `selfHost: { enabled, port, host, dataDir }` object.

- `happyherd server` writes this on startup.
- `happyherd server --port 8080 --host 0.0.0.0` writes `http://0.0.0.0:8080` (or printed LAN IP).
- `happyherd server --reset` clears the field and exits without starting the server.
- Users can also just edit `~/.happyherd/settings.json` by hand.
...
```ts
this.serverUrl =
  process.env.HAPPYHERD_SERVER_URL              // env var still wins (debug/override)
  || settings.serverUrl                     // NEW — from ~/.happyherd/settings.json
...
HappyHerd-server serves the bundled webapp on its own port. At serve time, the static handler rewrites `index.html` to inject:

```html
<script>window.__HAPPYHERD_CONFIG__ = { serverUrl: location.origin, disableAnalytics: true };</script>
```

And `packages/happyherd-app/sources/sync/serverConfig.ts:10-14` gets one extra fallback:

```ts
export function getServerUrl(): string {
  return serverConfigStorage.getString(SERVER_KEY) ||
         (globalThis as any).__HAPPYHERD_CONFIG__?.serverUrl ||  // NEW
         process.env.EXPO_PUBLIC_HAPPYHERD_SERVER_URL ||
         DEFAULT_SERVER_URL;
}
```

Production `app.happy.engineering` never has `__HAPPYHERD_CONFIG__` set → no behavior change there. Self-host injects it → web calls its own origin. One bundle, two behaviors.

### 6. Daemon needs no changes

Because the daemon reads `configuration.serverUrl` like every other entry point, and `configuration` now reads from `settings.serverUrl`, the daemon Just Works™ when a user has run `happyherd server`. Zero code in `daemon/run.ts`. (v2 can add daemon-managed lifecycle if needed.)

## Disabling analytics (defense in depth)

### State of the world today
- **happyherd-cli, happyherd-control-agent, happyherd-server: no analytics at all** (verified — see Audit reference below).
- **happyherd-app (web + mobile): PostHog only**, already gated: `tracking = config.postHogKey ? new PostHog(...) : null` (`packages/happyherd-app/sources/track/tracking.ts:4`). Every call site is `tracking?.capture(...)`, so absence of the key already short-circuits everything.
...
const analyticsDisabled =
  ...
  (globalThis as any).__HAPPYHERD_CONFIG__?.disableAnalytics === true;
...
- `window.__HAPPYHERD_CONFIG__.disableAnalytics = true` injected by happyherd-server's static handler.

**Layer 3 — Self-host mode auto-disables.** The HTML rewrite in `happyherd server` always includes `disableAnalytics: true`:

```ts
injectHtmlConfig: { serverUrl: '/', disableAnalytics: true }
```

So any webapp served by `happyherd server` has analytics off regardless of what was baked into the bundle at build time. If a user's enterprise rebuilds happyherd with their own PostHog key (intentionally), they still can't accidentally leak when they `happyherd server` — the script-tag injection wins.
...
No analytics code exists. Nothing to disable. (If we ever add CLI telemetry in the future, the same env var name `HAPPYHERD_DISABLE_ANALYTICS=1` should be the kill switch — drop `EXPO_PUBLIC_` prefix.)
...
3. **Web app**: `window.__HAPPYHERD_CONFIG__` only ever set by the static handler in self-host mode. Production builds at `app.happy.engineering` never see it.
...
│   ├── webapp/                  pre-built happyherd-app web export (~28 MB)
│   │   ├── index.html
│   │   ├── _expo/
│   │   └── assets/
│   ├── pglite/
...
```
happyherd server                      start server, write URL, block
happyherd server --port N             custom port
happyherd server --host 0.0.0.0       bind for LAN/mobile
happyherd server --reset              clear settings.serverUrl, exit
happyherd server --no-persist         start server but don't touch settings (test mode)
```
...
**`packages/happyherd-server/sources/index.ts`** (NEW, ~40 LOC)
...
```
When `staticDir` set, register `@fastify/static` on `/*` with an `onSend` hook that prepends the `__HAPPYHERD_CONFIG__` script tag into `index.html`.
...
**`packages/happyherd-server/package.json`**
- Flip `"private": false`
- Rename to `"@slopus/happy-server"`
- Add `"main": "./sources/index.ts"` + `exports` map
- Add `@fastify/static` dep

**`packages/happyherd-app/sources/sync/serverConfig.ts`**
- 3-line `__HAPPYHERD_CONFIG__` fallback

**`packages/happyherd-app/sources/track/tracking.ts`**
- 3-layer kill switch (env var + `__HAPPYHERD_CONFIG__.disableAnalytics`) (~8 LOC)
...
- Generated on first `happyherd server` start: 32 random bytes → `~/.happyherd/server/master-secret` (0600 perms).
- Reused on subsequent starts.
- If deleted, all self-host data becomes unreadable. Print loud warning in `happyherd server --reset` and in startup logs.
- No derivation from existing happyherd credentials — keep self-host data independent.
...
├── logs/                    existing
└── server/                  NEW — only created by `happyherd server`
...
3. **Sharp dep in happyherd-server** — ~20 MB native binary per platform. Either npm optional-deps (sharp does this itself) or stub out image processing for the self-host minimum.
...
3. Add `window.__HAPPYHERD_CONFIG__` fallback to `serverConfig.ts`. No behavior change in production.
...
6. `happyherd server` command end-to-end. Smoke test: `happyherd server` in one terminal, `happyherd claude` in another, verify CLI targets localhost.
7. Docs / README.
8. Release.

## v2 ideas (not now)

- Daemon-managed server lifecycle (`happyherd daemon start --server`, auto-restart, launchd integration).
...
- `happyherd server status` / `happyherd server stop` as ergonomics on top of the foreground command.
...
- `PUBLIC_WEBAPP_URL` for happyherd-server OAuth redirects.
...
### Analytics
Only `happyherd-app` has PostHog (`packages/happyherd-app/sources/track/tracking.ts:4`), already gated by `config.postHogKey`. CLI / agent / server have zero analytics (grepped).

### Server URL configurability (already exists)
- CLI / agent: `HAPPYHERD_SERVER_URL` env var (default `https://api.cluster-fluster.com`).
- Web app: in-app Settings → Server screen + `EXPO_PUBLIC_HAPPYHERD_SERVER_URL`.
- Server: `pnpm standalone:dev` runs with PGlite — no Docker, no Redis. Entry: `packages/happyherd-server/sources/standalone.ts`.

### Hardcoded `happy.engineering` URLs (cosmetic)
- `happyherd-app/sources/components/SettingsView.tsx:396` — privacy link
- `happyherd-cli/src/ui/doctor.ts:225`, `src/claude/utils/systemPrompt.ts:20-23`, `src/commands/connect.ts:74` — docs / attribution
- `happyherd-server/sources/app/api/routes/connectRoutes.ts` — GitHub OAuth redirect
- `happyherd-app/app.config.js:46,77` + iOS/Android manifests — universal link domain

### Published package size
`happyherd@1.1.7` on npm: **121 MB packed / 238 MB unpacked**. Dominated by `tools/unpacked/difft` (113 MB) + ripgrep platform archives (~27 MB).
