...
⚠️ No lint script is defined in `packages/happyherd-server/package.json`, so linter execution is currently not available.
...
- Update `happyherd-control-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `happyherd-control-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
