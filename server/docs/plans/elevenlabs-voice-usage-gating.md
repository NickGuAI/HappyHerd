...
- Server token route: `packages/happyherd-server/sources/app/api/routes/voiceRoutes.ts`
- Client token fetch: `packages/happyherd-app/sources/sync/apiVoice.ts`
- Voice start decision: `packages/happyherd-app/sources/realtime/RealtimeSession.ts`
- ElevenLabs client session start:
  - `packages/happyherd-app/sources/realtime/RealtimeVoiceSession.tsx`
  - `packages/happyherd-app/sources/realtime/RealtimeVoiceSession.web.tsx`

## Existing Secret Assumptions

The repo already assumes ElevenLabs API access exists on the server:

- `packages/happyherd-server/sources/app/api/routes/voiceRoutes.ts` reads `process.env.ELEVENLABS_API_KEY`.
- `packages/happyherd-server/deploy/handy.yaml` extracts `/handy-elevenlabs`.
...
1. Derive a stable pseudonymous ElevenLabs `user_id` from the HappyHerd user ID.
...
`elevenUserId = "u_" + base64url(HMAC_SHA256(APP_SECRET, happyherdUserId))`

This keeps the join key stable across sessions without exposing the raw HappyHerd account ID to ElevenLabs.
...
Server authenticates JWT
  |
  +--> request.userId = HappyHerd account id
  |
...
Update `packages/happyherd-server/sources/app/api/routes/voiceRoutes.ts` to:
...
- `packages/happyherd-app/sources/realtime/types.ts`
- `packages/happyherd-app/sources/realtime/RealtimeVoiceSession.tsx`
- `packages/happyherd-app/sources/realtime/RealtimeVoiceSession.web.tsx`
- `packages/happyherd-app/sources/realtime/RealtimeSession.ts`
- `packages/happyherd-app/sources/sync/apiVoice.ts`
