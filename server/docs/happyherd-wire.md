...
This document describes the shared wire package: `@happyherd/wire`.

## Why this package exists

Before `happyherd-wire`, wire-level message and session-protocol schemas were duplicated across packages (CLI, app, server, and agent). That caused drift risk and made protocol evolution harder.

`@happyherd/wire` centralizes those shared schemas and types so all clients and services agree on the same wire contract.

## Package identity

- npm name: `@happyherd/wire`
- workspace path: `packages/happyherd-wire`
...
- Session protocol imports now reference `@happyherd/wire` directly.
- `src/sessionProtocol/types.ts` now re-exports from `@happyherd/wire` as compatibility shim.
- API wire schemas in `src/api/types.ts` now source shared message/update schemas from `@happyherd/wire`.

### App (`packages/happyherd-app`)

- Shared API message/update schemas in `sources/sync/apiTypes.ts` now import these from `@happyherd/wire`:
  - `ApiMessageSchema`
  - `ApiUpdateNewMessageSchema`
  - `ApiUpdateSessionStateSchema`
  - `ApiUpdateMachineStateSchema`

### Server (`packages/happyherd-server`)

- Prisma JSON message content type now references `SessionMessageContent` from `@happyherd/wire`.
- Event router uses shared `SessionMessageContent` type for `new-message` payload typing.

### Agent (`packages/happyherd-control-agent`)

- `RawMessage` now aliases `SessionMessage` from `@happyherd/wire`.

## Versioning model

All other workspace packages now declare a versioned dependency on `@happyherd/wire`.

This intentionally mirrors post-publish consumption and reduces hidden coupling to workspace-local files.

## Build and release

`@happyherd/wire` is configured the same way as existing publishable libraries in this repo:
...
```bash
yarn release
# choose happyherd-wire
```

or:

```bash
yarn workspace @happyherd/wire release
```

When building workspaces from a clean checkout, build `@happyherd/wire` first so dependent packages can resolve generated `dist` outputs.
...
3. Bump and release `@happyherd/wire`.
4. Update downstream package versions if needed.
5. Publish dependent package updates only after the new `happyherd-wire` version is available.

## Notes

- `happyherd-wire` should stay focused on wire contracts only (types + Zod schemas + small helpers).
