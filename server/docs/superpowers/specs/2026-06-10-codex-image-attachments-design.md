...
Give Codex image attachment parity with Claude in HappyHerd sessions. A user should be able to attach one or more images to a Codex message from the HappyHerd app, send the message, and have Codex receive the images together with the text.

## Scope

- Reuse the existing HappyHerd attachment pipeline: encrypted blob upload, session `file` event, and inline image rendering in chat history.
...
- Keep history, resume, and fork behavior safe: show HappyHerd-owned `file` events when they exist, and avoid inventing image records when only an unavailable provider-local path remains.
...
- Rebuild Codex provider history as the source of truth for HappyHerd attachments.
- Store decrypted attachment bytes in HappyHerd server storage or session metadata.
...
Codex fork backfill currently happens only on the `HAPPYHERD_FORK_CODEX_THREAD_ID` path in `runCodex.ts`, where the runner calls `readThread` and maps provider turns through `mapCodexThreadToSessionEnvelopes`. Direct `happyherd codex --resume <threadId>` currently resumes the provider thread but does not replay provider turns into a new HappyHerd session.

## Architecture

Use HappyHerd attachments as the source of truth and Codex `localImage` items as the delivery mechanism.
...
HappyHerd history is authoritative for images that were sent through HappyHerd. Because the app stores `file` events before text, reopening the same HappyHerd session should render images from those stored encrypted refs without relying on Codex provider history.

Codex thread backfill should continue to restore text, agent messages, reasoning, and tools from provider history. It should also restore provider `localImage` inputs when the local cached file still exists and the target HappyHerd session does not already contain the matching image event.

If a Codex thread item contains provider image input during a provider-history backfill:

- If HappyHerd already has a matching `file` event in the session stream, prefer that stored event.
- If no matching HappyHerd `file` event exists, the provider item is `localImage`, and the file still exists locally, backfill uploads it through the normal encrypted attachment path and emits a `file` event.
...
This gives good behavior for HappyHerd-originated sessions while avoiding false history records when provider-local files have disappeared.

Fork backfill is required because the current Codex fork flow creates a new HappyHerd session from provider thread history. Direct `happyherd codex --resume <threadId>` provider-history image backfill is allowed but not required for the first implementation unless the implementation already extends that path to read provider turns. Existing HappyHerd-session reopen and reconnect flows must continue to use stored HappyHerd `file` events.
...
Cached plaintext image files are local to the machine running the CLI. They are not uploaded to HappyHerd server, are not referenced from HappyHerd server metadata, and do not include original user file names.
...
- Codex fork backfill uploads existing local-image cache files into the new HappyHerd session and degrades safely when provider-local image paths are unavailable
